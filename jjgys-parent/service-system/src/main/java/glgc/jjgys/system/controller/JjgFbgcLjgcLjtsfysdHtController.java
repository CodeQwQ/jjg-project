...
public class JjgFbgcLjgcLjtsfysdHtController {
    ...
    public Result generateJdb(@RequestBody CommonInfoVo commonInfoVo) throws Exception {
        boolean is_Success = jjgFbgcLjgcLjtsfysdHtService.generateJdb(commonInfoVo);
    ...
    public Result findQueryPageHt(@PathVariable long current,
                                ...
                                @RequestBody JjgFbgcLjgcLjtsfysdHt jjgFbgcLjgcLjtsfysdHt) throws ParseException {
        //创建page对象
        ...
        if (jjgFbgcLjgcLjtsfysdHt != null){
            ...
            String userid = jjgFbgcLjgcLjtsfysdHt.getUserid();
            if ("1".equals(userid)){
            ...
            if (!StringUtils.isEmpty(sysj)){
                ...
                Date date = sdf.parse(sysj.toString());
    ...
    public Result removeAll(@RequestBody CommonInfoVo commonInfoVo){
        ...
        if ("1".equals(userid)){
            ...
            if (remove){
                File f = new File(filespath+File.separator+proname+File.separator+htd+File.separator+"01路基土石方压实度.xlsx");
                if (f.exists()){
                    f.delete();
                }
            }
    ...
    @PostMapping("ht/createManyRecords")
    public Result createManyRecords(@RequestBody List<JjgFbgcLjgcLjtsfysdHt> rawData, HttpServletRequest request) {
        String userID = ReceiveUtils.getUserIDFromRequest(request);
        if (userID == null) {
            return Result.fail("用户未登录");
        }
        int res = jjgFbgcLjgcLjtsfysdHtService.createMoreRecords(rawData, userID);
