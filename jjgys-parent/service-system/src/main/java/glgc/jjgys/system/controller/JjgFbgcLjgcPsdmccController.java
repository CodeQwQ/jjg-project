...
public class JjgFbgcLjgcPsdmccController {
    ...
    public Result generateJdb(@RequestBody CommonInfoVo commonInfoVo) throws Exception {
        boolean is_Success = jjgFbgcLjgcPsdmccService.generateJdb(commonInfoVo);
    ...
    public Result findQueryPage(@PathVariable long current,
                                ...
                                @RequestBody JjgFbgcLjgcPsdmcc jjgFbgcLjgcPsdmcc) throws ParseException {
        //创建page对象
        ...
        if (jjgFbgcLjgcPsdmcc != null){
            ...
            String userid = jjgFbgcLjgcPsdmcc.getUserid();
            if ("1".equals(userid)){
    ...
    public Result removeAll(@RequestBody CommonInfoVo commonInfoVo){
        ...
        if ("1".equals(userid)){
            ...
            if (remove){
                File f = new File(filespath+File.separator+proname+File.separator+htd+File.separator+"04路基排水断面尺寸.xlsx");
                if (f.exists()){
                    f.delete();
                }
            }
    ...
    public Result createManyRecords(@RequestBody List<JjgFbgcLjgcPsdmcc> rawData, HttpServletRequest request) {
        String userID = ReceiveUtils.getUserIDFromRequest(request);
        if (userID == null) {
            return Result.fail("用户未登录");
        }
        int res = jjgFbgcLjgcPsdmccService.createMoreRecords(rawData, userID);
