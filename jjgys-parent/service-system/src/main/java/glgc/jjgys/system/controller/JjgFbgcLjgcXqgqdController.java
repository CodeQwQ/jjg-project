...
public class JjgFbgcLjgcXqgqdController {
    ...
    public Result generateJdb(@RequestBody CommonInfoVo commonInfoVo) throws Exception {
        boolean is_Success = jjgFbgcLjgcXqgqdService.generateJdb(commonInfoVo);
    ...
    public Result findQueryPage(@PathVariable long current,
                                ...
                                @RequestBody JjgFbgcLjgcXqgqd jjgFbgcLjgcXqgqd) throws ParseException {
        //创建page对象
        ...
        if(jjgFbgcLjgcXqgqd != null){
            ...
            String userid = jjgFbgcLjgcXqgqd.getUserid();
            if ("1".equals(userid)){
...
//            wrapper.like("fbgc",jjgFbgcLjgcHdgqd.getFbgc());
            //调用方法分页查询
            System.out.println(jjgFbgcLjgcXqgqdService.list(wrapper));
    ...
    public Result removeAll(@RequestBody CommonInfoVo commonInfoVo){
        ...
        if ("1".equals(userid)){
            ...
            if (remove){
                File f = new File(filespath+File.separator+proname+File.separator+htd+File.separator+"06路基小桥砼强度.xlsx");
                if (f.exists()){
                    f.delete();
                }
            }
    ...
    public Result createManyRecords(@RequestBody List<JjgFbgcLjgcXqgqd> rawData, HttpServletRequest request) {
        String userID = ReceiveUtils.getUserIDFromRequest(request);
        if (userID == null) {
            return Result.fail("用户未登录");
        }
        int res = jjgFbgcLjgcXqgqdService.createMoreRecords(rawData, userID);
