...
public class JjgFbgcLjgcZdgqdController {
    ...
    public Result generateJdb(@RequestBody CommonInfoVo commonInfoVo) throws Exception {
        boolean is_Success = jjgFbgcLjgcZdgqdService.generateJdb(commonInfoVo);
    ...
    public Result removeAll(@RequestBody CommonInfoVo commonInfoVo){
        ...
        if ("1".equals(userid)){
            ...
            if (remove){
                File f = new File(filespath+File.separator+proname+File.separator+htd+File.separator+"10路基支挡砼强度.xlsx");
                if (f.exists()){
                    f.delete();
                }
            }
    ...
    public Result findQueryPage(@PathVariable long current,
                                ...
                                @RequestBody JjgFbgcLjgcZdgqd jjgFbgcLjgcZdgqd) throws ParseException {
        //创建page对象
        ...
        if (jjgFbgcLjgcZdgqd != null){
            ...
            String userid = jjgFbgcLjgcZdgqd.getUserid();
            if ("1".equals(userid)){
    ...
    public Result createManyRecords(@RequestBody List<JjgFbgcLjgcZdgqd> rawData, HttpServletRequest request) {
        String userID = ReceiveUtils.getUserIDFromRequest(request);
        if (userID == null) {
            return Result.fail("用户未登录");
        }
        int res = jjgFbgcLjgcZdgqdService.createMoreRecords(rawData, userID);
