...
public class JjgFbgcLmgcHntlmhdzxfController {
    ...
    public Result generateJdb(@RequestBody CommonInfoVo commonInfoVo) throws Exception {
        boolean is_Success = jjgFbgcLmgcHntlmhdzxfService.generateJdb(commonInfoVo);
    ...
    public Result findQueryPage(@PathVariable long current,
                                ...
                                @RequestBody JjgFbgcLmgcHntlmhdzxf jjgFbgcLmgcHntlmhdzxf) throws ParseException {
        //创建page对象
        ...
        if (jjgFbgcLmgcHntlmhdzxf != null){
            ...
            String userid = jjgFbgcLmgcHntlmhdzxf.getUserid();
            if ("1".equals(userid)){
    ...
    public Result removeAll(@RequestBody CommonInfoVo commonInfoVo){
        ...
        if ("1".equals(userid)){
            ...
            if (remove){
                File f = new File(filespath+File.separator+proname+File.separator+htd+File.separator+"23混凝土路面厚度-钻芯法.xlsx");
                if (f.exists()){
                    f.delete();
                }
            }
    ...
    public Result createManyRecords(@RequestBody List<JjgFbgcLmgcHntlmhdzxf> rawData, HttpServletRequest request) {
        String userID = ReceiveUtils.getUserIDFromRequest(request);
        if (userID == null) {
            return Result.fail("用户未登录");
        }
        int res = jjgFbgcLmgcHntlmhdzxfService.createMoreRecords(rawData, userID);
