...
public class JjgFbgcLmgcTlmxlbgcController {
    ...
    public Result generateJdb(@RequestBody CommonInfoVo commonInfoVo) throws Exception {
        boolean is_Success = jjgFbgcLmgcTlmxlbgcService.generateJdb(commonInfoVo);
    ...
    public Result findQueryPage(@PathVariable long current,
                                ...
                                @RequestBody JjgFbgcLmgcTlmxlbgc jjgFbgcLmgcTlmxlbgc) throws ParseException {
        //创建page对象
        ...
        if (jjgFbgcLmgcTlmxlbgc != null){
            ...
            String userid = jjgFbgcLmgcTlmxlbgc.getUserid();
            if ("1".equals(userid)){
    ...
    public Result removeAll(@RequestBody CommonInfoVo commonInfoVo){
        ...
        if ("1".equals(userid)){
            ...
            if (remove){
                File f = new File(filespath+File.separator+proname+File.separator+htd+File.separator+"17混凝土路面相邻板高差.xlsx");
                if (f.exists()){
                    f.delete();
                }
            }
    ...
    public Result createManyRecords(@RequestBody List<JjgFbgcLmgcTlmxlbgc> rawData, HttpServletRequest request) {
        String userID = ReceiveUtils.getUserIDFromRequest(request);
        if (userID == null) {
            return Result.fail("用户未登录");
        }
        int res = jjgFbgcLmgcTlmxlbgcService.createMoreRecords(rawData, userID);
