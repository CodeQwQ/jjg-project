...
public class JjgFbgcQlgcSbJgccController {
    ...
    public Result generateJdb(@RequestBody CommonInfoVo commonInfoVo) throws Exception {
        boolean is_Success = jjgFbgcQlgcSbJgccService.generateJdb(commonInfoVo);
    ...
    public Result findQueryPage(@PathVariable long current,
                                ...
                                @RequestBody JjgFbgcQlgcSbJgcc jjgFbgcQlgcSbJgcc) throws ParseException {
        ...
        if (jjgFbgcQlgcSbJgcc != null){
            ...
            String userid = jjgFbgcQlgcSbJgcc.getUserid();
            if ("1".equals(userid)){
    ...
    public Result removeAll(@RequestBody CommonInfoVo commonInfoVo){
        ...
        if ("1".equals(userid)){
            ...
            if (remove){
                File f = new File(filespath+File.separator+proname+File.separator+htd+File.separator+"30桥梁上部主要结构尺寸.xlsx");
                if (f.exists()){
                    f.delete();
                }
            }
    ...
    public Result createManyRecords(@RequestBody List<JjgFbgcQlgcSbJgcc> rawData, HttpServletRequest request) {
        String userID = ReceiveUtils.getUserIDFromRequest(request);
        if (userID == null) {
            return Result.fail("用户未登录");
        }
        int res = jjgFbgcQlgcSbJgccService.createMoreRecords(rawData, userID);
