...
public class JjgFbgcQlgcXbBhchdController {
    ...
    public Result generateJdb(@RequestBody CommonInfoVo commonInfoVo) throws Exception {
        boolean is_Success = jjgFbgcQlgcXbBhchdService.generateJdb(commonInfoVo);
    ...
    public Result findQueryPage(@PathVariable long current,
                                ...
                                @RequestBody JjgFbgcQlgcXbBhchd jjgFbgcQlgcXbBhchd) throws ParseException {
        //创建page对象
        ...
        if (jjgFbgcQlgcXbBhchd != null){
            ...
            String userid = jjgFbgcQlgcXbBhchd.getUserid();
            if ("1".equals(userid)){
    ...
    public Result removeAll(@RequestBody CommonInfoVo commonInfoVo){
        ...
        if ("1".equals(userid)){
            ...
            if (remove){
                File f = new File(filespath+File.separator+proname+File.separator+htd+File.separator+"27桥梁下部保护层厚度.xlsx");
                if (f.exists()){
                    f.delete();
                }
            }
    ...
    public Result createManyRecords(@RequestBody List<JjgFbgcQlgcXbBhchd> rawData, HttpServletRequest request) {
        String userID = ReceiveUtils.getUserIDFromRequest(request);
        if (userID == null) {
            return Result.fail("用户未登录");
        }
        int res = jjgFbgcQlgcXbBhchdService.createMoreRecords(rawData, userID);
