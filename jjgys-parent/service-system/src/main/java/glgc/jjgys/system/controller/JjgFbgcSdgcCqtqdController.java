...
public class JjgFbgcSdgcCqtqdController {
    ...
    public Result generateJdb(@RequestBody CommonInfoVo commonInfoVo) throws Exception {
        boolean is_Success = jjgFbgcSdgcCqtqdService.generateJdb(commonInfoVo);
    ...
    public Result removeAll(@RequestBody CommonInfoVo commonInfoVo){
        ...
        if ("1".equals(userid)){
            ...
            if (remove){
                File f = new File(filespath+File.separator+proname+File.separator+htd+File.separator+"38隧道衬砌砼强度.xlsx");
                if (f.exists()){
                    f.delete();
                }
            }
    ...
    public Result findQueryPage(@PathVariable long current,
                                ...
                                @RequestBody JjgFbgcSdgcCqtqd jjgFbgcSdgcCqtqd) throws ParseException {
        //创建page对象
        ...
        if (jjgFbgcSdgcCqtqd != null){
            ...
            String userid = jjgFbgcSdgcCqtqd.getUserid();
            if ("1".equals(userid)){
    ...
    public Result createManyRecords(@RequestBody List<JjgFbgcSdgcCqtqd> rawData, HttpServletRequest request) {
        String userID = ReceiveUtils.getUserIDFromRequest(request);
        if (userID == null) {
            return Result.fail("用户未登录");
        }
        int res = jjgFbgcSdgcCqtqdService.createMoreRecords(rawData, userID);
