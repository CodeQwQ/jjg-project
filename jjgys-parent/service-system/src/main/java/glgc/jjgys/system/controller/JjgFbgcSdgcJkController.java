...
public class JjgFbgcSdgcJkController {
    ...
    public Result generateJdb(@RequestBody CommonInfoVo commonInfoVo) throws Exception {
        boolean is_Success = jjgFbgcSdgcJkService.generateJdb(commonInfoVo);
    ...
    public Result findQueryPage(@PathVariable long current,
                                ...
                                @RequestBody JjgFbgcSdgcJk jjgFbgcSdgcJk) throws ParseException {
        //创建page对象
        ...
        if (jjgFbgcSdgcJk != null){
            ...
            String userid = jjgFbgcSdgcJk.getUserid();
            if ("1".equals(userid)){
    ...
    public Result removeAll(@RequestBody CommonInfoVo commonInfoVo){
        ...
        if ("1".equals(userid)){
            ...
            if (list!=null){
                ...
                path.add(filespath+File.separator+proname+File.separator+"隧道净空实测数据"+File.separator+"隧道净空实测数据"+File.separator+"隧道净空实测数据.xlsx");
                path.add(filespath+File.separator+proname+File.separator+"隧道净空实测数据"+File.separator+"隧道净空实测数据"+File.separator);
                path.add(filespath+File.separator+proname+File.separator+"隧道净空实测数据"+File.separator);
                deleteLocalFile(path);
            }
            ...
            for (int i=0;i<sdlist.size();i++){
                st.add("42隧道断面测量坐标表-"+sdlist.get(i).getSdname()+".xlsx");
            }
            list1.addAll(st);
