...
public class JjgFbgcJtaqssJabxServiceImpl extends ServiceImpl<JjgFbgcJtaqssJabxMapper, JjgFbgcJtaqssJabx> implements JjgFbgcJtaqssJabxService {
    ...
    public boolean generateJdb(CommonInfoVo commonInfoVo) throws Exception {
        ...
        if (data == null || data.size()==0){
            ...
            try {
                ...
                inputStream.close();
                //查询是否有白线逆反射系数
                List<JjgFbgcJtaqssJabx> bxnfsxsdata = jjgFbgcJtaqssJabxMapper.selectbxnfsxs(proname,htd,fbgc,idlist);
                List<JjgFbgcJtaqssJabx> hxnfsxsdata = jjgFbgcJtaqssJabxMapper.selecthxnfsxs(proname,htd,fbgc,idlist);
                bxnfsxs(data);
    ...
    public boolean DBtohxnfsxsExcel(List<JjgFbgcJtaqssJabx> data,XSSFWorkbook xwb,String proname,String htd,String fbgc) throws ParseException {
        ...
        for(JjgFbgcJtaqssJabx row:data){
            if(!type.contains(row.getBxlx())) {
                type+=row.getBxlx()+",";
                error.put(row.getBxlx(), row.getHxnfsxsgdz());
            }else if(!error.get(row.getBxlx()).contains(row.getHxnfsxsgdz())) {
                error.put(row.getBxlx(), error.get(type)+","+row.getHxnfsxsgdz());
            }
            z++;
        }
    ...
    public void bxnfsxs(List<JjgFbgcJtaqssJabx> data) throws IOException, ParseException {
        ...
        String fbgc = data.get(0).getFbgc();
        XSSFWorkbook xwb = null;
        File f = new File(filepath+File.separator+proname+File.separator+htd+File.separator+"57交安标线逆反射系数.xlsx");
        if (data != null && data.size()>0){
            //存放鉴定表的目录
            File fdir = new File(filepath + File.separator + proname + File.separator + htd);
            if (!fdir.exists()) {
    ...
    public boolean DBtobxnfsxsExcel(List<JjgFbgcJtaqssJabx> data,XSSFWorkbook xwb,String proname,String htd,String fbgc) throws ParseException {
        Collections.sort(data, new Comparator<JjgFbgcJtaqssJabx>() {
            @Override
            public int compare(JjgFbgcJtaqssJabx o1, JjgFbgcJtaqssJabx o2) {
                // 名字相同时按照 qdzh 排序
                String zh = o1.getWz();
                String zh1 = o2.getWz();
    ...
    public void importjabx(MultipartFile file, CommonInfoVo commonInfoVo) {
        ...
        try {
                    ...
                    .registerReadListener(
                            new ExcelHandler<JjgFbgcJtaqssJabxVo>(JjgFbgcJtaqssJabxVo.class) {
                                ...
                                public void handle(List<JjgFbgcJtaqssJabxVo> dataList) {
                                    ...
                                    if (size % 5 != 0){
                                        throw new JjgysException(20001,"您上传的"+htd+"合同段中02交安标线实测数据.xlsx文件，数据量错误，请确保每个桩号有5条数据");
                                    }
                                    int rowNumber=2;
                                    for(JjgFbgcJtaqssJabxVo jabxVo: dataList)
                                    {
                                        if (StringUtils.isEmpty(jabxVo.getBxlx())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中02交安标线实测数据.xlsx,第"+rowNumber+"行的数据中，标线类型为空，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(jabxVo.getWz())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中02交安标线实测数据.xlsx,第"+rowNumber+"行的数据中，位置为空，请修改后重新上传");
                                        }
                                        /*if (!StringUtils.isNumeric(jabxVo.getHdgdz()) || StringUtils.isEmpty(jabxVo.getHdgdz())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中02交安标线实测数据.xlsx,第"+rowNumber+"行的数据中，厚度规定值有误，请修改后重新上传");
                                        }
                                        if (jabxVo.getHdyxpsz().contains("+")) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中02交安标线实测数据.xlsx,第"+rowNumber+"行的数据中，厚度允许偏差+值有误(删除+)，请修改后重新上传");
                                        }
                                        if (jabxVo.getHdyxpsf().contains("-")) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中02交安标线实测数据.xlsx,第"+rowNumber+"行的数据中，厚度允许偏差-值有误(删除-)，请修改后重新上传");
                                        }*/

                                        JjgFbgcJtaqssJabx fbgcJtaqssJabx = new JjgFbgcJtaqssJabx();
                                        BeanUtils.copyProperties(jabxVo,fbgcJtaqssJabx);
                                        fbgcJtaqssJabx.setUserid(commonInfoVo.getUserid());
            ...
            throw new JjgysException(20001,"您上传的"+htd+"合同段中02交安标线实测数据.xlsx,解析excel出错，请传入正确格式的excel");
        }catch (NullPointerException e) {
            throw new JjgysException(20001,"您上传的"+htd+"合同段中02交安标线实测数据.xlsx,请检查数据的正确性或删除文件最后的空数据，然后重新上传");
        }catch (ExcelAnalysisException e) {
            throw new JjgysException(20001,"您上传的"+htd+"合同段中02交安标线实测数据.xlsx,请将检测日期修改为2021/01/01的格式，然后重新上传");
        }

    }

    @Override
    public int createMoreRecords(List<JjgFbgcJtaqssJabx> data, String userID) {
        return ReceiveUtils.createMore(data, jjgFbgcJtaqssJabxMapper, userID);
    }
