...
public class JjgFbgcJtaqssJabzServiceImpl extends ServiceImpl<JjgFbgcJtaqssJabzMapper, JjgFbgcJtaqssJabz> implements JjgFbgcJtaqssJabzService {
    ...
    public void importjabz(MultipartFile file, CommonInfoVo commonInfoVo) {
        ...
        try {
                                ...
                                public void handle(List<JjgFbgcJtaqssJabzVo> dataList) {
                                    ...
                                    for(JjgFbgcJtaqssJabzVo jabzVo: dataList)
                                    {
                                        if (StringUtils.isEmpty(jabzVo.getWz())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中01交安标志实测数据.xlsx文件，第"+rowNumber+"行的数据中，位置为空，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(jabzVo.getLzlx())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中01交安标志实测数据.xlsx文件，第"+rowNumber+"行的数据中，立柱类型为空，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(jabzVo.getSzdyxps())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中01交安标志实测数据.xlsx文件，第"+rowNumber+"行的数据中，竖直度允许偏差为空，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(jabzVo.getHdyxps())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中01交安标志实测数据.xlsx文件，第"+rowNumber+"行的数据中，厚度允许偏差为空，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(jabzVo.getJkgdz()) || StringUtils.isEmpty(jabzVo.getJkgdz())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中01交安标志实测数据.xlsx文件，第"+rowNumber+"行的数据中，净空规定值(mm)值有误，请修改后重新上传");
                                        }
                                        ...
                                        fbgcJtaqssJabz.setUserid(commonInfoVo.getUserid());
            ...
            throw new JjgysException(20001,"您上传的"+htd+"合同段中01交安标志实测数据.xlsx文件解析excel出错，请传入正确格式的excel");
        }catch (NullPointerException e) {
            throw new JjgysException(20001,"您上传的"+htd+"合同段中01交安标志实测数据.xlsx文件，请检查数据的正确性或删除文件最后的空数据，然后重新上传");
        }catch (ExcelAnalysisException e) {
            throw new JjgysException(20001,"您上传的"+htd+"合同段中01交安标志实测数据.xlsx文件，请将检测日期修改为2021/01/01的格式，然后重新上传");
        }
    ...
    public void exportjabz(HttpServletResponse response) {
        ...
        try {
            ...
            File t = new File(courseFile+File.separator+"01交安标志实测数据.xlsx");
            if (t.exists()){
                t.delete();
            }
        }
    ...
    public boolean generateJdb(CommonInfoVo commonInfoVo) throws IOException, ParseException {
        ...
        if (data == null || data.size() == 0) {
            ...
            try {
                ...
                InputStream inputStream = getClass().getClassLoader().getResourceAsStream("static/标志.xlsx");
    ...
    private void fillCommonCellData(XSSFSheet sheet, int tableNum, int index, JjgFbgcJtaqssJabz row) {
        ...
        //蓝色V类
        if(!"".equals(row.getLasvlyxps()) && row.getLasvlyxps()!=null){
            //sheet.getRow(tableNum*(17+2)+18).getCell(3).setCellValue(row.getLsvlyxps());
            if(row.getLasvlyxps().contains("≥")){
                sheet.getRow(tableNum*(17+2)+18).getCell(3).setCellValue(row.getLasvlyxps());
            }
            else{
                sheet.getRow(tableNum*(17+2)+18).getCell(3).setCellValue("≥"+row.getLasvlyxps());
            }
        }
        if(!"".equals(row.getLasvlscz1()) && row.getLasvlscz1()!=null){
            sheet.getRow(tableNum*(17+2)+18).getCell(index).setCellValue(Double.valueOf(row.getLasvlscz1()));
        }else{
            sheet.getRow(tableNum*(17+2)+18).getCell(index).setCellValue("-");
        }
        if(!"".equals(row.getLasvlscz2()) && row.getLasvlscz2()!=null){
            sheet.getRow(tableNum*(17+2)+18).getCell(index+1).setCellValue(Double.valueOf(row.getLasvlscz2()));
        }else{
            sheet.getRow(tableNum*(17+2)+16).getCell(index+1).setCellValue("-");
        }
        //蓝色IV类
        if(!"".equals(row.getLaswlyxps()) && row.getLaswlyxps()!=null){
            if(row.getLaswlyxps().contains("≥")){
                sheet.getRow(tableNum*(17+2)+19).getCell(3).setCellValue(row.getLaswlyxps());
            }
            else{
                sheet.getRow(tableNum*(17+2)+19).getCell(3).setCellValue("≥"+row.getLaswlyxps());
            }
        ...
        if(!"".equals(row.getLaswlscz1()) && row.getLaswlscz1()!=null){
            sheet.getRow(tableNum*(17+2)+19).getCell(index).setCellValue(Double.valueOf(row.getLaswlscz1()));
        }else{
            sheet.getRow(tableNum*(17+2)+19).getCell(index).setCellValue("-");
        }
        if(!"".equals(row.getLaswlscz2()) && row.getLaswlscz2()!=null){
            sheet.getRow(tableNum*(17+2)+19).getCell(index+1).setCellValue(Double.valueOf(row.getLaswlscz2()));
        }else{
    ...
    @Override
    public int createMoreRecords(List<JjgFbgcJtaqssJabz> data, String userID) {
        return ReceiveUtils.createMore(data, jjgFbgcJtaqssJabzMapper, userID);
    }
