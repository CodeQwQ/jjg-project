...
public class JjgFbgcLjgcLjtsfysdHtServiceImpl extends ServiceImpl<JjgFbgcLjgcLjtsfysdHtMapper, JjgFbgcLjgcLjtsfysdHt> implements JjgFbgcLjgcLjtsfysdHtService {
    ...
    public boolean generateJdb(CommonInfoVo commonInfoVo) throws IOException {
        // 把全局变量两个评定数据的hash清空,另外一个在写入的时候进行初始化，因为在另一个service里面
        evaluateDataht.clear();
        ...
        List<JjgFbgcLjgcLjtsfysdSl> sldata = jjgFbgcLjgcLjtsfysdSlService.getdata(proname, htd, fbgc,idlist);
        //鉴定表要存放的路径
        ...
        if (htdata.size() <= 0 && sldata.size() <= 0){
            ...
            try {
                ...
                InputStream inputStream = getClass().getClassLoader().getResourceAsStream("static/路基压实度.xlsx");
                ...
                //沙砾
                LinkedHashMap<String, ArrayList<String>> EvalSLdata = jjgFbgcLjgcLjtsfysdSlService.writeAndGetData(xwb,proname,htd,fbgc,idlist);
                //灰土
                List<String> numsList = jjgFbgcLjgcLjtsfysdHtMapper.selectNums(proname,htd,fbgc,idlist);
    ...
    public void fillData(LinkedHashMap<String,ArrayList<String>> Data,XSSFWorkbook wb,String sheetName, String dataSheetName, XSSFCellStyle cellstyle){
        ...
        for(Map.Entry<String, ArrayList<String>> entry : Data.entrySet()){
            ...
            if((endrow-5)/29 > (startrow-5)/29){
                ...
                wb.getSheet(sheetName).getRow(startrow).getCell(1).setCellValue(pile.substring(0, pile.length()));
                ...
                wb.getSheet(sheetName).getRow((startrow-5)/29*29+29+5).getCell(1).setCellValue(pile.substring(0, pile.length()));
    ...
    public void exportysdht(HttpServletResponse response) throws IOException {
        ...
        try {
            ...
            String filename = "01路基土石方压实度_灰土实测数据.xlsx";// 设置下载时客户端Excel的名称
            ...
            File t = new File(courseFile+File.separator+"01路基土石方压实度_灰土实测数据.xlsx");
            if (t.exists()){
                t.delete();
            }
        }
    ...
    public void importysdht(MultipartFile file, CommonInfoVo commonInfoVo) throws IOException, ParseException {
        ...
        if(checklist.equals(titlelist)){
            for (int j = 1;j<columns;j++){//列
                ...
                jjgFbgcLjgcLjtsfysdHt.setUserid(commonInfoVo.getUserid());
            ...
            throw new JjgysException(20001,"您上传的"+commonInfoVo.getHtd()+"合同段中01路基土石方压实度_灰土实测数据.xlsx文件，解析excel出错，请传入正确格式的excel");
        }
    }
    ...
    @Override
    public int createMoreRecords(List<JjgFbgcLjgcLjtsfysdHt> data, String userID) {
        return ReceiveUtils.createMore(data, jjgFbgcLjgcLjtsfysdHtMapper, userID);
    }
