...
public class JjgFbgcLjgcLjtsfysdSlServiceImpl extends ServiceImpl<JjgFbgcLjgcLjtsfysdSlMapper, JjgFbgcLjgcLjtsfysdSl> implements JjgFbgcLjgcLjtsfysdSlService {
    ...
    @Override
    public LinkedHashMap<String, ArrayList<String>> writeAndGetData(XSSFWorkbook wb, String proname, String htd, String fbgc, List<Long> idlist) {
        //try {
            evaluateData.clear();
            //获取砂砾的数据
            List<JjgFbgcLjgcLjtsfysdSl> sldata = getdata(proname, htd, fbgc, idlist);
            //获取要生成页数的数量
            List<String> numsList = jjgFbgcLjgcLjtsfysdSlMapper.selectNums(proname, htd, fbgc,idlist);
    ...
    private int gettableSlNum(List<String> numlist) {
        ...
        for (String i: numlist){
            ...
            tableNum += dataNum%4 == 0 ? dataNum/4 : dataNum/4+1;
    ...
    public void fillCommonCellData(XSSFWorkbook wb,XSSFSheet sheet, int tableNum, int index, JjgFbgcLjgcLjtsfysdSl row){
        ...
        try {
            sheet.getRow(tableNum*34+19-2).getCell(3+index*2).setCellValue(Double.parseDouble(row.getHh()));//盒号
        }catch(Exception e){
    ...
    public void fillTitleCellData(XSSFSheet sheet, int tableNum, JjgFbgcLjgcLjtsfysdSl row, String position,String projectname,String htd){
        ...
        sheet.getRow(tableNum*34+3).getCell(2).setCellValue(position);//桩号
     ...
     * @return
     */
    public List<JjgFbgcLjgcLjtsfysdSl> getdata(String proname, String htd, String fbgc, List<Long> idlist){
    ...
    public void exportysdsl(HttpServletResponse response) throws IOException {
        ...
        try {
            ...
            String filename = "01路基土石方压实度_砂砾实测数据.xlsx";// 设置下载时客户端Excel的名称
            ...
            File t = new File(courseFile+File.separator+"01路基土石方压实度_砂砾实测数据.xlsx");
            if (t.exists()){
                t.delete();
            }
        }
    ...
    public void importysdsl(MultipartFile file, CommonInfoVo commonInfoVo) throws IOException, ParseException {
        ...
        if(checklist.equals(titlelist)){
            for (int j = 1;j<columns;j++){//列
                ...
                jjgFbgcLjgcLjtsfysdSl.setUserid(commonInfoVo.getUserid());
                jjgFbgcLjgcLjtsfysdSl.setCreatetime(new Date());
                jjgFbgcLjgcLjtsfysdSlMapper.insert(jjgFbgcLjgcLjtsfysdSl);
            }
        }else {
            throw new JjgysException(20001,"您上传的"+commonInfoVo.getHtd()+"合同段中01路基土石方压实度_砂砾实测数据.xlsx文件，解析excel出错，请传入正确格式的excel");
        }

    }

    @Override
    public int createMoreRecords(List<JjgFbgcLjgcLjtsfysdSl> data, String userID) {
        return ReceiveUtils.createMore(data, jjgFbgcLjgcLjtsfysdSlMapper, userID);
    }
}
