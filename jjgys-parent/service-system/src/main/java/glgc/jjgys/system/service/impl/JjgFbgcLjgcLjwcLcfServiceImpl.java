...
public class JjgFbgcLjgcLjwcLcfServiceImpl extends ServiceImpl<JjgFbgcLjgcLjwcLcfMapper, JjgFbgcLjgcLjwcLcf> implements JjgFbgcLjgcLjwcLcfService {
    ...
    public boolean generateJdb(CommonInfoVo commonInfoVo) throws IOException {
        ...
        if (data == null || data.size()==0){
            ...
            try {
                ...
                InputStream inputStream = getClass().getClassLoader().getResourceAsStream("static/路基弯沉(落锤法).xlsx");
    ...
    public boolean DBtoExcel(List<JjgFbgcLjgcLjwcLcf> data,XSSFWorkbook wb) throws IOException {
        Collections.sort(data, new Comparator<JjgFbgcLjgcLjwcLcf>() {
            @Override
            public int compare(JjgFbgcLjgcLjwcLcf o1, JjgFbgcLjgcLjwcLcf o2) {
                // 名字相同时按照 qdzh 排序
    ...
    public void exportljwclcf(HttpServletResponse response) {
        ...
        try {
            ...
            File t = new File(courseFile+File.separator+"02路基弯沉(落锤法)实测数据.xlsx");
            if (t.exists()){
                t.delete();
            }
        }
    ...
    public void importljwclcf(MultipartFile file, CommonInfoVo commonInfoVo) {
        ...
        try {
                                ...
                                public void handle(List<JjgFbgcLjgcLjwcLcfVo> dataList) {
                                    ...
                                    for(JjgFbgcLjgcLjwcLcfVo ljwclcfVo: dataList)
                                    {
                                        if (StringUtils.isEmpty(ljwclcfVo.getZh())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中02路基弯沉(落锤法)实测数据.xlsx文件，第"+rowNumber+"行的数据中，桩号为空，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(ljwclcfVo.getSjwcz()) || StringUtils.isEmpty(ljwclcfVo.getSjwcz())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中02路基弯沉(落锤法)实测数据.xlsx文件，第"+rowNumber+"行的数据中，设计弯沉值有误，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(ljwclcfVo.getMbkkzb())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中02路基弯沉(落锤法)实测数据.xlsx文件，第"+rowNumber+"行的数据中，目标可靠指标值有误，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(ljwclcfVo.getWdyxxs())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中02路基弯沉(落锤法)实测数据.xlsx文件，第"+rowNumber+"行的数据中，温度影响系数值有误，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(ljwclcfVo.getSdyxxs())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中02路基弯沉(落锤法)实测数据.xlsx文件，第"+rowNumber+"行的数据中，湿度影响系数值有误，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(ljwclcfVo.getJjyxxs())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中02路基弯沉(落锤法)实测数据.xlsx文件，第"+rowNumber+"行的数据中，季节影响系数值有误，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(ljwclcfVo.getJglx())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中02路基弯沉(落锤法)实测数据.xlsx文件，第"+rowNumber+"行的数据中，结构类型为空，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(ljwclcfVo.getJgcc())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中02路基弯沉(落锤法)实测数据.xlsx文件，第"+rowNumber+"行的数据中，结构层次为空，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(ljwclcfVo.getLcz())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中02路基弯沉(落锤法)实测数据.xlsx文件，第"+rowNumber+"行的数据中，落锤重值有误，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(ljwclcfVo.getYqmc())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中02路基弯沉(落锤法)实测数据.xlsx文件，第"+rowNumber+"行的数据中，仪器名称为空，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(ljwclcfVo.getCjzh())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中02路基弯沉(落锤法)实测数据.xlsx文件，第"+rowNumber+"行的数据中，抽检桩号为空，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(ljwclcfVo.getCd())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中02路基弯沉(落锤法)实测数据.xlsx文件，第"+rowNumber+"行的数据中，车道为空，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(ljwclcfVo.getScwcz())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中02路基弯沉(落锤法)实测数据.xlsx文件，第"+rowNumber+"行的数据中，实测弯沉值有误，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(ljwclcfVo.getLbwd())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中02路基弯沉(落锤法)实测数据.xlsx文件，第"+rowNumber+"行的数据中，路表温度值有误，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(ljwclcfVo.getXh()) || StringUtils.isEmpty(ljwclcfVo.getXh())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中02路基弯沉(落锤法)实测数据.xlsx文件，第"+rowNumber+"行的数据中，序号值有误，请修改后重新上传");
                                        }
                                        ...
                                        ljwclcf.setUserid(commonInfoVo.getUserid());
            ...
            throw new JjgysException(20001,"您上传的"+htd+"合同段中02路基弯沉(落锤法)实测数据.xlsx文件，解析excel出错，请传入正确格式的excel");
        }catch (NullPointerException e) {
            throw new JjgysException(20001,"您上传的"+htd+"合同段中02路基弯沉(落锤法)实测数据.xlsx文件，请检查数据的正确性或删除文件最后的空数据，然后重新上传");
        }catch (ExcelAnalysisException e) {
            throw new JjgysException(20001,"您上传的"+htd+"合同段中02路基弯沉(落锤法)实测数据.xlsx文件，请将检测日期修改为2021/01/01的格式，然后重新上传");
        }
