...
public class JjgFbgcLjgcLjwcServiceImpl extends ServiceImpl<JjgFbgcLjgcLjwcMapper, JjgFbgcLjgcLjwc> implements JjgFbgcLjgcLjwcService {
    ...
    public boolean generateJdb(CommonInfoVo commonInfoVo) throws IOException {
        ...
        if (data == null || data.size()==0){
            ...
            try {
                ...
                InputStream inputStream = getClass().getClassLoader().getResourceAsStream("static/路基弯沉.xlsx");
                Files.copy(inputStream, f.toPath(), StandardCopyOption.REPLACE_EXISTING);
                FileInputStream out = new FileInputStream(f);
                wb = new XSSFWorkbook(out);
                int num = gettableNum(proname, htd, fbgc,idlist);
                createTable(num,wb);
    ...
    public boolean DBtoExcel(List<JjgFbgcLjgcLjwc> data,XSSFWorkbook wb) throws IOException {
        Collections.sort(data, new Comparator<JjgFbgcLjgcLjwc>() {
            @Override
            public int compare(JjgFbgcLjgcLjwc o1, JjgFbgcLjgcLjwc o2) {
                // 名字相同时按照 qdzh 排序
                String zh = o1.getCjzh();
                String zh1 = o2.getCjzh();
     ...
     * @return
     */
    public int gettableNum(String proname, String htd, String fbgc, List<Long> idlist){
        QueryWrapper<JjgFbgcLjgcLjwc> queryWrapper = new QueryWrapper();
        queryWrapper.select("DISTINCT xh").like("proname",proname).like("htd",htd).like("fbgc",fbgc).in("userid",idlist);
        ...
        for(int i = 0; i < xuhaoList.size(); i++){
            ...
            queryWrapperxh.like("proname",proname).like("htd",htd).like("fbgc",fbgc).like("xh", xuhaoList.get(i)).in("userid",idlist);
    ...
    public void exportljwc(HttpServletResponse response) {
        ...
        try {
            ...
            File t = new File(courseFile+File.separator+"02路基弯沉(贝克曼梁法)实测数据.xlsx");
            if (t.exists()){
                t.delete();
            }
        }
    ...
    public void importljwc(MultipartFile file, CommonInfoVo commonInfoVo) {
        ...
        try {
                                ...
                                public void handle(List<JjgFbgcLjgcLjwcVo> dataList) {
                                    ...
                                    for(JjgFbgcLjgcLjwcVo ljwcVo: dataList)
                                    {
                                        if (StringUtils.isEmpty(ljwcVo.getZh())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中02路基弯沉(贝克曼梁法)实测数据.xlsx文件，第"+rowNumber+"行的数据中，桩号为空，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(ljwcVo.getYswcz()) || StringUtils.isEmpty(ljwcVo.getYswcz())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中02路基弯沉(贝克曼梁法)实测数据.xlsx文件，第"+rowNumber+"行的数据中，验收弯沉值有误，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(ljwcVo.getMbkkzb())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中02路基弯沉(贝克曼梁法)实测数据.xlsx文件，第"+rowNumber+"行的数据中，目标可靠指标值有误，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(ljwcVo.getSdyxxs())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中02路基弯沉(贝克曼梁法)实测数据.xlsx文件，第"+rowNumber+"行的数据中，温度影响系数值有误，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(ljwcVo.getJjyxxs())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中02路基弯沉(贝克曼梁法)实测数据.xlsx文件，第"+rowNumber+"行的数据中，季节影响系数值有误，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(ljwcVo.getJglx())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中02路基弯沉(贝克曼梁法)实测数据.xlsx文件，第"+rowNumber+"行的数据中，结构类型为空，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(ljwcVo.getJgcc())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中02路基弯沉(贝克曼梁法)实测数据.xlsx文件，第"+rowNumber+"行的数据中，结构层次为空，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(ljwcVo.getHzz())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中02路基弯沉(贝克曼梁法)实测数据.xlsx文件，第"+rowNumber+"行的数据中，后轴重值有误，请修改后重新上传");
                                        }
                                        if ( StringUtils.isEmpty(ljwcVo.getLtqy())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中02路基弯沉(贝克曼梁法)实测数据.xlsx文件，第"+rowNumber+"行的数据中，轮胎气压值有误，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(ljwcVo.getCjzh())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中02路基弯沉(贝克曼梁法)实测数据.xlsx文件，第"+rowNumber+"行的数据中，抽检桩号为空，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(ljwcVo.getCd())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中02路基弯沉(贝克曼梁法)实测数据.xlsx文件，第"+rowNumber+"行的数据中，车道 l为空，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(ljwcVo.getZz())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中02路基弯沉(贝克曼梁法)实测数据.xlsx文件，第"+rowNumber+"行的数据中，左值有误，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(ljwcVo.getYz())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中02路基弯沉(贝克曼梁法)实测数据.xlsx文件，第"+rowNumber+"行的数据中，右值有误，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(ljwcVo.getLbwd())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中02路基弯沉(贝克曼梁法)实测数据.xlsx文件，第"+rowNumber+"行的数据中，路表温度值有误，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(ljwcVo.getXh()) || StringUtils.isEmpty(ljwcVo.getXh())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中02路基弯沉(贝克曼梁法)实测数据.xlsx文件，第"+rowNumber+"行的数据中，序号值有误，请修改后重新上传");
                                        }
                                        JjgFbgcLjgcLjwc ljwc = new JjgFbgcLjgcLjwc();
                                        BeanUtils.copyProperties(ljwcVo,ljwc);
                                        ljwc.setCreatetime(new Date());
                                        ljwc.setUserid(commonInfoVo.getUserid());
            ...
            throw new JjgysException(20001,"您上传的"+htd+"合同段中02路基弯沉(贝克曼梁法)实测数据.xlsx文件，解析excel出错，请传入正确格式的excel");
        }catch (NullPointerException e) {
            throw new JjgysException(20001,"您上传的"+htd+"合同段中02路基弯沉(贝克曼梁法)实测数据.xlsx文件，请检查数据的正确性或删除文件最后的空数据，然后重新上传");
        }catch (ExcelAnalysisException e) {
            throw new JjgysException(20001,"您上传的"+htd+"合同段中02路基弯沉(贝克曼梁法)实测数据.xlsx文件，请将检测日期修改为2021/01/01的格式，然后重新上传");
        }

    }

    @Override
    public int createMoreRecords(List<JjgFbgcLjgcLjwc> data, String userID) {
        return ReceiveUtils.createMore(data, jjgFbgcLjgcLjwcMapper, userID);
    }
}
