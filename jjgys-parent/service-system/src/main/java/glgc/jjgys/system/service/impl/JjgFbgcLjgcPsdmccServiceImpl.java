...
public class JjgFbgcLjgcPsdmccServiceImpl extends ServiceImpl<JjgFbgcLjgcPsdmccMapper, JjgFbgcLjgcPsdmcc> implements JjgFbgcLjgcPsdmccService {
    ...
    public boolean generateJdb(CommonInfoVo commonInfoVo) throws IOException, ParseException {
        ...
        if (data == null || data.size()==0){
            ...
            try {
                ...
                InputStream inputStream = getClass().getClassLoader().getResourceAsStream("static/排水断面尺寸.xlsx");
    ...
    public boolean DBtoExcel(List<JjgFbgcLjgcPsdmcc> data,String proname,String htd,String fbgc,String sheetname) throws IOException, ParseException {
        ...
        for(JjgFbgcLjgcPsdmcc row:data){
            if(zh.equals(row.getZh())){
                ...
                //sheet.getRow(start).getCell(0).setCellValue(row.getZh());// 这里zh是旧的，应该设置为旧的值
    ...
    public void exportpsdmcc(HttpServletResponse response) {
        ...
        try {
            ...
            File t = new File(courseFile+File.separator+"04排水断面尺寸实测数据.xlsx");
            if (t.exists()){
                t.delete();
            }
        }
    ...
    public void importpsdmcc(MultipartFile file,CommonInfoVo commonInfoVo) {
        ...
        try {
                                ...
                                public void handle(List<JjgFbgcLjgcPsdmccVo> dataList) {
                                    ...
                                    for(JjgFbgcLjgcPsdmccVo psdmccVo: dataList)
                                    {
                                        if (StringUtils.isEmpty(psdmccVo.getZh())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中04排水断面尺寸实测数据.xlsx文件，第"+rowNumber+"行的数据中，桩号为空，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(psdmccVo.getBw())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中04排水断面尺寸实测数据.xlsx文件，第"+rowNumber+"行的数据中，部位值为空，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(psdmccVo.getLb())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中04排水断面尺寸实测数据.xlsx文件，第"+rowNumber+"行的数据中，类别为空，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(psdmccVo.getSjz()) || StringUtils.isEmpty(psdmccVo.getSjz())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中04排水断面尺寸实测数据.xlsx文件，第"+rowNumber+"行的数据中，设计值有误，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(psdmccVo.getScz()) || StringUtils.isEmpty(psdmccVo.getScz())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中04排水断面尺寸实测数据.xlsx文件，第"+rowNumber+"行的数据中，实测值有误，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(psdmccVo.getYxwcz()) || StringUtils.isEmpty(psdmccVo.getYxwcz())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中04排水断面尺寸实测数据.xlsx文件，第"+rowNumber+"行的数据中，允许误差+值有误，请修改后重新上传");
                                        }
                                        ...
                                        jjgFbgcLjgcPsdmcc.setUserid(commonInfoVo.getUserid());
            ...
            throw new JjgysException(20001,"您上传的"+htd+"合同段中04排水断面尺寸实测数据.xlsx文件，解析excel出错，请传入正确格式的excel");
        }catch (NullPointerException e) {
            throw new JjgysException(20001,"您上传的"+htd+"合同段中04排水断面尺寸实测数据.xlsx文件，请检查数据的正确性或删除文件最后的空数据，然后重新上传");
        }catch (ExcelAnalysisException e) {
            throw new JjgysException(20001,"您上传的"+htd+"合同段中04排水断面尺寸实测数据.xlsx文件，请将检测日期修改为2021/01/01的格式，然后重新上传");
        }
    ...
    @Override
    public int createMoreRecords(List<JjgFbgcLjgcPsdmcc> data, String userID) {
        return ReceiveUtils.createMore(data, jjgFbgcLjgcPsdmccMapper, userID);
    }
}
