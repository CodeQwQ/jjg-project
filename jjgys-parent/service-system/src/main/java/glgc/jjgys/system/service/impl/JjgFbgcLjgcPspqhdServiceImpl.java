...
public class JjgFbgcLjgcPspqhdServiceImpl extends ServiceImpl<JjgFbgcLjgcPspqhdMapper, JjgFbgcLjgcPspqhd> implements JjgFbgcLjgcPspqhdService {
    ...
    public boolean generateJdb(CommonInfoVo commonInfoVo) throws IOException, ParseException {
        ...
        if (data == null || data.size()==0){
            ...
            try {
                ...
                InputStream inputStream = getClass().getClassLoader().getResourceAsStream("static/排水铺砌厚度.xlsx");
    ...
    private void calculateSheet(XSSFSheet sheet) {
        ...
        for (int i = sheet.getFirstRowNum(); i <= sheet.getPhysicalNumberOfRows(); i++) {
            ...
            if(flag && !"".equals(row.getCell(4).toString())){
                System.out.println(row.getRowNum()+"wsww");
    ...
    public void exportpspqhd(HttpServletResponse response) {
        ...
        try {
            ...
            File t = new File(courseFile+File.separator+"05排水铺砌厚度实测数据.xlsx");
            if (t.exists()){
                t.delete();
            }
        }
    ...
    public void importpspqhd(MultipartFile file,CommonInfoVo commonInfoVo) {
        ...
        try {
                                ...
                                public void handle(List<JjgFbgcLjgcPspqhdVo> dataList) {
                                    ...
                                    for(JjgFbgcLjgcPspqhdVo pspqhdVo: dataList)
                                    {
                                        if (StringUtils.isEmpty(pspqhdVo.getZh())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中05排水铺砌厚度实测数据.xlsx文件，第"+rowNumber+"行的数据中，桩号为空，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(pspqhdVo.getBw())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中05排水铺砌厚度实测数据.xlsx文件，第"+rowNumber+"行的数据中，部位值为空，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(pspqhdVo.getLb())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中05排水铺砌厚度实测数据.xlsx文件，第"+rowNumber+"行的数据中，类别为空，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(pspqhdVo.getSjz()) || StringUtils.isEmpty(pspqhdVo.getSjz())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中05排水铺砌厚度实测数据.xlsx文件，第"+rowNumber+"行的数据中，设计值有误，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(pspqhdVo.getScz()) || StringUtils.isEmpty(pspqhdVo.getScz())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中05排水铺砌厚度实测数据.xlsx文件，第"+rowNumber+"行的数据中，实测值有误，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(pspqhdVo.getYxwc())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中05排水铺砌厚度实测数据.xlsx文件，第"+rowNumber+"行的数据中，允许误差为空，请修改后重新上传");
                                        }
                                        ...
                                        fbgcLjgcPspqhd.setUserid(commonInfoVo.getUserid());
            ...
            throw new JjgysException(20001,"您上传的"+htd+"合同段中05排水铺砌厚度实测数据.xlsx文件，解析excel出错，请传入正确格式的excel");
        }catch (NullPointerException e) {
            throw new JjgysException(20001,"您上传的"+htd+"合同段中05排水铺砌厚度实测数据.xlsx文件，请检查数据的正确性或删除文件最后的空数据，然后重新上传");
        }catch (ExcelAnalysisException e) {
            throw new JjgysException(20001,"您上传的"+htd+"合同段中05排水铺砌厚度实测数据.xlsx文件，请将检测日期修改为2021/01/01的格式，然后重新上传");
        }
    ...
    @Override
    public int createMoreRecords(List<JjgFbgcLjgcPspqhd> data, String userID) {
        return ReceiveUtils.createMore(data, jjgFbgcLjgcPspqhdMapper, userID);
    }
}
