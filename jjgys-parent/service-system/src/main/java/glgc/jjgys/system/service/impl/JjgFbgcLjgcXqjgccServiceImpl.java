...
public class JjgFbgcLjgcXqjgccServiceImpl extends ServiceImpl<JjgFbgcLjgcXqjgccMapper, JjgFbgcLjgcXqjgcc> implements JjgFbgcLjgcXqjgccService {
    ...
    public boolean generateJdb(CommonInfoVo commonInfoVo) throws Exception {
        ...
        if (data == null || data.size()==0){
            ...
            try {
                ...
                InputStream inputStream = getClass().getClassLoader().getResourceAsStream("static/小桥结构尺寸.xlsx");
    ...
    public void exportxqjgcc(HttpServletResponse response) {
        ...
        try {
            ...
            File t = new File(courseFile+File.separator+"07小桥结构尺寸实测数据.xlsx");
            if (t.exists()){
                t.delete();
            }
        }
    ...
    public void importxqjgcc(MultipartFile file,CommonInfoVo commonInfoVo) {
        ...
        try {
                                ...
                                public void handle(List<JjgFbgcLjgcXqjgccVo> dataList) {
                                    ...
                                    for(JjgFbgcLjgcXqjgccVo xqjgccVo: dataList)
                                    {
                                        ...
                                        if (StringUtils.isEmpty(xqjgccVo.getZh())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中07小桥结构尺寸实测数据.xlsx文件，第"+rowNumber+"行的数据中，桩号为空，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(xqjgccVo.getBw())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中07小桥结构尺寸实测数据.xlsx文件，第"+rowNumber+"行的数据中，部位值为空，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(xqjgccVo.getLb())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中07小桥结构尺寸实测数据.xlsx文件，第"+rowNumber+"行的数据中，类别为空，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(xqjgccVo.getSjz()) || StringUtils.isEmpty(xqjgccVo.getSjz())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中07小桥结构尺寸实测数据.xlsx文件，第"+rowNumber+"行的数据中，设计值有误，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(xqjgccVo.getScz()) || StringUtils.isEmpty(xqjgccVo.getScz())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中07小桥结构尺寸实测数据.xlsx文件，第"+rowNumber+"行的数据中，实测值有误，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(xqjgccVo.getYxwcz()) || StringUtils.isEmpty(xqjgccVo.getYxwcz())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中07小桥结构尺寸实测数据.xlsx文件，第"+rowNumber+"行的数据中，允许误差+值有误，请修改后重新上传");
                                        }
                                        ...
                                        fbgcLjgcXqjgcc.setUserid(commonInfoVo.getUserid());
            ...
            throw new JjgysException(20001,"您上传的"+htd+"合同段中07小桥结构尺寸实测数据.xlsx文件，解析excel出错，请传入正确格式的excel");
        }catch (NullPointerException e) {
            throw new JjgysException(20001,"您上传的"+htd+"合同段中07小桥结构尺寸实测数据.xlsx文件，请检查数据的正确性或删除文件最后的空数据，然后重新上传");
        }catch (ExcelAnalysisException e) {
            throw new JjgysException(20001,"您上传的"+htd+"合同段中07小桥结构尺寸实测数据.xlsx文件，请将检测日期修改为2021/01/01的格式，然后重新上传");
        }
    ...
    @Override
    public int createMoreRecords(List<JjgFbgcLjgcXqjgcc> data, String userID) {
        return ReceiveUtils.createMore(data, jjgFbgcLjgcXqjgccMapper, userID);
    }
