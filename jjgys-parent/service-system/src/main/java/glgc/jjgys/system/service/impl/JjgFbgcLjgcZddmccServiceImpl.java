...
public class JjgFbgcLjgcZddmccServiceImpl extends ServiceImpl<JjgFbgcLjgcZddmccMapper, JjgFbgcLjgcZddmcc> implements JjgFbgcLjgcZddmccService {
    ...
    public boolean generateJdb(CommonInfoVo commonInfoVo) throws IOException {
        ...
        if (data == null || data.size() == 0) {
            ...
            try {
                ...
                InputStream inputStream = getClass().getClassLoader().getResourceAsStream("static/支挡断面尺寸.xlsx");
    ...
    public void exportzddmcc(HttpServletResponse response) {
        ...
        try {
            ...
            File t = new File(courseFile+File.separator+"11支挡断面尺寸实测数据.xlsx");
            if (t.exists()){
                t.delete();
            }
        }
    ...
    public void importzddmcc(MultipartFile file,CommonInfoVo commonInfoVo) {
        ...
        try {
                                ...
                                public void handle(List<JjgFbgcLjgcZddmccVo> dataList) {
                                    ...
                                    for(JjgFbgcLjgcZddmccVo zddmccVo: dataList)
                                    {
                                        if (StringUtils.isEmpty(zddmccVo.getCjzh())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中11支挡断面尺寸实测数据.xlsx文件，第"+rowNumber+"行的数据中，抽检桩号为空，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(zddmccVo.getBw())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中11支挡断面尺寸实测数据.xlsx文件，第"+rowNumber+"行的数据中，部位值为空，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(zddmccVo.getLb())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中11支挡断面尺寸实测数据.xlsx文件，第"+rowNumber+"行的数据中，类别为空，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(zddmccVo.getSjz()) || StringUtils.isEmpty(zddmccVo.getSjz())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中11支挡断面尺寸实测数据.xlsx文件，第"+rowNumber+"行的数据中，设计值有误，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(zddmccVo.getScz()) || StringUtils.isEmpty(zddmccVo.getScz())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中11支挡断面尺寸实测数据.xlsx文件，第"+rowNumber+"行的数据中，实测值有误，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(zddmccVo.getYxwcz())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中11支挡断面尺寸实测数据.xlsx文件，第"+rowNumber+"行的数据中，允许误差+值有误，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(zddmccVo.getYxwcf())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中11支挡断面尺寸实测数据.xlsx文件，第"+rowNumber+"行的数据中，允许误差-值有误，请修改后重新上传");
                                        }
                                        if (zddmccVo.getYxwcf().contains("不小于") ||  zddmccVo.getYxwcz().contains("不小于")){
                                            if (!"不小于设计值".equals(zddmccVo.getYxwcf()) || !"不小于设计值".equals(zddmccVo.getYxwcz())){
                                                throw new JjgysException(20001, "您上传的"+htd+"合同段中11支挡断面尺寸实测数据.xlsx文件，第"+rowNumber+"行的数据中，允许误差值请确保是“不小于设计值”的字眼，请修改后重新上传");
                                            }
                                        ...
                                        fbgcLjgcZddmcc.setUserid(commonInfoVo.getUserid());
            ...
            throw new JjgysException(20001,"您上传的"+htd+"合同段中11支挡断面尺寸实测数据.xlsx文件，解析excel出错，请传入正确格式的excel");
        }catch (NullPointerException e) {
            throw new JjgysException(20001,"您上传的"+htd+"合同段中11支挡断面尺寸实测数据.xlsx文件，请检查数据的正确性或删除文件最后的空数据，然后重新上传");
        }catch (ExcelAnalysisException e) {
            throw new JjgysException(20001,"您上传的"+htd+"合同段中11支挡断面尺寸实测数据.xlsx文件，请将检测日期修改为2021/01/01的格式，然后重新上传");
        }
    ...
    @Override
    public int createMoreRecords(List<JjgFbgcLjgcZddmcc> data, String userID) {
        return ReceiveUtils.createMore(data, jjgFbgcLjgcZddmccMapper, userID);
    }
}
