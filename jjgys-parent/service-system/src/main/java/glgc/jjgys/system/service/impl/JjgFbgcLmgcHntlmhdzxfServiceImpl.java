...
//				sheet.getRow(index+i).getCell(9).setCellValue(Double.parseDouble(data.get(i)[12])<=60?-10:Double.parseDouble(data.get(i)[12])*-0.15);
                //候文强的计算方式
                sheet.getRow(index+i).getCell(9).setCellValue(data.get(i).getYxps());
            }
            return true;
        }
        else{
    ...
    public void exportHntlmhd(HttpServletResponse response) {
        ...
        try {
            ...
            File t = new File(courseFile+File.separator+"09混凝土路面厚度钻芯法实测数据.xlsx");
            if (t.exists()){
                t.delete();
            }
        }
    ...
    public void importHntlmhd(MultipartFile file, CommonInfoVo commonInfoVo) {
        ...
        try {
                                ...
                                public void handle(List<JjgFbgcLmgcHntlmhdzxfVo> dataList) {
                                    ...
                                    for(JjgFbgcLmgcHntlmhdzxfVo hntlmhdzxfVo: dataList)
                                    {
                                        if (StringUtils.isEmpty(hntlmhdzxfVo.getZh())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中09混凝土路面厚度钻芯法实测数据.xlsx文件，第"+rowNumber+"行的数据中，桩号为空，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(hntlmhdzxfVo.getBw())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中09混凝土路面厚度钻芯法实测数据.xlsx文件，第"+rowNumber+"行的数据中，部位值为空，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(hntlmhdzxfVo.getLx())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中09混凝土路面厚度钻芯法实测数据.xlsx文件，第"+rowNumber+"行的数据中，路桥隧匝道为空，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(hntlmhdzxfVo.getCz1()) || StringUtils.isEmpty(hntlmhdzxfVo.getCz1())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中09混凝土路面厚度钻芯法实测数据.xlsx文件，第"+rowNumber+"行的数据中，测值1有误，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(hntlmhdzxfVo.getCz2()) || StringUtils.isEmpty(hntlmhdzxfVo.getCz2())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中09混凝土路面厚度钻芯法实测数据.xlsx文件，第"+rowNumber+"行的数据中，测值2有误，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(hntlmhdzxfVo.getCz3()) || StringUtils.isEmpty(hntlmhdzxfVo.getCz3())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中09混凝土路面厚度钻芯法实测数据.xlsx文件，第"+rowNumber+"行的数据中，测值3有误，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(hntlmhdzxfVo.getCz4()) || StringUtils.isEmpty(hntlmhdzxfVo.getCz4())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中09混凝土路面厚度钻芯法实测数据.xlsx文件，第"+rowNumber+"行的数据中，测值4有误，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(hntlmhdzxfVo.getSjz()) || StringUtils.isEmpty(hntlmhdzxfVo.getSjz())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中09混凝土路面厚度钻芯法实测数据.xlsx文件，第"+rowNumber+"行的数据中，设计值有误，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(hntlmhdzxfVo.getYxps())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中09混凝土路面厚度钻芯法实测数据.xlsx文件，第"+rowNumber+"行的数据中，c值有误，请修改后重新上传");
                                        }
                                        ...
                                        hntlmhdzxf.setUserid(commonInfoVo.getUserid());
            ...
            throw new JjgysException(20001,"您上传的"+htd+"合同段中09混凝土路面厚度钻芯法实测数据.xlsx文件，解析excel出错，请传入正确格式的excel");
        }catch (NullPointerException e) {
            throw new JjgysException(20001,"您上传的"+htd+"合同段中09混凝土路面厚度钻芯法实测数据.xlsx文件，请检查数据的正确性或删除文件最后的空数据，然后重新上传");
        }catch (ExcelAnalysisException e) {
            throw new JjgysException(20001,"您上传的"+htd+"合同段中09混凝土路面厚度钻芯法实测数据.xlsx文件，请将检测日期修改为2021/01/01的格式，然后重新上传");
        }
    ...
    @Override
    public int createMoreRecords(List<JjgFbgcLmgcHntlmhdzxf> data, String userID) {
        return ReceiveUtils.createMore(data, jjgFbgcLmgcHntlmhdzxfMapper, userID);
    }
}
