...
public class JjgFbgcLmgcHntlmqdServiceImpl extends ServiceImpl<JjgFbgcLmgcHntlmqdMapper, JjgFbgcLmgcHntlmqd> implements JjgFbgcLmgcHntlmqdService {
    ...
    public boolean generateJdb(CommonInfoVo commonInfoVo) throws IOException, ParseException {
        ...
        if (data == null || data.size()==0){
            ...
            try {
                ...
                InputStream inputStream = getClass().getClassLoader().getResourceAsStream("static/混凝土路面强度.xlsx");
    ...
    private boolean DBtoExcel(List<JjgFbgcLmgcHntlmqd> data, XSSFWorkbook wb) throws ParseException {
        /*Collections.sort(data, new Comparator<JjgFbgcLmgcHntlmqd>() {
            @Override
            public int compare(JjgFbgcLmgcHntlmqd o1, JjgFbgcLmgcHntlmqd o2) {

                String cd = o1.getQywzmc()+o1.getZh();
                String cd1 = o2.getQywzmc()+o2.getZh();
        ...
        for(int i =0; i < data.size(); i++){
            ...
            sheet.getRow(index+i).getCell(1).setCellValue(data.get(i).getZh()+data.get(i).getQywzmc());
    ...
    public void exporthntlmqd(HttpServletResponse response) {
        ...
        try {
            ...
            File t = new File(courseFile+File.separator+"05混凝土路面强度实测数据.xlsx");
            if (t.exists()){
                t.delete();
            }
        }
    ...
    public void importhntlmqd(MultipartFile file, CommonInfoVo commonInfoVo) {
        ...
        try {
                                ...
                                public void handle(List<JjgFbgcLmgcHntlmqdVo> dataList) {
                                    ...
                                    for(JjgFbgcLmgcHntlmqdVo hntlmqdVo: dataList)
                                    {
                                        if (StringUtils.isEmpty(hntlmqdVo.getZh())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中05混凝土路面强度实测数据.xlsx文件，第"+rowNumber+"行的数据中，桩号为空，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(hntlmqdVo.getQywzmc())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中05混凝土路面强度实测数据.xlsx文件，第"+rowNumber+"行的数据中，取样位置名称为空，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(hntlmqdVo.getSypjzj())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中05混凝土路面强度实测数据.xlsx文件，第"+rowNumber+"行的数据中，类别为空，请修改后重新上传");
                                        }
                                        if ( StringUtils.isEmpty(hntlmqdVo.getSypjzj())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中05混凝土路面强度实测数据.xlsx文件，第"+rowNumber+"行的数据中，试样平均直径值有误，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(hntlmqdVo.getSypjhd())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中05混凝土路面强度实测数据.xlsx文件，第"+rowNumber+"行的数据中，试样平均厚度值有误，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(hntlmqdVo.getJxhz())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中05混凝土路面强度实测数据.xlsx文件，第"+rowNumber+"行的数据中，极限荷载值有误，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(hntlmqdVo.getLmqdgdz())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中05混凝土路面强度实测数据.xlsx文件，第"+rowNumber+"行的数据中，路面强度规定值有误，请修改后重新上传");
                                        }
                                        ...
                                        hntlmqd.setUserid(commonInfoVo.getUserid());
            ...
            throw new JjgysException(20001,"您上传的"+htd+"合同段中05混凝土路面强度实测数据.xlsx文件，解析excel出错，请传入正确格式的excel");
        }catch (NullPointerException e) {
            throw new JjgysException(20001,"您上传的"+htd+"合同段中05混凝土路面强度实测数据.xlsx文件，请检查数据的正确性或删除文件最后的空数据，然后重新上传");
        }catch (ExcelAnalysisException e) {
            throw new JjgysException(20001,"您上传的"+htd+"合同段中05混凝土路面强度实测数据.xlsx文件，请将检测日期修改为2021/01/01的格式，然后重新上传");
        }
    ...
    @Override
    public int createMoreRecords(List<JjgFbgcLmgcHntlmqd> data, String userID) {
        return ReceiveUtils.createMore(data, jjgFbgcLmgcHntlmqdMapper, userID);
    }
}
