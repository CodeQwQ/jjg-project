...
public class JjgFbgcLmgcLmhpServiceImpl extends ServiceImpl<JjgFbgcLmgcLmhpMapper, JjgFbgcLmgcLmhp> implements JjgFbgcLmgcLmhpService {
    ...
    public boolean generateJdb(CommonInfoVo commonInfoVo) throws Exception {
        ...
        //先查询需要生成几个鉴定表 根据lmlx
        List<Map<String,String>> lmlx = jjgFbgcLmgcLmhpMapper.selectlx(proname,htd,idlist); //[{lxlx=主线}, {lxlx=岳口枢纽立交}, {lxlx=延壶路小桥连接线}, {lxlx=延长互通式立交}]
        if (lmlx.size()>0){
            for (Map<String, String> map : lmlx) {
                String lxlx = map.get("lxlx");
                DBtoExcelData(proname,htd,fbgc,lxlx,idlist);
            }
            return true;
        }else {
            return false;
     ...
     * @param idlist
     */
    private void DBtoExcelData(String proname, String htd, String fbgc, String lxlx, List<Long> idlist) throws Exception {
        /**
         * 为什么合同段都是路基的？
         *
         * 服务区只会匝道，桥梁和隧道都有可能有
        ...
        if (lxlx.equals("主线")){
            ...
            try {
                ...
                if (jjgLqsSdzf.size()>0){
                    for (JjgLqsSd jjgLqsSd : jjgLqsSdzf) {
                        hpsdzfdata.addAll(jjgFbgcLmgcLmhpMapper.selectSdZfDatayh(proname,htd,fbgc,String.valueOf(jjgLqsSd.getZhq()),String.valueOf(jjgLqsSd.getZhz()),idlist));
                        allData.addAll(jjgFbgcLmgcLmhpMapper.selectSdZfDatayh(proname,htd,fbgc,String.valueOf(jjgLqsSd.getZhq()),String.valueOf(jjgLqsSd.getZhz()),idlist));
                    }
                }
                if (jjgLqsSdyf.size()>0){
                    for (JjgLqsSd jjgLqsSd : jjgLqsSdyf) {
                        hpsdyfdata.addAll(jjgFbgcLmgcLmhpMapper.selectSdYfDatayh(proname,htd,fbgc,String.valueOf(jjgLqsSd.getZhq()),String.valueOf(jjgLqsSd.getZhz()),idlist));
                        allData.addAll(jjgFbgcLmgcLmhpMapper.selectSdYfDatayh(proname,htd,fbgc,String.valueOf(jjgLqsSd.getZhq()),String.valueOf(jjgLqsSd.getZhz()),idlist));
                    }
                }
                if (jjgLqsQlzf.size()>0){
                    for (JjgLqsQl jjgLqsQl : jjgLqsQlzf) {
                        hpqlzfdata.addAll(jjgFbgcLmgcLmhpMapper.selectQlZfDatayh(proname,htd,fbgc,String.valueOf(jjgLqsQl.getZhq()),String.valueOf(jjgLqsQl.getZhz()),idlist));
                        allData.addAll(jjgFbgcLmgcLmhpMapper.selectQlZfDatayh(proname,htd,fbgc,String.valueOf(jjgLqsQl.getZhq()),String.valueOf(jjgLqsQl.getZhz()),idlist));
                    }
                }
                if (jjgLqsQlyf.size()>0){
                    for (JjgLqsQl jjgLqsQl : jjgLqsQlyf) {
                        hpqlyfdata.addAll(jjgFbgcLmgcLmhpMapper.selectQlYfDatayh(proname,htd,fbgc,String.valueOf(jjgLqsQl.getZhq()),String.valueOf(jjgLqsQl.getZhz()),idlist));
                        allData.addAll(jjgFbgcLmgcLmhpMapper.selectQlYfDatayh(proname,htd,fbgc,String.valueOf(jjgLqsQl.getZhq()),String.valueOf(jjgLqsQl.getZhz()),idlist));
                    }
                }
                hpalldata = jjgFbgcLmgcLmhpMapper.selectAllListyh(proname,htd,fbgc,idlist);
            ...
            wrapperhphnt.in("userid",idlist);
            wrapperhphnt.orderByAsc("zh");
            List<JjgFbgcLmgcLmhp> hplxlxhnt = jjgFbgcLmgcLmhpMapper.selectList(wrapperhphnt);
            if (hplxlxhnt.size()>0){
                separateHpAllData(hplxlxhnt,lxlx,idlist);
            }
     ...
     * @param idlist
     */
    private void separateHpAllData(List<JjgFbgcLmgcLmhp> data, String lxlx, List<Long> idlist) throws IOException, ParseException {
        ...
        //沥青和水泥可能同时都有
        List<MultipleLists> listslq = separateLqData(lqdata, lxlx, "沥青混凝土",idlist);
        List<MultipleLists> listssn = separateLqData(sndata, lxlx, "水泥混凝土",idlist);
        ...
        try {
            ...
            for (int i = 0; i < arr.length; i++) {
                if (shouldBeCalculate(wb.getSheet(arr[i]))) {
                    calculateThicknessSheet(wb.getSheet(arr[i]));
        ...
        if (data.size()>0 && data != null){
            Collections.sort(data, new Comparator<Map<String, String>>() {
                ...
                public int compare(Map<String, String> obj1, Map<String, String> obj2) {
                    ...
                    Double ccxm1 = Double.parseDouble(String.valueOf(obj1.get("zh")));
                    Double ccxm2 = Double.parseDouble(String.valueOf(obj2.get("zh")));
    ...
    private void writeData(XSSFWorkbook wb, List<Map<String, String>> data, String sheetname) throws ParseException {
        ...
        if (data.size()>0 && data != null){
            ...
            int tableNum = 0;
            String name = data.get(0).get("name");
            String htname = data.get(0).get("htmc");
            String zy = data.get(0).get("zy");
            fillTitleCellData1(sheet,tableNum,proname,htd,fbgc,data.get(0).get("lmlx"),name,testtime,htname);
            for(Map<String, String> row:data){
                //比较检测时间，拿到最新的检测时间
                ...
                if(index/29 == 1){
                    ...
                    tableNum ++;
                    fillTitleCellData1(sheet,tableNum,proname,htd,fbgc,row.get("lmlx"),row.get("name"),testtime,htname);
                    index = 0;
                }
                //填写中间下方的普通单元格
                fillCommonCellData(sheet, tableNum, index+6, row);
     ...
     * @param lq
     */
    private List<MultipleLists> separateLqData(List<JjgFbgcLmgcLmhp> data, String lxlx, String lq,List<Long> idlist) {
        ...
        if (dfdata.size()>0){
            MultipleLists lqdfData = handleLqdfData(dfdata, lxlx, "单幅", lq,idlist);
            lists.add(lqdfData);
        }
        if (zfdata.size()>0){
            MultipleLists lqzfData = handleLqdfData(zfdata, lxlx, "左幅", lq,idlist);
            lists.add(lqzfData);
        }
        if (yfdata.size()>0){
            MultipleLists lqyfData = handleLqdfData(yfdata, lxlx, "右幅", lq,idlist);
            lists.add(lqyfData);
        }
        return lists;
    }
     ...
     * @param lq
     */
    private MultipleLists handleLqdfData(List<JjgFbgcLmgcLmhp> data, String lxlx, String df, String lq,List<Long> idlist) {
        ...
        if (lxlx.contains("连接线")){
            ...
            if (ljxdata.size()>0){
                for (JjgLjx jjgLjx : ljxdata) {
                    ...
                    map.put("lxlx", lxlx);
                    map.put("lq", lq);
                    map.put("idlist", String.valueOf(idlist).replaceAll("\\[","").replaceAll("\\]",""));
                    ...
                    if (selecthpljx.size()>0){
                        ...
                        if (qlzfzhdata.size() > 0) {
                            ...
                            mapljx.put("lq", lq);
                            mapljx.put("idlist", String.valueOf(idlist));
                        ...
                        if (sdzfzhdata.size() > 0) {
                            ...
                            mapzdql.put("lq", lq);
                            mapzdql.put("idlist", String.valueOf(idlist));
                            //需要吧桥梁查询的数据加上
            ...
            if (sdzhdata.size() > 0) {
                for (JjgLqsHntlmzd hntlmzd : sdzhdata) {
                    ...
                    map.put("lxlx", lxlx);
                    map.put("lq", lq);
                    List<JjgFbgcLmgcLmhp> writeData = jjgFbgcLmgcLmhpMapper.selecthpzd(map,idlist);
                    /**
                    ...
                    if (writeData.size() > 0) {
                        ...
                        if (qlzfzhdata.size() > 0) {
                            ...
                            mapzd.put("lxlx", lxlx);
                            mapzd.put("lq", lq);
                            mapzd.put("idlist", String.valueOf(idlist));
                            //需要吧桥梁查询的数据加上
    ...
    private void fillCommonCellData(XSSFSheet sheet, int tableNum, int index, Map<String, String> row) {
        if ("匝道".equals(row.get("lxlx"))){
            if ("左幅".equals(row.get("wz"))){
                sheet.getRow(tableNum*35+index).getCell(0).setCellValue("Z"+getPile(row.get("zy"),String.valueOf(row.get("zh"))));
            }
            if ("右幅".equals(row.get("wz"))){
                sheet.getRow(tableNum*35+index).getCell(0).setCellValue("Y"+getPile(row.get("zy"),String.valueOf(row.get("zh"))));
            }
        }else {
    ...
    private void fillTitleCellData1(XSSFSheet sheet, int tableNum, String proname, String htd, String fbgc, String lmlx,String name,String testtime,String zdname) {
        sheet.getRow(tableNum*35+1).getCell(2).setCellValue(proname);
        sheet.getRow(tableNum*35+1).getCell(8).setCellValue(htd);
        sheet.getRow(tableNum*35+2).getCell(2).setCellValue(fbgc);
        String mname = "";
        ...
        if (!"".equals(zdname)){
            sheet.getRow(tableNum*35+2).getCell(8).setCellValue(mname+"("+zdname+")");
        }else {
            sheet.getRow(tableNum*35+2).getCell(8).setCellValue(mname);
        }
    ...
    public List<Map<String, String>> lookJdbjg(CommonInfoVo commonInfoVo) throws IOException {
        ...
        List<Map<String,String>> mclist = jjgFbgcLmgcLmhpMapper.selectlxno(proname,htd);
        if (mclist.size()>0){
        ...
        if (!f.exists()) {
            ...
            for (int j = 0; j < wb.getNumberOfSheets(); j++) {
                ...
                if (!wb.isSheetHidden(wb.getSheetIndex(wb.getSheetAt(j)))) {
                    ...
                    // 竖向遍历每个sheet内的表格,表格高貌似是固定的，高为35
                    int index = 0;
                    while(true){
                        // 跳出循环的条件该行为null或者拿到的单元格为空
                        XSSFRow row = slSheet.getRow(1+index*35);
                        if(row == null || row.getCell(2).toString().isEmpty()) break;
                        XSSFCell xmname = slSheet.getRow(1+index*35).getCell(2);//项目名
                        XSSFCell htdname = slSheet.getRow(1+index*35).getCell(8);//合同段名
                        XSSFCell fbgcname = slSheet.getRow(2+index*35).getCell(8);//分部工程名

                        if(proname.equals(xmname.toString()) && htd.equals(htdname.toString())) {
                            Map map = new HashMap();
                            // 处理逻辑：按照原来的代码，把“分部工程”字段也加上到map内
                            slSheet.getRow(6+index*35).getCell(14).setCellType(CellType.STRING);//总点数
                            slSheet.getRow(6+index*35).getCell(16).setCellType(CellType.STRING);//合格点数
                            slSheet.getRow(6+index*35).getCell(18).setCellType(CellType.STRING);//合格率
                            double zds = Double.valueOf(slSheet.getRow(6+index*35).getCell(14).getStringCellValue());
                            double hgds = Double.valueOf(slSheet.getRow(6+index*35).getCell(16).getStringCellValue());
                            double hgl = Double.valueOf(slSheet.getRow(6+index*35).getCell(18).getStringCellValue());
                            String zdsz1 = decf.format(zds);
                            String hgdsz1 = decf.format(hgds);
                            String hglz1 = df.format(hgl);
                            map.put("检测项目", mc);
                            slSheet.getRow(6+index*35).getCell(9).setCellType(CellType.STRING);
                            map.put("允许偏差", slSheet.getRow(6+index*35).getCell(9).getStringCellValue());
                            map.put("路面类型", wb.getSheetName(j));
                            map.put("分部工程", fbgcname.toString()); // 这个是后来加的，用这个来区分是不是桥面系
                            map.put("检测点数", zdsz1);
                            map.put("合格点数", hgdsz1);
                            map.put("合格率", hglz1);
                            jgmap.add(map);
...
//                    jgmap.add(map);
    ...
    public void exportlmhp(HttpServletResponse response) {
        ...
        try {
            ...
            File t = new File(courseFile+File.separator+"10路面横坡实测数据.xlsx");
            if (t.exists()){
                t.delete();
            }
        }
    ...
    public void importlmhp(MultipartFile file, CommonInfoVo commonInfoVo) {
        ...
        try {
                                ...
                                public void handle(List<JjgFbgcLmgcLmhpVo> dataList) {
                                    ...
                                    for(JjgFbgcLmgcLmhpVo lmgcLmhpVo: dataList)
                                    {
                                        if (StringUtils.isEmpty(lmgcLmhpVo.getZh())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中10路面横坡实测数据.xlsx文件，第"+rowNumber+"行的数据中，桩号为空，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(lmgcLmhpVo.getLmlx())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中10路面横坡实测数据.xlsx文件，第"+rowNumber+"行的数据中，路面类型为空，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(lmgcLmhpVo.getLxlx())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中10路面横坡实测数据.xlsx文件，第"+rowNumber+"行的数据中，路线类型为空，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(lmgcLmhpVo.getZy())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中10路面横坡实测数据.xlsx文件，第"+rowNumber+"行的数据中，Z/Y为空，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(lmgcLmhpVo.getWz())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中10路面横坡实测数据.xlsx文件，第"+rowNumber+"行的数据中，位置为空，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(lmgcLmhpVo.getQsds()) || StringUtils.isEmpty(lmgcLmhpVo.getQsds())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中10路面横坡实测数据.xlsx文件，第"+rowNumber+"行的数据中，前视读数值有误，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(lmgcLmhpVo.getHsds()) || StringUtils.isEmpty(lmgcLmhpVo.getHsds())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中10路面横坡实测数据.xlsx文件，第"+rowNumber+"行的数据中，后视读数值有误，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(lmgcLmhpVo.getLength()) || StringUtils.isEmpty(lmgcLmhpVo.getLength())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中10路面横坡实测数据.xlsx文件，第"+rowNumber+"行的数据中，长值有误，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(lmgcLmhpVo.getSjz())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中10路面横坡实测数据.xlsx文件，第"+rowNumber+"行的数据中，设计值有误，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(lmgcLmhpVo.getYxps())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中10路面横坡实测数据.xlsx文件，第"+rowNumber+"行的数据中允许偏差值有误，请修改后重新上传");
                                        }
                                        ...
                                        lmhp.setUserid(commonInfoVo.getUserid());
            ...
            throw new JjgysException(20001,"您上传的"+htd+"合同段中10路面横坡实测数据.xlsx文件，解析excel出错，请传入正确格式的excel");
        }catch (NullPointerException e) {
            throw new JjgysException(20001,"您上传的"+htd+"合同段中10路面横坡实测数据.xlsx文件，请检查数据的正确性或删除文件最后的空数据，然后重新上传");
        }catch (ExcelAnalysisException e) {
            throw new JjgysException(20001,"您上传的"+htd+"合同段中10路面横坡实测数据.xlsx文件，请将检测日期修改为2021/01/01的格式，然后重新上传");
        }
    ...
    public List<Map<String, String>> selectmc(String proname, String htd, String fbgc) {
        List<Map<String,String>> sdmclist = jjgFbgcLmgcLmhpMapper.selectlxno(proname,htd);
        return sdmclist;
    }
    ...
    @Override
    public int createMoreRecords(List<JjgFbgcLmgcLmhp> data, String userID) {
        return ReceiveUtils.createMore(data, jjgFbgcLmgcLmhpMapper, userID);
    }
}
