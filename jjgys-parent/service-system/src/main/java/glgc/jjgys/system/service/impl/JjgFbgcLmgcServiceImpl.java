...
public class JjgFbgcLmgcServiceImpl implements JjgFbgcLmgcService {
    ...
    public void download(HttpServletResponse response,String filename,String proname,String htd,String workpath){
        ...
        list.add(new File(workpath+"/12沥青路面压实度-(不分离上面层).xlsx"));
        list.add(new File(workpath+"/12沥青路面压实度-(分离上面层).xlsx"));
        ...
        List<Map<String,String>> lmlx = jjgFbgcLmgcLmhpMapper.selectlxno(proname,htd); //[{lxlx=主线}, {lxlx=岳口枢纽立交}, {lxlx=延壶路小桥连接线}, {lxlx=延长互通式立交}]
        if (lmlx.size()>0){
    ...
    public void getFile(String workpath, String proname, String htd) {
        ...
        List<File> list=new ArrayList<>();
        list.add(new File(workpath+File.separator+htd+File.separator+"12沥青路面压实度-(不分离上面层).xlsx"));
        list.add(new File(workpath+File.separator+htd+File.separator+"12沥青路面压实度-(分离上面层).xlsx"));
