...
public class JjgFbgcLmgcTlmxlbgcServiceImpl extends ServiceImpl<JjgFbgcLmgcTlmxlbgcMapper, JjgFbgcLmgcTlmxlbgc> implements JjgFbgcLmgcTlmxlbgcService {
    ...
    public void exportxlbgs(HttpServletResponse response) {
        ...
        try {
            ...
            File t = new File(courseFile+File.separator+"06砼路面相邻板高差实测数据.xlsx");
            if (t.exists()){
                t.delete();
            }
        }
    ...
    public void importxlbgs(MultipartFile file, CommonInfoVo commonInfoVo) {
        ...
        try {
                                ...
                                public void handle(List<JjgFbgcLmgcTlmxlbgcVo> dataList) {
                                    ...
                                    for(JjgFbgcLmgcTlmxlbgcVo tlmxlbgcVo: dataList)
                                    {
                                        if (StringUtils.isEmpty(tlmxlbgcVo.getZh())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中06砼路面相邻板高差实测数据.xlsx文件，第"+rowNumber+"行的数据中，桩号为空，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(tlmxlbgcVo.getQywzmc())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中06砼路面相邻板高差实测数据.xlsx文件，第"+rowNumber+"行的数据中，取样位置名称为空，请修改后重新上传");
                                        }

                                        if ( StringUtils.isEmpty(tlmxlbgcVo.getScz1())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中06砼路面相邻板高差实测数据.xlsx文件，第"+rowNumber+"行的数据中，实测值1有误，请修改后重新上传");
                                        }
                                        if ( StringUtils.isEmpty(tlmxlbgcVo.getScz2())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中06砼路面相邻板高差实测数据.xlsx文件，第"+rowNumber+"行的数据中，实测值2有误，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(tlmxlbgcVo.getScz3())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中06砼路面相邻板高差实测数据.xlsx文件，第"+rowNumber+"行的数据中，实测值3有误，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(tlmxlbgcVo.getScz4())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中06砼路面相邻板高差实测数据.xlsx文件，第"+rowNumber+"行的数据中，实测值4有误，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(tlmxlbgcVo.getBgcgdz())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中06砼路面相邻板高差实测数据.xlsx文件，第"+rowNumber+"行的数据中，板高差规定值有误，请修改后重新上传");
                                        }
            ...
            throw new JjgysException(20001,"您上传的"+htd+"合同段中06砼路面相邻板高差实测数据.xlsx文件，解析excel出错，请传入正确格式的excel");
        }catch (NullPointerException e) {
            throw new JjgysException(20001,"您上传的"+htd+"合同段中06砼路面相邻板高差实测数据.xlsx文件，请检查数据的正确性或删除文件最后的空数据，然后重新上传");
        }catch (ExcelAnalysisException e) {
            throw new JjgysException(20001,"您上传的"+htd+"合同段中06砼路面相邻板高差实测数据.xlsx文件，请将检测日期修改为2021/01/01的格式，然后重新上传");
        }
    ...
    @Override
    public int createMoreRecords(List<JjgFbgcLmgcTlmxlbgc> data, String userID) {
        return ReceiveUtils.createMore(data, jjgFbgcLmgcTlmxlbgcMapper, userID);
    }
}
