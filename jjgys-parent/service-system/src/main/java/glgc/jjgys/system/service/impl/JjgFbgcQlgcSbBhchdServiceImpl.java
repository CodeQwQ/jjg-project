...
public class JjgFbgcQlgcSbBhchdServiceImpl extends ServiceImpl<JjgFbgcQlgcSbBhchdMapper, JjgFbgcQlgcSbBhchd> implements JjgFbgcQlgcSbBhchdService {
    ...
    public boolean generateJdb(CommonInfoVo commonInfoVo) throws IOException, ParseException {
        ...
        List<Map<String,Object>> selectnum = jjgFbgcQlgcSbBhchdMapper.selectnum(proname, htd, fbgc,idlist);
    ...
    public List<Map<String, Object>> getqlname(String proname, String htd) {
        List<Map<String, Object>> list = jjgFbgcQlgcSbBhchdMapper.getqlName(proname,htd);
        return list;
    }
    ...
    public void export(HttpServletResponse response) {
        ...
        try {
            ...
            File t = new File(courseFile+File.separator+"07桥梁上部保护层厚度实测数据.xlsx");
            if (t.exists()){
                t.delete();
            }
        }
    ...
    public void importbhchd(MultipartFile file, CommonInfoVo commonInfoVo) {
        ...
        try {
                                ...
                                public void handle(List<JjgFbgcQlgcSbBhchdVo> dataList) {
                                    ...
                                    for(JjgFbgcQlgcSbBhchdVo sbJgccVo: dataList)
                                    {
                                        if (StringUtils.isEmpty(sbJgccVo.getQlmc())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中07桥梁上部保护层厚度实测数据.xlsx文件，第"+rowNumber+"行的数据中，桥梁名称为空，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(sbJgccVo.getGjbhjjcbw())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中07桥梁上部保护层厚度实测数据.xlsx文件，第"+rowNumber+"行的数据中，构建编号及检测部位为空，请修改后重新上传");
                                        }

                                        if (!StringUtils.isNumeric(sbJgccVo.getGjzj()) || StringUtils.isEmpty(sbJgccVo.getGjzj())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中07桥梁上部保护层厚度实测数据.xlsx文件，第"+rowNumber+"行的数据中，钢筋直径值有误，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(sbJgccVo.getSjz())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中07桥梁上部保护层厚度实测数据.xlsx文件，第"+rowNumber+"行的数据中，设计值为空，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(sbJgccVo.getScz1()) || StringUtils.isEmpty(sbJgccVo.getScz1())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中07桥梁上部保护层厚度实测数据.xlsx文件，第"+rowNumber+"行的数据中，实测值1有误，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(sbJgccVo.getScz2()) || StringUtils.isEmpty(sbJgccVo.getScz2())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中07桥梁上部保护层厚度实测数据.xlsx文件，第"+rowNumber+"行的数据中，实测值2有误，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(sbJgccVo.getScz3()) || StringUtils.isEmpty(sbJgccVo.getScz3())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中07桥梁上部保护层厚度实测数据.xlsx文件，第"+rowNumber+"行的数据中，实测值3有误，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(sbJgccVo.getScz4()) || StringUtils.isEmpty(sbJgccVo.getScz4())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中07桥梁上部保护层厚度实测数据.xlsx文件，第"+rowNumber+"行的数据中，实测值4有误，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(sbJgccVo.getScz5()) || StringUtils.isEmpty(sbJgccVo.getScz5())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中07桥梁上部保护层厚度实测数据.xlsx文件，第"+rowNumber+"行的数据中，实测值5有误，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(sbJgccVo.getScz6()) || StringUtils.isEmpty(sbJgccVo.getScz6())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中07桥梁上部保护层厚度实测数据.xlsx文件，第"+rowNumber+"行的数据中，实测值6有误，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(sbJgccVo.getXzz()) || StringUtils.isEmpty(sbJgccVo.getXzz())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中07桥梁上部保护层厚度实测数据.xlsx文件，第"+rowNumber+"行的数据中，修正值有误，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(sbJgccVo.getYxwcz()) || StringUtils.isEmpty(sbJgccVo.getYxwcz())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中07桥梁上部保护层厚度实测数据.xlsx文件，第"+rowNumber+"行的数据中，允许误差+值有误，请修改后重新上传");
                                        }
                                        ...
                                        fbgcQlgcSbBhchd.setUserid(commonInfoVo.getUserid());
            ...
            throw new JjgysException(20001,"您上传的"+htd+"合同段中07桥梁上部保护层厚度实测数据.xlsx文件，解析excel出错，请传入正确格式的excel");
        }catch (NullPointerException e) {
            throw new JjgysException(20001,"您上传的"+htd+"合同段中07桥梁上部保护层厚度实测数据.xlsx文件，请检查数据的正确性或删除文件最后的空数据，然后重新上传");
        }catch (ExcelAnalysisException e) {
            throw new JjgysException(20001,"您上传的"+htd+"合同段中07桥梁上部保护层厚度实测数据.xlsx文件，请将检测日期修改为2021/01/01的格式，然后重新上传");
        }

    }

    @Override
    public int createMoreRecords(List<JjgFbgcQlgcSbBhchd> data, String userID) {
        return ReceiveUtils.createMore(data, jjgFbgcQlgcSbBhchdMapper, userID);
    }
}
