...
public class JjgFbgcQlgcXbBhchdServiceImpl extends ServiceImpl<JjgFbgcQlgcXbBhchdMapper, JjgFbgcQlgcXbBhchd> implements JjgFbgcQlgcXbBhchdService {
    ...
    public boolean generateJdb(CommonInfoVo commonInfoVo) throws IOException, ParseException {
        ...
        List<Map<String,Object>> selectnum = jjgFbgcQlgcXbBhchdMapper.selectnum(proname, htd, fbgc,idlist);
        ...
        if (data == null || data.size()==0){
            ...
            try {
               /* File directory = new File("service-system/src/main/resources/static");
                ...
                InputStream inputStream = getClass().getClassLoader().getResourceAsStream("static/桥梁保护层厚度鉴定表.xlsx");
    ...
    public void export(HttpServletResponse response) {
        ...
        try {
            ...
            File t = new File(courseFile+File.separator+"03桥梁下部保护层厚度实测数据.xlsx");
            if (t.exists()){
                t.delete();
            }
        }
    ...
    public void importbhchd(MultipartFile file, CommonInfoVo commonInfoVo) {
        ...
        try {
                                ...
                                public void handle(List<JjgFbgcQlgcXbBhchdVo> dataList) {
                                    ...
                                    for(JjgFbgcQlgcXbBhchdVo xbJgccVo: dataList)
                                    {
                                        if (StringUtils.isEmpty(xbJgccVo.getQlmc())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中03桥梁下部保护层厚度实测数据.xlsx文件，第"+rowNumber+"行的数据中，桥梁名称为空，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(xbJgccVo.getGjbhjjcbw())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中03桥梁下部保护层厚度实测数据.xlsx文件，第"+rowNumber+"行的数据中，构建编号及检测部位为空，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(xbJgccVo.getGjzj()) || StringUtils.isEmpty(xbJgccVo.getGjzj())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中03桥梁下部保护层厚度实测数据.xlsx文件，第"+rowNumber+"行的数据中，钢筋直径值有误，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(xbJgccVo.getSjz())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中03桥梁下部保护层厚度实测数据.xlsx文件，第"+rowNumber+"行的数据中，设计值为空，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(xbJgccVo.getScz1()) || StringUtils.isEmpty(xbJgccVo.getScz1())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中03桥梁下部保护层厚度实测数据.xlsx文件，第"+rowNumber+"行的数据中，实测值1有误，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(xbJgccVo.getScz2()) || StringUtils.isEmpty(xbJgccVo.getScz2())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中03桥梁下部保护层厚度实测数据.xlsx文件，第"+rowNumber+"行的数据中，实测值2有误，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(xbJgccVo.getScz3()) || StringUtils.isEmpty(xbJgccVo.getScz3())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中03桥梁下部保护层厚度实测数据.xlsx文件，第"+rowNumber+"行的数据中，实测值3有误，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(xbJgccVo.getScz4()) || StringUtils.isEmpty(xbJgccVo.getScz4())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中03桥梁下部保护层厚度实测数据.xlsx文件，第"+rowNumber+"行的数据中，实测值4有误，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(xbJgccVo.getScz5()) || StringUtils.isEmpty(xbJgccVo.getScz5())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中03桥梁下部保护层厚度实测数据.xlsx文件，第"+rowNumber+"行的数据中，实测值5有误，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(xbJgccVo.getScz6()) || StringUtils.isEmpty(xbJgccVo.getScz6())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中03桥梁下部保护层厚度实测数据.xlsx文件，第"+rowNumber+"行的数据中，实测值6有误，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(xbJgccVo.getXzz()) || StringUtils.isEmpty(xbJgccVo.getXzz())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中03桥梁下部保护层厚度实测数据.xlsx文件，第"+rowNumber+"行的数据中，修正值有误，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(xbJgccVo.getYxwcz()) || StringUtils.isEmpty(xbJgccVo.getYxwcz())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中03桥梁下部保护层厚度实测数据.xlsx文件，第"+rowNumber+"行的数据中，允许误差+值有误，请修改后重新上传");
                                        }
                                        ...
                                        fbgcQlgcXbBhchd.setUserid(commonInfoVo.getUserid());
            ...
            throw new JjgysException(20001,"您上传的"+htd+"合同段中03桥梁下部保护层厚度实测数据.xlsx文件，解析excel出错，请传入正确格式的excel");
        }catch (NullPointerException e) {
            throw new JjgysException(20001,"您上传的"+htd+"合同段中03桥梁下部保护层厚度实测数据.xlsx文件，请检查数据的正确性或删除文件最后的空数据，然后重新上传");
        }catch (ExcelAnalysisException e) {
            throw new JjgysException(20001,"您上传的"+htd+"合同段中03桥梁下部保护层厚度实测数据.xlsx文件，请将检测日期修改为2021/01/01的格式，然后重新上传");
        }
    ...
    public List<Map<String, Object>> getqlname(String proname, String htd) {
        List<Map<String, Object>> list = jjgFbgcQlgcXbBhchdMapper.getqlName(proname,htd);
        return list;
    }
    ...
    @Override
    public int createMoreRecords(List<JjgFbgcQlgcXbBhchd> data, String userID) {
        return ReceiveUtils.createMore(data, jjgFbgcQlgcXbBhchdMapper, userID);
    }
}
