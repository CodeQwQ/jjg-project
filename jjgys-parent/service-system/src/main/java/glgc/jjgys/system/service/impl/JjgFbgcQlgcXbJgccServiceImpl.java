...
public class JjgFbgcQlgcXbJgccServiceImpl extends ServiceImpl<JjgFbgcQlgcXbJgccMapper, JjgFbgcQlgcXbJgcc> implements JjgFbgcQlgcXbJgccService {
    ...
    public boolean generateJdb(CommonInfoVo commonInfoVo) throws IOException, ParseException {
        ...
        List<Map<String,Object>> selectnum = jjgFbgcQlgcXbJgccMapper.selectnum(proname, htd, fbgc,idlist);
    ...
    public void exportqlxbjgcc(HttpServletResponse response) {
        ...
        try {
            ...
            File t = new File(courseFile+File.separator+"02桥梁下部结构尺寸实测数据.xlsx");
            if (t.exists()){
                t.delete();
            }
        }
    ...
    public void importqlxbjgcc(MultipartFile file, CommonInfoVo commonInfoVo) {
        ...
        try {
                                ...
                                public void handle(List<JjgFbgcQlgcXbJgccVo> dataList) {
                                    ...
                                    for(JjgFbgcQlgcXbJgccVo xbJgccVo: dataList)
                                    {
                                        if (StringUtils.isEmpty(xbJgccVo.getQlmc())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中02桥梁下部结构尺寸实测数据.xlsx文件，第"+rowNumber+"行的数据中，桥梁名称为空，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(xbJgccVo.getDth())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中02桥梁下部结构尺寸实测数据.xlsx文件，第"+rowNumber+"行的数据中，墩台号为空，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(xbJgccVo.getBw())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中02桥梁下部结构尺寸实测数据.xlsx文件，第"+rowNumber+"行的数据中，部位值为空，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(xbJgccVo.getLb())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中02桥梁下部结构尺寸实测数据.xlsx文件，第"+rowNumber+"行的数据中，类别为空，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(xbJgccVo.getSjz())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中02桥梁下部结构尺寸实测数据.xlsx文件，第"+rowNumber+"行的数据中，设计值有误，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(xbJgccVo.getScz()) || StringUtils.isEmpty(xbJgccVo.getScz())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中02桥梁下部结构尺寸实测数据.xlsx文件，第"+rowNumber+"行的数据中，实测值有误，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(xbJgccVo.getYxwcz()) || StringUtils.isEmpty(xbJgccVo.getYxwcz())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中02桥梁下部结构尺寸实测数据.xlsx文件，第"+rowNumber+"行的数据中，允许误差+值有误，请修改后重新上传");
                                        }
                                        ...
                                        xbJgcc.setUserid(commonInfoVo.getUserid());
            ...
            throw new JjgysException(20001,"您上传的"+htd+"合同段中02桥梁下部结构尺寸实测数据.xlsx文件，第解析excel出错，请传入正确格式的excel");
        }catch (NullPointerException e) {
            throw new JjgysException(20001,"您上传的"+htd+"合同段中02桥梁下部结构尺寸实测数据.xlsx文件，第请检查数据的正确性或删除文件最后的空数据，然后重新上传");
        }catch (ExcelAnalysisException e) {
            throw new JjgysException(20001,"您上传的"+htd+"合同段中02桥梁下部结构尺寸实测数据.xlsx文件，第请将检测日期修改为2021/01/01的格式，然后重新上传");
        }
    ...
    public List<Map<String, Object>> getqlname(String proname, String htd) {
        List<Map<String, Object>> list = jjgFbgcQlgcXbJgccMapper.getqlName(proname,htd);
        return list;
    }

    @Override
    public int createMoreRecords(List<JjgFbgcQlgcXbJgcc> data, String userID) {
        return ReceiveUtils.createMore(data, jjgFbgcQlgcXbJgccMapper, userID);
    }
}
