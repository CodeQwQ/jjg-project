...
public class JjgFbgcQlgcXbSzdServiceImpl extends ServiceImpl<JjgFbgcQlgcXbSzdMapper, JjgFbgcQlgcXbSzd> implements JjgFbgcQlgcXbSzdService {
    ...
    public boolean generateJdb(CommonInfoVo commonInfoVo) throws IOException, ParseException {
        ...
        if (data == null || data.size()==0){
            ...
            try {
                ...
                InputStream inputStream = getClass().getClassLoader().getResourceAsStream("static/桥梁下部竖直度17规范表格.xlsx");
    ...
    private boolean DBtoExcel(List<JjgFbgcQlgcXbSzd> data, XSSFWorkbook wb) throws ParseException {
        SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy.MM.dd");
            int totalTable = gettableNum(data);
            XSSFSheet sheet = wb.getSheet("竖直度");
            ...
            for(JjgFbgcQlgcXbSzd row:data){
                if(name.equals(row.getQlmc())){
                    ...
                    if(index%29 == 0 && tableNum <totalTable){
     ...
     * @return
     */
    private int gettableNum(List<JjgFbgcQlgcXbSzd> mapList) {
        Map<String, Integer> resultMap = new HashMap<>();
        for (JjgFbgcQlgcXbSzd map : mapList) {
    ...
    public void exportqlxbszd(HttpServletResponse response) {
        ...
        try {
            ...
            File t = new File(courseFile+File.separator+"04桥梁下部竖直度实测数据.xlsx");
            if (t.exists()){
                t.delete();
            }
        }
    ...
    public void importqlxbszd(MultipartFile file, CommonInfoVo commonInfoVo) {
        ...
        try {
                                ...
                                public void handle(List<JjgFbgcQlgcXbSzdVo> dataList) {
                                    ...
                                    for(JjgFbgcQlgcXbSzdVo xbSzdVo: dataList)
                                    {
                                        if (StringUtils.isEmpty(xbSzdVo.getQlmc())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中04桥梁下部竖直度实测数据.xlsx文件，第"+rowNumber+"行的数据中，桥梁名称为空，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(xbSzdVo.getDth())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中04桥梁下部竖直度实测数据.xlsx文件，第"+rowNumber+"行的数据中，墩台号为空，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(xbSzdVo.getDzgd()) || StringUtils.isEmpty(xbSzdVo.getDzgd())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中04桥梁下部竖直度实测数据.xlsx文件，第"+rowNumber+"行的数据中，墩柱高度值有误，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(xbSzdVo.getHxscz())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中04桥梁下部竖直度实测数据.xlsx文件，第"+rowNumber+"行的数据中，横向实测值有误，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(xbSzdVo.getHxscz())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中04桥梁下部竖直度实测数据.xlsx文件，第"+rowNumber+"行的数据中，纵向实测值有误，请修改后重新上传");
                                        }
                                        ...
                                        fbgcQlgcXbSzd.setUserid(commonInfoVo.getUserid());
            ...
            throw new JjgysException(20001,"您上传的"+htd+"合同段中04桥梁下部竖直度实测数据.xlsx文件，解析excel出错，请传入正确格式的excel");
        }catch (NullPointerException e) {
            throw new JjgysException(20001,"您上传的"+htd+"合同段中04桥梁下部竖直度实测数据.xlsx文件，请检查数据的正确性或删除文件最后的空数据，然后重新上传");
        }catch (ExcelAnalysisException e) {
            throw new JjgysException(20001,"您上传的"+htd+"合同段中04桥梁下部竖直度实测数据.xlsx文件，请将检测日期修改为2021/01/01的格式，然后重新上传");
        }
    ...
    public List<Map<String, Object>> getqlname(String proname, String htd) {
        List<Map<String, Object>> list = jjgFbgcQlgcXbSzdMapper.getqlName(proname,htd);
        return list;
    }
    ...
    @Override
    public int createMoreRecords(List<JjgFbgcQlgcXbSzd> data, String userID) {
        return ReceiveUtils.createMore(data,jjgFbgcQlgcXbSzdMapper, userID);
    }
}
