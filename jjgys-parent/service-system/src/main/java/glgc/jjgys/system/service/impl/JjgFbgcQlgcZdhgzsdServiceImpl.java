...
public class JjgFbgcQlgcZdhgzsdServiceImpl extends ServiceImpl<JjgFbgcQlgcZdhgzsdMapper, JjgFbgcQlgcZdhgzsd> implements JjgFbgcQlgcZdhgzsdService {
    ...
    public boolean generateJdb(CommonInfoVo commonInfoVo) throws IOException, ParseException {
        ...
        List<Map<String,Object>> lxlist = jjgFbgcQlgcZdhgzsdMapper.selectlxid(proname,htd,idlist);
        if (lxlist.size()>0){
            for (Map<String, Object> map : lxlist) {
                String zx = map.get("lxbs").toString();
                int num = jjgFbgcQlgcZdhgzsdMapper.selectcdnumid(proname,htd,zx,idlist);
                int cds = 0;
                if (num == 1){
                    cds = 2;
                }else {
    ...
    private void handlezxData(String proname, String htd, String zx, int cdsl, String sjz, List<Long> idlist) throws IOException, ParseException {
        log.info("准备数据......");
        List<Map<String, Object>> datazf = jjgFbgcQlgcZdhgzsdMapper.selectzfListyh(proname, htd,zx,idlist);
        List<Map<String, Object>> datayf =  jjgFbgcQlgcZdhgzsdMapper.selectyfListyh(proname, htd,zx,idlist);
    ...
    public void exportgzsd(HttpServletResponse response, String cdsl) throws IOException {
        ...
        try {
            ...
            File t = new File(courseFile+File.separator+"桥面构造深度实测数据.xlsx");
            if (t.exists()){
                t.delete();
            }
        }
    ...
    public void importgzsd(MultipartFile file, CommonInfoVo commonInfoVo) throws IOException {
        ...
        for (int i = 0; i < number; i++) {
            ...
            try {
                ...
                throw new JjgysException(20001,"您上传的"+htd+"合同段中桥面构造深度实测数据.xlsx文件，解析excel出错，请传入正确格式的excel");
            }catch (NullPointerException e) {
                throw new JjgysException(20001,"您上传的"+htd+"合同段中桥面构造深度实测数据.xlsx文件，请检查数据的正确性或删除文件最后的空数据，然后重新上传");
            }catch (ExcelAnalysisException e) {
                throw new JjgysException(20001,"您上传的"+htd+"合同段中桥面构造深度实测数据.xlsx文件，请将检测日期修改为2021/01/01的格式，然后重新上传");
            }
        }
