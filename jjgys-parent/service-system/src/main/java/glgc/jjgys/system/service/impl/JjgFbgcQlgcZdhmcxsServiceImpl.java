...
public class JjgFbgcQlgcZdhmcxsServiceImpl extends ServiceImpl<JjgFbgcQlgcZdhmcxsMapper, JjgFbgcQlgcZdhmcxs> implements JjgFbgcQlgcZdhmcxsService {
    ...
    public boolean generateJdb(CommonInfoVo commonInfoVo) throws IOException, ParseException {
        ...
        List<Map<String,Object>> lxlist = jjgFbgcQlgcZdhmcxsMapper.selectlxid(proname,htd,idlist);
        if (lxlist.size()>0){
            for (Map<String, Object> map : lxlist) {
                String zx = map.get("lxbs").toString();
                int num = jjgFbgcQlgcZdhmcxsMapper.selectcdnumid(proname,htd,zx,idlist);
                int cds = 0;
                if (num == 1){
                    cds = 2;
                }else {
    ...
    private void handlezxData(String proname, String htd, String zx, int cdsl, String sjz, List<Long> idlist) throws IOException, ParseException {
        List<Map<String, Object>> datazf = jjgFbgcQlgcZdhmcxsMapper.selectzfListyh(proname, htd,zx,idlist);
        List<Map<String, Object>> datayf = jjgFbgcQlgcZdhmcxsMapper.selectyfListyh(proname, htd,zx,idlist);
            ...
            for (int j = 0; j < wb.getNumberOfSheets(); j++) {
                if (!wb.isSheetHidden(wb.getSheetIndex(wb.getSheetAt(j)))) {
                    ...
                    if (proname.equals(xmname.toString()) && htd.equals(htdname.toString())) {
                        ...
                        map.put("总点数", zdsz1);
                        map.put("设计值", slSheet.getRow(37).getCell(sjzcd).getStringCellValue());
                        map.put("合格点数", hgdsz1);
    ...
    public void exportmcxs(HttpServletResponse response, String cdsl) throws IOException {
        ...
        try {
            ...
            File t = new File(courseFile+File.separator+"桥面摩擦系数实测数据.xlsx");
            if (t.exists()){
                t.delete();
            }
        }
    ...
    public void importmcxs(MultipartFile file, CommonInfoVo commonInfoVo) throws IOException {
        ...
        for (int i = 0; i < number; i++) {
            ...
            try {
                ...
                throw new JjgysException(20001,"您上传的"+htd+"合同段中桥面摩擦系数实测数据.xlsx文件，解析excel出错，请传入正确格式的excel");
            }catch (NullPointerException e) {
                throw new JjgysException(20001,"您上传的"+htd+"合同段中桥面摩擦系数实测数据.xlsx文件，请检查数据的正确性或删除文件最后的空数据，然后重新上传");
            }catch (ExcelAnalysisException e) {
                throw new JjgysException(20001,"您上传的"+htd+"合同段中桥面摩擦系数实测数据.xlsx文件，请将检测日期修改为2021/01/01的格式，然后重新上传");
            }
        }
    ...
    public List<Map<String, Object>> selectlx(String proname, String htd,String userid) {
        ...
        List<Map<String, Object>> selectlx = jjgFbgcQlgcZdhmcxsMapper.selectlxid(proname, htd,idlist);
        return selectlx;
    }
