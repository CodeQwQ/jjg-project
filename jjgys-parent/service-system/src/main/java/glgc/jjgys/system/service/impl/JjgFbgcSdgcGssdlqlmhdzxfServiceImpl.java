...
public class JjgFbgcSdgcGssdlqlmhdzxfServiceImpl extends ServiceImpl<JjgFbgcSdgcGssdlqlmhdzxfMapper, JjgFbgcSdgcGssdlqlmhdzxf> implements JjgFbgcSdgcGssdlqlmhdzxfService {
    ...
    public boolean generateJdb(CommonInfoVo commonInfoVo) throws IOException, ParseException {
        ...
        List<Map<String,Object>> sdmclist = jjgFbgcSdgcGssdlqlmhdzxfMapper.selectsdmcid(proname,htd,fbgc,idlist);
        if (sdmclist.size()>0){
     ...
     * @param idlist
     */
    private void DBtoExcelsd(String proname, String htd, String fbgc, String sdmc, List<Long> idlist) throws IOException, ParseException {
        ...
        if (data == null || data.size()==0 ){
            ...
            List<JjgFbgcSdgcGssdlqlmhdzxf> zxzfdata = jjgFbgcSdgcGssdlqlmhdzxfMapper.selectzxzf(proname,htd,fbgc,sdmc,idlist);
            List<JjgFbgcSdgcGssdlqlmhdzxf> zxyfdata = jjgFbgcSdgcGssdlqlmhdzxfMapper.selectzxyf(proname,htd,fbgc,sdmc,idlist);
            List<JjgFbgcSdgcGssdlqlmhdzxf> sdzfdata = jjgFbgcSdgcGssdlqlmhdzxfMapper.selectsdzf(proname,htd,fbgc,sdmc,idlist);
            List<JjgFbgcSdgcGssdlqlmhdzxf> sdyfdata = jjgFbgcSdgcGssdlqlmhdzxfMapper.selectsdyf(proname,htd,fbgc,sdmc,idlist);
            List<JjgFbgcSdgcGssdlqlmhdzxf> qlzfdata = jjgFbgcSdgcGssdlqlmhdzxfMapper.selectqlzf(proname,htd,fbgc,sdmc,idlist);
            List<JjgFbgcSdgcGssdlqlmhdzxf> qlyfdata = jjgFbgcSdgcGssdlqlmhdzxfMapper.selectqlyf(proname,htd,fbgc,sdmc,idlist);
            List<JjgFbgcSdgcGssdlqlmhdzxf> zddata = jjgFbgcSdgcGssdlqlmhdzxfMapper.selectzd(proname,htd,fbgc,sdmc,idlist);
    ...
    private void lqlmysd(XSSFWorkbook wb, List<JjgFbgcSdgcGssdlqlmhdzxf> data, String sheetname) throws ParseException {
        ...
        if(data.size() > 0){
            //if (level == 0){
                ...
                sheet.getRow(2).getCell(1).setCellValue(data.get(0).getFbgc()+"("+data.get(0).getSdmc()+")");
                if(type.contains("隧")){
    ...
    public List<Map<String, Object>> lookJdbjgPage(CommonInfoVo commonInfoVo) throws IOException {
        ...
        //Integer level = projectService.getlevel(proname);
        List<Map<String,Object>> sdmclist = jjgFbgcSdgcGssdlqlmhdzxfMapper.selectsdmc1id(proname,htd,idlist);
        if (sdmclist.size()>0){
            for (Map<String, Object> m : sdmclist) {
                for (String k : m.keySet()){
                    String sdmc = m.get(k).toString();
    ...
    public List<Map<String, Object>> getsdnum(String proname, String htd) {
        List<Map<String,Object>> list = jjgFbgcSdgcGssdlqlmhdzxfMapper.getsdnum(proname,htd);
        return list;
    }
    ...
    public void exportsdzxf(HttpServletResponse response) {
        ...
        try {
            ...
            File t = new File(courseFile+File.separator+"14高速隧道沥青路面厚度钻芯法实测数据.xlsx");
            if (t.exists()){
                t.delete();
            }
        }
    ...
    public void importsdzxf(MultipartFile file, CommonInfoVo commonInfoVo) {
        ...
        try {
                                ...
                                public void handle(List<JjgFbgcSdgcGssdlqlmhdzxfVo> dataList) {
                                    ...
                                    for(JjgFbgcSdgcGssdlqlmhdzxfVo zxfvo: dataList)
                                    {
                                        if (StringUtils.isEmpty(zxfvo.getZh())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中14高速隧道沥青路面厚度钻芯法实测数据.xlsx文件，第"+rowNumber+"行的数据中，桩号为空，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(zxfvo.getLqszd())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中14高速隧道沥青路面厚度钻芯法实测数据.xlsx文件，第"+rowNumber+"行的数据中，路桥隧匝道为空，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(zxfvo.getSdmc())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中14高速隧道沥青路面厚度钻芯法实测数据.xlsx文件，第"+rowNumber+"行的数据中，隧道名称为空，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(zxfvo.getSmcz1()) || StringUtils.isEmpty(zxfvo.getSmcz1())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中14高速隧道沥青路面厚度钻芯法实测数据.xlsx文件，第"+rowNumber+"行的数据中，上面层测值1有误，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(zxfvo.getSmcz2()) || StringUtils.isEmpty(zxfvo.getSmcz2())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中14高速隧道沥青路面厚度钻芯法实测数据.xlsx文件，第"+rowNumber+"行的数据中，上面层测值2有误，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(zxfvo.getSmcz3()) || StringUtils.isEmpty(zxfvo.getSmcz3())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中14高速隧道沥青路面厚度钻芯法实测数据.xlsx文件，第"+rowNumber+"行的数据中，上面层测值3有误，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(zxfvo.getSmcz4()) || StringUtils.isEmpty(zxfvo.getSmcz4())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中14高速隧道沥青路面厚度钻芯法实测数据.xlsx文件，第"+rowNumber+"行的数据中，上面层测值4有误，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(zxfvo.getSmcsjz()) || StringUtils.isEmpty(zxfvo.getSmcsjz())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中14高速隧道沥青路面厚度钻芯法实测数据.xlsx文件，第"+rowNumber+"行的数据中，上面层设计值有误，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(zxfvo.getZhdz1()) || StringUtils.isEmpty(zxfvo.getZhdz1())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中14高速隧道沥青路面厚度钻芯法实测数据.xlsx文件，第"+rowNumber+"行的数据中，总厚度测值1有误，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(zxfvo.getZhdz2()) || StringUtils.isEmpty(zxfvo.getZhdz2())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中14高速隧道沥青路面厚度钻芯法实测数据.xlsx文件，第"+rowNumber+"行的数据中，总厚度测值2有误，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(zxfvo.getZhdz3()) || StringUtils.isEmpty(zxfvo.getZhdz3())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中14高速隧道沥青路面厚度钻芯法实测数据.xlsx文件，第"+rowNumber+"行的数据中，总厚度测值3有误，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(zxfvo.getZhdz4()) || StringUtils.isEmpty(zxfvo.getZhdz4())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中14高速隧道沥青路面厚度钻芯法实测数据.xlsx文件，第"+rowNumber+"行的数据中，总厚度测值4有误，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(zxfvo.getZhdsjz()) || StringUtils.isEmpty(zxfvo.getZhdsjz())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中14高速隧道沥青路面厚度钻芯法实测数据.xlsx文件，第"+rowNumber+"行的数据中，总厚度设计值有误，请修改后重新上传");
                                        }
                                        ...
                                        sdlmzxf.setUserid(commonInfoVo.getUserid());
            ...
            throw new JjgysException(20001,"您上传的"+htd+"合同段中14高速隧道沥青路面厚度钻芯法实测数据.xlsx文件，解析excel出错，请传入正确格式的excel");
        }catch (NullPointerException e) {
            throw new JjgysException(20001,"您上传的"+htd+"合同段中14高速隧道沥青路面厚度钻芯法实测数据.xlsx文件，请检查数据的正确性或删除文件最后的空数据，然后重新上传");
        }catch (ExcelAnalysisException e) {
            throw new JjgysException(20001,"您上传的"+htd+"合同段中14高速隧道沥青路面厚度钻芯法实测数据.xlsx文件，请将检测日期修改为2021/01/01的格式，然后重新上传");
        }
    ...
    @Override
    public int createMoreRecords(List<JjgFbgcSdgcGssdlqlmhdzxf> data, String userID) {
        return ReceiveUtils.createMore(data, jjgFbgcSdgcGssdlqlmhdzxfMapper, userID);
    }
}
