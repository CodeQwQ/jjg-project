...
public class JjgFbgcSdgcHntlmqdServiceImpl extends ServiceImpl<JjgFbgcSdgcHntlmqdMapper, JjgFbgcSdgcHntlmqd> implements JjgFbgcSdgcHntlmqdService {
    ...
    public boolean generateJdb(CommonInfoVo commonInfoVo) throws IOException, ParseException {
        ...
        List<Map<String,Object>> sdmclist = jjgFbgcSdgcHntlmqdMapper.selectsdmcid(proname,htd,idlist);
        if (sdmclist.size()>0){
    ...
    private boolean DBtoExcel(List<JjgFbgcSdgcHntlmqd> data,String sdmc, XSSFWorkbook wb) throws ParseException {
        ...
        if(data.size() > 0){
            ...
            sheet.getRow(2).getCell(2).setCellValue("隧道工程("+sdmc+")");
    ...
    public List<Map<String, Object>> getsdnum(String proname, String htd) {
        List<Map<String,Object>> list = jjgFbgcSdgcHntlmqdMapper.getsdnum(proname,htd);
        return list;
    }
    ...
    public void exportsdhntlmqd(HttpServletResponse response) {
        ...
        try {
            ...
            File t = new File(courseFile+File.separator+"10隧道混凝土路面强度实测数据.xlsx");
            if (t.exists()){
                t.delete();
            }
        }
    ...
    public void importsdhntlmqd(MultipartFile file, CommonInfoVo commonInfoVo) {
        ...
        try {
                                ...
                                public void handle(List<JjgFbgcSdgcHntlmqdVo> dataList) {
                                    ...
                                    for(JjgFbgcSdgcHntlmqdVo hntlmqdVo: dataList)
                                    {
                                        if (StringUtils.isEmpty(hntlmqdVo.getSdmc())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中10隧道混凝土路面强度实测数据.xlsx文件，第"+rowNumber+"行的数据中，隧道名称为空，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(hntlmqdVo.getZh())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中10隧道混凝土路面强度实测数据.xlsx文件，第"+rowNumber+"行的数据中，桩号为空，请修改后重新上传");
                                        }

                                        if (StringUtils.isEmpty(hntlmqdVo.getSypjzj())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中10隧道混凝土路面强度实测数据.xlsx文件，第"+rowNumber+"行的数据中，试样平均直径值有误，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(hntlmqdVo.getSypjhd())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中10隧道混凝土路面强度实测数据.xlsx文件，第"+rowNumber+"行的数据中，试样平均厚度值有误，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(hntlmqdVo.getJxhz())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中10隧道混凝土路面强度实测数据.xlsx文件，第"+rowNumber+"行的数据中，极限荷载值有误，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(hntlmqdVo.getLmqdgdz()) || StringUtils.isEmpty(hntlmqdVo.getLmqdgdz())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中10隧道混凝土路面强度实测数据.xlsx文件，第"+rowNumber+"行的数据中，路面强度规定值有误，请修改后重新上传");
                                        }
            ...
            throw new JjgysException(20001,"您上传的"+htd+"合同段中10隧道混凝土路面强度实测数据.xlsx文件，解析excel出错，请传入正确格式的excel");
        }catch (NullPointerException e) {
            throw new JjgysException(20001,"您上传的"+htd+"合同段中10隧道混凝土路面强度实测数据.xlsx文件，请检查数据的正确性或删除文件最后的空数据，然后重新上传");
        }catch (ExcelAnalysisException e) {
            throw new JjgysException(20001,"您上传的"+htd+"合同段中10隧道混凝土路面强度实测数据.xlsx文件，请将检测日期修改为2021/01/01的格式，然后重新上传");
        }
    ...
    @Override
    public int createMoreRecords(List<JjgFbgcSdgcHntlmqd> data, String userID) {
        return ReceiveUtils.createMore(data, jjgFbgcSdgcHntlmqdMapper, userID);
    }
}
