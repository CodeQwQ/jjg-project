...
public class JjgFbgcSdgcSdhntlmhdzxfServiceImpl extends ServiceImpl<JjgFbgcSdgcSdhntlmhdzxfMapper, JjgFbgcSdgcSdhntlmhdzxf> implements JjgFbgcSdgcSdhntlmhdzxfService {
    ...
    public boolean generateJdb(CommonInfoVo commonInfoVo) throws IOException, ParseException {
        ...
        List<Map<String,Object>> sdmclist = jjgFbgcSdgcSdhntlmhdzxfMapper.selectsdmcid(proname,htd,idlist);
        if (sdmclist.size()>0){
    ...
    private void DBtoExcelsd(String proname, String htd, String fbgc, String sdmc, List<Long> idlist) throws IOException, ParseException {
        ...
        if (data == null || data.size()==0 ){
            ...
            try {
                ...
                InputStream inputStream = getClass().getClassLoader().getResourceAsStream("static/混凝土路面厚度-钻芯法.xlsx");
...
//						? -5 : (sheet.getRow(3).getCell(2).getNumericCellValue())*-0.08));
    ...
    public List<Map<String, Object>> getsdnum(String proname, String htd) {
        List<Map<String,Object>> list = jjgFbgcSdgcSdhntlmhdzxfMapper.getsdnum(proname,htd);
        return list;
    }
    ...
    public void exportsdhntzxf(HttpServletResponse response) {
        ...
        try {
            ...
            File t = new File(courseFile+File.separator+"14隧道混凝土路面厚度钻芯法实测数据.xlsx");
            if (t.exists()){
                t.delete();
            }
        }
    ...
    public void importsdhntzxf(MultipartFile file, CommonInfoVo commonInfoVo) {
        ...
        try {
                                ...
                                public void handle(List<JjgFbgcSdgcSdhntlmhdzxfVo> dataList) {
                                    ...
                                    for(JjgFbgcSdgcSdhntlmhdzxfVo zxfvo: dataList)
                                    {
                                        if (StringUtils.isEmpty(zxfvo.getZh())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中14隧道混凝土路面厚度钻芯法实测数据.xlsx文件，第"+rowNumber+"行的数据中，桩号为空，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(zxfvo.getBw())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中14隧道混凝土路面厚度钻芯法实测数据.xlsx文件，第"+rowNumber+"行的数据中，部位值为空，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(zxfvo.getLqszd())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中14隧道混凝土路面厚度钻芯法实测数据.xlsx文件，第"+rowNumber+"行的数据中，路桥隧匝道为空，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(zxfvo.getSdmc())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中14隧道混凝土路面厚度钻芯法实测数据.xlsx文件，第"+rowNumber+"行的数据中，隧道名称为空，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(zxfvo.getCz1()) || StringUtils.isEmpty(zxfvo.getCz1())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中14隧道混凝土路面厚度钻芯法实测数据.xlsx文件，第"+rowNumber+"行的数据中，测值1有误，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(zxfvo.getCz2()) || StringUtils.isEmpty(zxfvo.getCz2())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中14隧道混凝土路面厚度钻芯法实测数据.xlsx文件，第"+rowNumber+"行的数据中，测值2有误，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(zxfvo.getCz3()) || StringUtils.isEmpty(zxfvo.getCz3())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中14隧道混凝土路面厚度钻芯法实测数据.xlsx文件，第"+rowNumber+"行的数据中，测值3有误，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(zxfvo.getCz4()) || StringUtils.isEmpty(zxfvo.getCz4())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中14隧道混凝土路面厚度钻芯法实测数据.xlsx文件，第"+rowNumber+"行的数据中，测值4有误，请修改后重新上传");
                                        }

                                        if (!StringUtils.isNumeric(zxfvo.getSjz()) || StringUtils.isEmpty(zxfvo.getSjz())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中14隧道混凝土路面厚度钻芯法实测数据.xlsx文件，第"+rowNumber+"行的数据中，设计值有误，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(zxfvo.getYxps()) || StringUtils.isEmpty(zxfvo.getYxps())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中14隧道混凝土路面厚度钻芯法实测数据.xlsx文件，第"+rowNumber+"行的数据中允许偏差值有误，请修改后重新上传");
                                        }
            ...
            throw new JjgysException(20001,"您上传的"+htd+"合同段中14隧道混凝土路面厚度钻芯法实测数据.xlsx文件，解析excel出错，请传入正确格式的excel");
        }catch (NullPointerException e) {
            throw new JjgysException(20001,"您上传的"+htd+"合同段中14隧道混凝土路面厚度钻芯法实测数据.xlsx文件，请检查数据的正确性或删除文件最后的空数据，然后重新上传");
        }catch (ExcelAnalysisException e) {
            throw new JjgysException(20001,"您上传的"+htd+"合同段中14隧道混凝土路面厚度钻芯法实测数据.xlsx文件，请将检测日期修改为2021/01/01的格式，然后重新上传");
        }
            ...
            for (int j = 0; j < wb.getNumberOfSheets(); j++) {
                if (!wb.isSheetHidden(wb.getSheetIndex(wb.getSheetAt(j)))) {
                    ...
                    if (proname.equals(xmname.toString()) && htd.equals(htdname.toString())) {
                        ...
                        String hgdsz1 = decf.format(hgds);
                        String hglz1 = df.format(hgl);
                        map.put("检测项目", StringUtils.substringBetween(value, "钻芯法-", "."));
                        map.put("检测点数", zdsz1);
    ...
    @Override
    public int createMoreRecords(List<JjgFbgcSdgcSdhntlmhdzxf> data, String userID) {
        return ReceiveUtils.createMore(data, jjgFbgcSdgcSdhntlmhdzxfMapper, userID);
    }
}
