...
public class JjgFbgcSdgcSdhpServiceImpl extends ServiceImpl<JjgFbgcSdgcSdhpMapper, JjgFbgcSdgcSdhp> implements JjgFbgcSdgcSdhpService {
    ...
    public boolean generateJdb(CommonInfoVo commonInfoVo) throws IOException, ParseException {
        ...
        List<Map<String,Object>> sdmclist = jjgFbgcSdgcSdhpMapper.selectsdmcid(proname,htd,fbgc,idlist);
        if (sdmclist.size()>0){
            for (Map<String, Object> m : sdmclist)
            {
                for (String k : m.keySet()){
                    String sdmc = m.get(k).toString();
                    List<Map<String,Object>> zhlist = jjgFbgcSdgcSdhpMapper.selectzh(proname,htd,fbgc,sdmc,idlist);
                    String zh = zhlist.get(0).get("zh").toString();
                    String substring = zh.substring(0, 1);
                    if (substring.equals("Z") || substring.equals("Y")){
                        DBtoExcelZYsd(proname,htd,fbgc,sdmc,idlist);
                    }else {
     ...
     * @throws ParseException
     */
    private void DBtoExcelZYsd(String proname, String htd, String fbgc, String sdmc, List<Long> idlist) throws IOException, ParseException {
        XSSFWorkbook wb = null;
        QueryWrapper<JjgFbgcSdgcSdhp> wrapper=new QueryWrapper<>();
        wrapper.like("proname",proname);
        wrapper.like("htd",htd);
    ...
    private void fillTitleCellData(XSSFSheet sheet, int tableNum, String proname, String htd, String fbgc,String lmlx,String sdmc) {
        ...
        sheet.getRow(tableNum*35+2).getCell(2).setCellValue(fbgc+"("+sdmc+")");
        sheet.getRow(tableNum*35+2).getCell(8).setCellValue("路面面层");
    ...
    public List<Map<String, Object>> lookJdbjgPage(CommonInfoVo commonInfoVo) throws IOException {
        ...
        for (int i=0;i<selectsdmc.size();i++){
            ...
            if (f.exists()){
                ...
                for (int j = 0; j < wb.getNumberOfSheets(); j++) {
                    String sheetnameleft = wb.getSheetName(j);
                    XSSFSheet sheetleft = wb.getSheet(sheetnameleft);
                    XSSFCell xmnameleft = sheetleft.getRow(1).getCell(2);//项目名
                    XSSFCell htdnameleft = sheetleft.getRow(1).getCell(8);//合同段名
                    sheetleft.getRow(0).getCell(14).setCellType(CellType.STRING);
                    sheetleft.getRow(0).getCell(16).setCellType(CellType.STRING);
                    sheetleft.getRow(0).getCell(18).setCellType(CellType.STRING);
            ...
            /*if (!f.exists()) {
                ...
                List<Map<String,Object>> zhlist = jjgFbgcSdgcSdhpMapper.selectzh1id(proname,htd,sdmc,idlist);
                String zh = zhlist.get(0).get("zh").toString();
                ...
                if (substring.equals("Z")){
                    ...
                    XSSFCell xmnameleft = sheetright.getRow(1).getCell(2);//项目名
                    XSSFCell htdnameleft = sheetright.getRow(1).getCell(8);//合同段名
    ...
    public List<Map<String, Object>> getsdnum(String proname, String htd) {
        List<Map<String,Object>> list = jjgFbgcSdgcSdhpMapper.getsdnum(proname,htd);
        return list;
    }
    ...
    public void export(HttpServletResponse response) {
        ...
        try {
            ...
            File t = new File(courseFile+File.separator+"15隧道横坡实测数据.xlsx");
            if (t.exists()){
                t.delete();
            }
        }
    ...
    public void importsdhp(MultipartFile file, CommonInfoVo commonInfoVo) {
        ...
        try {
                                ...
                                public void handle(List<JjgFbgcSdgcSdhpVo> dataList) {
                                    ...
                                    for(JjgFbgcSdgcSdhpVo fbgcSdgcSdhpVo: dataList)
                                    {
                                        if (StringUtils.isEmpty(fbgcSdgcSdhpVo.getZh())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中15隧道横坡实测数据.xlsx文件，第"+rowNumber+"行的数据中，桩号为空，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(fbgcSdgcSdhpVo.getSdmc())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中15隧道横坡实测数据.xlsx文件，第"+rowNumber+"行的数据中，隧道名称为空，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(fbgcSdgcSdhpVo.getLmlx())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中15隧道横坡实测数据.xlsx文件，第"+rowNumber+"行的数据中，路面类型为空，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(fbgcSdgcSdhpVo.getWz())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中15隧道横坡实测数据.xlsx文件，第"+rowNumber+"行的数据中，位置为空，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(fbgcSdgcSdhpVo.getQsds()) || StringUtils.isEmpty(fbgcSdgcSdhpVo.getQsds())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中15隧道横坡实测数据.xlsx文件，第"+rowNumber+"行的数据中，前视读数值有误，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(fbgcSdgcSdhpVo.getHsds()) || StringUtils.isEmpty(fbgcSdgcSdhpVo.getHsds())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中15隧道横坡实测数据.xlsx文件，第"+rowNumber+"行的数据中，后视读数值有误，请修改后重新上传");
                                        }
                                        if (!StringUtils.isNumeric(fbgcSdgcSdhpVo.getLength()) || StringUtils.isEmpty(fbgcSdgcSdhpVo.getLength())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中15隧道横坡实测数据.xlsx文件，第"+rowNumber+"行的数据中，长值有误，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(fbgcSdgcSdhpVo.getSjz())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中15隧道横坡实测数据.xlsx文件，第"+rowNumber+"行的数据中，设计值有误，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(fbgcSdgcSdhpVo.getYxps())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中15隧道横坡实测数据.xlsx文件，第"+rowNumber+"行的数据中允许偏差值有误，请修改后重新上传");
                                        }
                                        ...
                                        fbgcSdgcSdhp.setUserid(commonInfoVo.getUserid());
            ...
            throw new JjgysException(20001,"您上传的"+htd+"合同段中15隧道横坡实测数据.xlsx文件，解析excel出错，请传入正确格式的excel");
        }catch (NullPointerException e) {
            throw new JjgysException(20001,"您上传的"+htd+"合同段中15隧道横坡实测数据.xlsx文件，请检查数据的正确性或删除文件最后的空数据，然后重新上传");
        }catch (ExcelAnalysisException e) {
            throw new JjgysException(20001,"您上传的"+htd+"合同段中15隧道横坡实测数据.xlsx文件，请将检测日期修改为2021/01/01的格式，然后重新上传");
        }
    ...
    @Override
    public int createMoreRecords(List<JjgFbgcSdgcSdhp> data, String userID) {
        return ReceiveUtils.createMore(data, jjgFbgcSdgcSdhpMapper, userID);
    }
}
