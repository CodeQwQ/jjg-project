...
public class JjgFbgcSdgcTlmxlbgcServiceImpl extends ServiceImpl<JjgFbgcSdgcTlmxlbgcMapper, JjgFbgcSdgcTlmxlbgc> implements JjgFbgcSdgcTlmxlbgcService {
    ...
    public boolean generateJdb(CommonInfoVo commonInfoVo) throws IOException, ParseException {
        ...
        List<Map<String,Object>> sdmclist = jjgFbgcSdgcTlmxlbgcMapper.selectsdmcid(proname,htd,idlist);
        if (sdmclist.size()>0){
    ...
    private boolean DBtoExcel(List<JjgFbgcSdgcTlmxlbgc> data, XSSFWorkbook wb) throws ParseException {
        ...
        if(data.size() > 0){
            ...
            for(int i =0; i < data.size(); i++){
                sheet.addMergedRegion(new CellRangeAddress(index+i, index+i+3, 0, 0));
                sheet.getRow(index+i).getCell(0).setCellValue(i+1);

                sheet.addMergedRegion(new CellRangeAddress(index+i, index+i+3, 1, 2));
                ...
                sheet.getRow(index+i+3).getCell(3).setCellValue(Double.parseDouble(data.get(i).getScz4()));
                sheet.addMergedRegion(new CellRangeAddress(index+i+3, index+i+3, 4, 5));
                sheet.addMergedRegion(new CellRangeAddress(index+i+3, index+i+3, 6, 7));
                index+=3;
            }
            return true;
        }
    ...
    private int gettableNum(int size) {
       /* int a = 0;
        ...
        //return size % 8 <= 29 ? size/30+1 : size/30+2;
    }
    ...
    public List<Map<String, Object>> getsdnum(String proname, String htd) {
        List<Map<String,Object>> list = jjgFbgcSdgcTlmxlbgcMapper.getsdnum(proname,htd);
        return list;
    }
    ...
    public void exportsdxlbgs(HttpServletResponse response) {
        ...
        try {
            ...
            File t = new File(courseFile+File.separator+"11隧道砼路面相邻板高差实测数据.xlsx");
            if (t.exists()){
                t.delete();
            }
        }
    ...
    public void importsdxlbgs(MultipartFile file, CommonInfoVo commonInfoVo) {
        ...
        try {
                                ...
                                public void handle(List<JjgFbgcSdgcTlmxlbgcVo> dataList) {
                                    ...
                                    for(JjgFbgcSdgcTlmxlbgcVo tlmxlbgcVo: dataList)
                                    {
                                        if (StringUtils.isEmpty(tlmxlbgcVo.getZh())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中11隧道砼路面相邻板高差实测数据.xlsx文件，第"+rowNumber+"行的数据中，桩号为空，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(tlmxlbgcVo.getSdmc())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中11隧道砼路面相邻板高差实测数据.xlsx文件，第"+rowNumber+"行的数据中，隧道名称为空，请修改后重新上传");
                                        }

                                        if (StringUtils.isEmpty(tlmxlbgcVo.getQyfwz())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中11隧道砼路面相邻板高差实测数据.xlsx文件，第"+rowNumber+"行的数据中，规定值有误，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(tlmxlbgcVo.getScz1())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中11隧道砼路面相邻板高差实测数据.xlsx文件，第"+rowNumber+"行的数据中，实测值1有误，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(tlmxlbgcVo.getScz2())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中11隧道砼路面相邻板高差实测数据.xlsx文件，第"+rowNumber+"行的数据中，实测值2有误，请修改后重新上传");
                                        }
                                        if (StringUtils.isEmpty(tlmxlbgcVo.getScz3())) {
                                            throw new JjgysException(20001, "您上传的"+htd+"合同段中11隧道砼路面相邻板高差实测数据.xlsx文件，第"+rowNumber+"行的数据中，实测值3有误，请修改后重新上传");
                                        }
                                        ...
                                        tlmxlbgc.setUserid(commonInfoVo.getUserid());
            ...
            throw new JjgysException(20001,"您上传的"+htd+"合同段中11隧道砼路面相邻板高差实测数据.xlsx文件，解析excel出错，请传入正确格式的excel");
        }catch (NullPointerException e) {
            throw new JjgysException(20001,"您上传的"+htd+"合同段中11隧道砼路面相邻板高差实测数据.xlsx文件，请检查数据的正确性或删除文件最后的空数据，然后重新上传");
        }catch (ExcelAnalysisException e) {
            throw new JjgysException(20001,"您上传的"+htd+"合同段中11隧道砼路面相邻板高差实测数据.xlsx文件，请将检测日期修改为2021/01/01的格式，然后重新上传");
        }
    ...
    @Override
    public int createMoreRecords(List<JjgFbgcSdgcTlmxlbgc> data, String userID) {
        return ReceiveUtils.createMore(data, jjgFbgcSdgcTlmxlbgcMapper, userID);
    }
}
