...
public class JjgFbgcSdgcZdhczServiceImpl extends ServiceImpl<JjgFbgcSdgcZdhczMapper, JjgFbgcSdgcZdhcz> implements JjgFbgcSdgcZdhczService {
    ...
    public boolean generateJdb(CommonInfoVo commonInfoVo) throws IOException, ParseException {
        ...
        List<Map<String,Object>> lxlist = jjgFbgcSdgcZdhczMapper.selectlxid(proname,htd,idlist);
        if (lxlist.size()>0){
            for (Map<String, Object> map : lxlist) {
                String zx = map.get("lxbs").toString();
                int num = jjgFbgcSdgcZdhczMapper.selectcdnumid(proname,htd,zx,idlist);
                int cds = 0;
                if (num == 1){
                    cds = 2;
                }else {
    ...
    private void handlezxData(String proname, String htd, String zx, int cdsl, String sjz, List<Long> idlist) throws IOException, ParseException {
        log.info("准备数据......");
        List<Map<String, Object>> datazf = jjgFbgcSdgcZdhczMapper.selectzfListyh(proname, htd,zx,idlist);
        List<Map<String, Object>> datayf = jjgFbgcSdgcZdhczMapper.selectyfListyh(proname, htd,zx,idlist);
        ...
        if (!f.exists()) {
            ...
            for (int j = 0; j < wb.getNumberOfSheets(); j++) {
                if (!wb.isSheetHidden(wb.getSheetIndex(wb.getSheetAt(j)))) {
                     ...
                     * 5c 24
                     */
                    XSSFCell htdname = slSheet.getRow(1).getCell(nu);//合同段名
                    Map map = new HashMap();

                    if (proname.equals(xmname.toString()) && htd.equals(htdname.toString())) {
                        slSheet.getRow(0).getCell(ds).setCellType(CellType.STRING);//总点数
                        slSheet.getRow(0).getCell(ds+1).setCellType(CellType.STRING);//合格点数

                        slSheet.getRow(0).getCell(ds-4).setCellType(CellType.STRING);
                        slSheet.getRow(0).getCell(ds-3).setCellType(CellType.STRING);
                        slSheet.getRow(25).getCell(sj).setCellType(CellType.STRING);
                        double zds = Double.valueOf(slSheet.getRow(0).getCell(ds).getStringCellValue());
                        double hgds = Double.valueOf(slSheet.getRow(0).getCell(ds+1).getStringCellValue());
                        String zdsz1 = decf.format(zds);
                        String hgdsz1 = decf.format(hgds);
                        map.put("隧道名称", zx);
                        map.put("路面类型", wb.getSheetName(j));
                        map.put("总点数", zdsz1);
                        map.put("设计值", slSheet.getRow(25).getCell(sj).getStringCellValue());
                        map.put("合格点数", hgdsz1);
                        map.put("合格率", zds!=0 ? df.format(hgds/zds*100) : 0);
                        map.put("Max",slSheet.getRow(0).getCell(ds-4).getStringCellValue());
                        map.put("Min",slSheet.getRow(0).getCell(ds-3).getStringCellValue());
                    }
                    jgmap.add(map);
                }
            }
    ...
    public void exportcz(HttpServletResponse response, String cdsl) throws IOException {
        ...
        try {
            ...
            File t = new File(courseFile+File.separator+"隧道车辙实测数据.xlsx");
            if (t.exists()){
                t.delete();
            }
        }
    ...
    public List<Map<String, Object>> selectlx(String proname, String htd, String userid) {
        ...
        List<Map<String, Object>> selectlx = jjgFbgcSdgcZdhczMapper.selectlxid(proname, htd,idlist);
        return selectlx;
    }
