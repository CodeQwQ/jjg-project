...
public class JjgFbgcSdgcZdhmcxsServiceImpl extends ServiceImpl<JjgFbgcSdgcZdhmcxsMapper, JjgFbgcSdgcZdhmcxs> implements JjgFbgcSdgcZdhmcxsService {
    ...
    public boolean generateJdb(CommonInfoVo commonInfoVo) throws IOException, ParseException {
        ...
        List<Map<String,Object>> lxlist = jjgFbgcSdgcZdhmcxsMapper.selectlxid(proname,htd,idlist);
        if (lxlist.size()>0){
            for (Map<String, Object> map : lxlist) {
                String zx = map.get("lxbs").toString();
                int num = jjgFbgcSdgcZdhmcxsMapper.selectcdnumid(proname,htd,zx,idlist);
                int cds = 0;
                if (num == 1){
                    cds = 2;
                }else {
    ...
    private void handlezxData(String proname, String htd, String zx, int cdsl, String sjz, List<Long> idlist) throws IOException, ParseException {
        //查询用户类型
        List<Map<String,Object>> datazf = jjgFbgcSdgcZdhmcxsMapper.selectzfListyh(proname,htd,zx,idlist);
        List<Map<String,Object>> datayf = jjgFbgcSdgcZdhmcxsMapper.selectyfListyh(proname,htd,zx,idlist);
    ...
    private void writeExcelData(String proname, String htd, List<Map<String, Object>> qlzxList, List<Map<String, Object>> qlyxList, int cdsl, String sjz, String zx) throws IOException, ParseException {
        ...
        String fname="50隧道路面摩擦系数-"+zx+".xlsx";
    ...
    private void fillTitleCellData(XSSFSheet sheet, int tableNum, String proname, String htd, String name,String time,String sheetname,int cdsl,String sjz) {
        ...
        if (cdsl ==2){
            ...
            sheet.getRow(tableNum * 44 + 2).getCell(1).setCellValue("隧道工程");
            ...
            sheet.getRow(tableNum * 44 + 2).getCell(2).setCellValue("隧道工程");
            if (cdsl == 3){
                sheet.getRow(tableNum * 44 + 1).getCell(5).setCellValue(fbgcname+"("+name+")");
            }
            if (cdsl == 4){
    ...
    private List<Map<String, Object>> lookdata(String proname, String htd, String zx, int cds) throws IOException {
        ...
        File f = new File(filepath + File.separator + proname + File.separator + htd + File.separator + "50隧道路面摩擦系数-"+zx+".xlsx");
        int pronamecl = 0;
        int htdcl = 0;
        int cdjg = 3*cds+3;
        int sjzz = cds*2+3;
            ...
            for (int j = 0; j < wb.getNumberOfSheets(); j++) {
                if (!wb.isSheetHidden(wb.getSheetIndex(wb.getSheetAt(j)))) {
                    ...
                    if (proname.equals(xmname.toString()) && htd.equals(htdname.toString())) {
                        slSheet.getRow(1).getCell(cdjg).setCellType(CellType.STRING);//总点数  cdjg
                        slSheet.getRow(1).getCell(cdjg+1).setCellType(CellType.STRING);//合格点数 cdjg+1

                        slSheet.getRow(1).getCell(cdjg+3).setCellType(CellType.STRING);//合格点数cdjg+3
                        slSheet.getRow(1).getCell(cdjg+4).setCellType(CellType.STRING);//合格点数cdjg+4
                        slSheet.getRow(37).getCell(sjzz).setCellType(CellType.STRING);
                        double zds = Double.valueOf(slSheet.getRow(1).getCell(cdjg).getStringCellValue());
                        double hgds = Double.valueOf(slSheet.getRow(1).getCell(cdjg+1).getStringCellValue());
                        ...
                        map.put("总点数", zdsz1);
                        map.put("设计值", slSheet.getRow(37).getCell(sjzz).getStringCellValue());
                        map.put("合格点数", hgdsz1);
                        map.put("合格率", zds!=0 ? df.format(hgds/zds*100) : 0);
                        map.put("Min", slSheet.getRow(1).getCell(cdjg+4).getStringCellValue());
                        map.put("Max", slSheet.getRow(1).getCell(cdjg+3).getStringCellValue());
                        jgmap.add(map);
                    }
                }
            }
    ...
    public List<Map<String, Object>> selectlx(String proname, String htd, String userid) {
        ...
        List<Map<String, Object>> selectlx = jjgFbgcSdgcZdhmcxsMapper.selectlxid(proname,htd,idlist);
        return selectlx;
    }
