...
public class JjgZdhCzJgfcServiceImpl extends ServiceImpl<JjgZdhCzJgfcMapper, JjgZdhCzJgfc> implements JjgZdhCzJgfcService {
    ...
    @Override
    public boolean generateJdb(String proname, String sjz) throws IOException, ParseException {
        //需要根据项目名称批量删除
        ...
        if (zx.equals("主线")) {
                ...
                for (JjgLqsJgLjx jjgLjx : jjgLjxList) {
                    ...
                    if (jjgLqssd != null){
                        for (JjgLqsJgSd jjgLqsSd : jjgLqssd) {
                            ...
                            //连接线隧道
                            sdmcxs.addAll(jjgZdhCzJgfcMapper.selectsdcz(proname,lf,wz,sdzhq,sdz,sdname,zhq1,zhz1));
                        }
                    }
                    ...
                    if (jjgLqsql != null){
                        for (JjgLqsJgQl jjgLqsQl : jjgLqsql) {
                            ...
                            //连接线桥梁
                            qlmcxs.addAll(jjgZdhCzJgfcMapper.selectqlcz(proname,lf, wz,qlzhq, qlzhz, qlname, zhq1, zhz1));
            ...
            if (zdList != null){
                for (JjgLqsJgHntlmzd jjgLqsHntlmzd : zdList) {
                    ...
                    List<JjgLqsJgSd> jjgLqssd = jjgLqsJgSdMapper.selectsdListz(proname,zhq,zhz,bz,wz,zdlf);
                    //有可能是单幅，有可能是左右幅都有
                    if (jjgLqssd != null){
                        for (JjgLqsJgSd jjgLqsSd : jjgLqssd) {
                            String lf = jjgLqsSd.getLf();
                            ...
                            String zhz1 = String.valueOf((jjgLqsSd.getZhz()));
                            //匝道隧道
                            sdmcxs.addAll(jjgZdhCzJgfcMapper.selectsdczyh(proname,bz,lf,sdzhq,sdz,zx,zhq1,zhz1));
                        }
                    }


                    List<JjgLqsJgQl> jjgLqsql = jjgLqsJgQlMapper.selectqlListz(proname,zhq,zhz,bz,wz,zdlf);
                    if (jjgLqsql != null){
                        for (JjgLqsJgQl jjgLqsQl : jjgLqsql) {
                            String lf = jjgLqsQl.getLf();
                            Double qlq = jjgLqsQl.getZhq();
                            ...
                            String zhz1 = String.valueOf(jjgLqsQl.getZhz());
                            //匝道桥梁
                            qlmcxs.addAll(jjgZdhCzJgfcMapper.selectqlczyh(proname,bz,lf, qlzhq, qlzhz, zx, zhq1, zhz1));
    ...
    private void DBtoExcelZd(String proname, String htd, List<Map<String, Object>> data, List<Map<String, Object>> zfsdqlData, XSSFWorkbook wb, String sheetname, int cdsl, String sjz, String zx) throws ParseException {
        ...
        if (data!=null && !data.isEmpty()){
            ...
            System.out.println(getNumzd(lmdata,cdsl));
    ...
    public List<Map<String, Object>> lookJdbjg(String proname) throws IOException {
        ...
        List<String> htds = jjgZdhCzJgfcMapper.gethtd(proname);
        if (htds != null && htds.size()>0){
            for (String htd : htds) {
                ...
                if (lxlist.size()>0){
                    for (Map<String, Object> map : lxlist) {
                        String zx = map.get("lxbs").toString();
                        int num = jjgZdhCzJgfcMapper.selectcdnum(proname,htd,zx);
                        if (num == 1){
                            num = num+1;
                        }
    ...
    private List<Map<String, Object>> lookjdb(String proname, String htd, String zx, int cdsl) throws IOException {
        ...
        if (zx.equals("主线")){
            ...
            f = new File(jgfilepath + File.separator + proname + File.separator + htd + File.separator + "69互通车辙-"+zx+".xlsx");
        }
        if (!f.exists()) {
            return new ArrayList<>();
        } else {
            ...
            for (int j = 0; j < wb.getNumberOfSheets(); j++) {
                if (!wb.isSheetHidden(wb.getSheetIndex(wb.getSheetAt(j)))) {
                    ...
                    if (proname.equals(xmname.toString()) && htd.equals(htdname.toString())) {
                        ...
                        map.put("pjz",slSheet.getRow(1).getCell(c+3).getStringCellValue());
                    }
                    jgmap.add(map);
                }
            }
