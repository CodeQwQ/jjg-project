...
public class JjgZdhCzServiceImpl extends ServiceImpl<JjgZdhCzMapper, JjgZdhCz> implements JjgZdhCzService {
    ...
    public boolean generateJdb(CommonInfoVo commonInfoVo) throws IOException, ParseException {
        ...
        List<Map<String,Object>> lxlist = jjgZdhCzMapper.selectlxid(proname,htd,idlist);
        if (lxlist.size()>0){
            for (Map<String, Object> map : lxlist) {
                String zx = map.get("lxbs").toString();
                int num = jjgZdhCzMapper.selectcdnumid(proname,htd,zx,idlist);
                int cds = 0;
                if (num == 1){
                    cds = 2;
                }else {
    ...
    private void handlezxData(String proname, String htd, String zx, int cdsl, String sjz, List<Long> idlist) throws IOException, ParseException {
        ...
        if (zx.equals("主线")) {
            ...
            List<Map<String, Object>> datazf = jjgZdhCzMapper.selectzfListyh(proname, htd, zx,idlist);
            List<Map<String, Object>> datayf = jjgZdhCzMapper.selectyfListyh(proname, htd, zx,idlist);
            ...
            if (jjgLqsSdzf.size() > 0) {
                for (JjgLqsSd jjgLqsSd : jjgLqsSdzf) {
                    ...
                    hpsdzfdata.addAll(jjgZdhCzMapper.selectSdZfDatayh(proname, htd, zx, zhq, zhz,idlist));
                }
            }
            if (jjgLqsSdyf.size() > 0) {
                for (JjgLqsSd jjgLqsSd : jjgLqsSdyf) {
                    String zhq = String.valueOf(jjgLqsSd.getZhq());
                    String zhz = String.valueOf(jjgLqsSd.getZhz());
                    hpsdyfdata.addAll(jjgZdhCzMapper.selectSdyfDatayh(proname, htd, zx, zhq, zhz,idlist));
                }
            }
            if (jjgLqsQlzf.size() > 0) {
                for (JjgLqsQl jjgLqsQl : jjgLqsQlzf) {
                    ...
                    hpqlzfdata.addAll(jjgZdhCzMapper.selectQlZfDatayh(proname, htd, zx, zhq, zhz,idlist));
                }
            }
            if (jjgLqsQlyf.size() > 0) {
                for (JjgLqsQl jjgLqsQl : jjgLqsQlyf) {
                    String zhq = String.valueOf(jjgLqsQl.getZhq());
                    String zhz = String.valueOf(jjgLqsQl.getZhz());
                    hpqlyfdata.addAll(jjgZdhCzMapper.selectQlYfDatayh(proname, htd, zx, zhq, zhz,idlist));
                }
            }
            ...
            //查询的是摩擦系数表中的连接线
            List<Map<String, Object>> dataljxzf = jjgZdhCzMapper.selectzfListyh(proname, htd, zx,idlist);
            List<Map<String, Object>> dataljxyf = jjgZdhCzMapper.selectyfListyh(proname, htd, zx,idlist);
                ...
                for (JjgLjx jjgLjx : jjgLjxList) {
                    ...
                    if (jjgLqssd != null){
                        for (JjgLqsSd jjgLqsSd : jjgLqssd) {
                            ...
                            //sdmcxs.addAll(jjgZdhCzMapper.selectsdczyh(proname,bz,lf,sdzhq,sdz,zx,zhq1,zhz1,idlist));
                            sdmcxs.addAll(jjgZdhCzMapper.selectsdczyhx(proname,lf,wz,sdzhq,sdz,sdname,zhq1,zhz1,idlist));
                        }
                    }
                    ...
                    if (jjgLqsql != null){
                        for (JjgLqsQl jjgLqsQl : jjgLqsql) {
                            ...
                            //qlmcxs.addAll(jjgZdhCzMapper.selectqlczyh(proname,bz,lf, qlzhq, qlzhz, zx, zhq1, zhz1,idlist));
                            qlmcxs.addAll(jjgZdhCzMapper.selectqlczyhx(proname,lf,wz, qlzhq, qlzhz, qlname, zhq1, zhz1,idlist));
            ...
            List<Map<String,Object>> datazdzf = jjgZdhCzMapper.selectzfListyh(proname,htd,zx,idlist);
            List<Map<String,Object>> datazdyf = jjgZdhCzMapper.selectyfListyh(proname,htd,zx,idlist);
            ...
            List<Map<String, Object>> qlzfsj = groupByZh1zd(zfqlmcxs);
            List<Map<String, Object>> qlyfsj = groupByZh1zd(yfqlmcxs);
            List<Map<String, Object>> sdzfsj = groupByZh1zd(zfsdmcxs);
    ...
    private void DBtoExcelZd(String proname, String htd, List<Map<String, Object>> data, List<Map<String, Object>> zfsdqlData, XSSFWorkbook wb, String sheetname, int cdsl, String sjz, String zx) throws ParseException {
        ...
        if (data!=null && !data.isEmpty()){
            ...
            createTable2(getNumzd(lmdata,cdsl),wb,sheetname,cdsl);
                ...
                if (cdsl == 2 || cdsl == 3){
                    for (Map<String, Object> lm : lmdata) {
                        if (lm.get("zdbs").toString().equals(zdbs)) {
                            ...
                            if (!lm.get("cz").toString().equals("") && !lm.get("cz").toString().isEmpty()) {
                                ...
                                System.out.println(zdbs+" "+lm.get("qdzh")+" "+tableNum);
                    ...
                    for (Map<String, Object> lm : lmdata) {
                        if (lm.get("zdbs").toString().equals(zdbs)) {
                            ...
                            tableNum++;
                            index = 0;
                            System.out.println(zdbs +" "+tableNum);
                            fillTitleCellData(sheet, tableNum, proname, htd, zdbs + "匝道", time, sheetname, cdsl, sjz);
                            if (index % 73 == 0 && index != 0) {
                                tableNum++;
                                fillTitleCellData(sheet, tableNum, proname, htd, zdbs + "匝道", time, sheetname, cdsl, sjz);
     ...
     * @return
     */
    private List<Map<String, Object>> lookjdb(String proname, String htd, String zx, int cdsl, int flag) throws IOException {
            ...
            for (int j = 0; j < wb.getNumberOfSheets(); j++) {
                if (!wb.isSheetHidden(wb.getSheetIndex(wb.getSheetAt(j)))) {
                    ...
                    if (proname.equals(xmname.toString()) && htd.equals(htdname.toString())) {
                        if(flag == 1 || slSheet.getSheetName().contains("路面")){
                            // 合并取法
                            //2c 12
                            //3c 16
                            //4c 15
                            ...
                            slSheet.getRow(25).getCell(e).setCellType(CellType.STRING);
                            ...
                            map.put("分部工程名称", "路面");
                            map.put("总点数", zdsz1);
                            map.put("设计值", slSheet.getRow(25).getCell(e).getStringCellValue());
                            map.put("合格点数", hgdsz1);
                            ...
                            // 分开取法：除了路面表直接拿顶上，其他表都可以遍历第6行拿值。路面表的处理逻辑和上面一样，就放到上面处理了

                            // 遍历工作表行处理表格，在表格开头拿就完了
                            int dataStartRow = 6; // 第6行开始的总数据
                            while (slSheet.getRow(dataStartRow).getCell(c) != null && !slSheet.getRow(dataStartRow).getCell(c).getStringCellValue().trim().isEmpty()){
                                slSheet.getRow(dataStartRow).getCell(c+5).setCellType(CellType.STRING);//总点数
                                slSheet.getRow(dataStartRow).getCell(c+6).setCellType(CellType.STRING);//合格点数

                                slSheet.getRow(dataStartRow).getCell(c+1).setCellType(CellType.STRING);
                                slSheet.getRow(dataStartRow).getCell(c+2).setCellType(CellType.STRING);
                                slSheet.getRow(dataStartRow).getCell(c).setCellType(CellType.STRING); // 分部工程名称

                                slSheet.getRow(25).getCell(e).setCellType(CellType.STRING);

                                double zds = Double.valueOf(slSheet.getRow(dataStartRow).getCell(c+5).getStringCellValue());
                                double hgds = Double.valueOf(slSheet.getRow(dataStartRow).getCell(c+6).getStringCellValue());
                                String zdsz1 = decf.format(zds);
                                String hgdsz1 = decf.format(hgds);
                                Map map = new HashMap();
                                map.put("检测项目", zx);
                                map.put("路面类型", wb.getSheetName(j));
                                // 同时匹配中文和英文括号的正则表达式
                                String s = slSheet.getRow(dataStartRow).getCell(c).getStringCellValue();
                                ...
                                map.put("合格点数", hgdsz1);
                                map.put("合格率", zds!=0 ? df.format(hgds/zds*100) : 0);
                                map.put("Max",slSheet.getRow(dataStartRow).getCell(c+1).getStringCellValue());
                                map.put("Min",slSheet.getRow(dataStartRow).getCell(c+2).getStringCellValue());
                                jgmap.add(map);
                                dataStartRow++;
                                slSheet.getRow(dataStartRow).getCell(c).setCellType(CellType.STRING); // 为了下一次的while校验
                            }
                        }
                    }
                }
    ...
    public void exportcz(HttpServletResponse response, String cdsl) throws IOException {
        ...
        try {
            ...
            File t = new File(courseFile+File.separator+"车辙实测数据.xlsx");
            if (t.exists()){
                t.delete();
            }
        }
                                ...
                                new ExcelHandler<JjgZdhCzVo>(JjgZdhCzVo.class) {
                                    ...
                                    public void handle(List<JjgZdhCzVo> dataList) {
                                        for(JjgZdhCzVo czVo: dataList)
                                        {
                                            ...
                                            cz.setUserid(commonInfoVo.getUserid());
