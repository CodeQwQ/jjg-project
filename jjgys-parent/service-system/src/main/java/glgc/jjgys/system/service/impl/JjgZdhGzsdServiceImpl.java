...
public class JjgZdhGzsdServiceImpl extends ServiceImpl<JjgZdhGzsdMapper, JjgZdhGzsd> implements JjgZdhGzsdService {
    ...
    public boolean generateJdb(CommonInfoVo commonInfoVo) throws IOException, ParseException {
        ...
        List<Map<String,Object>> lxlist = jjgZdhGzsdMapper.selectlxid(proname,htd,idlist);
        if (lxlist.size()>0){
            for (Map<String, Object> map : lxlist) {
                String zx = map.get("lxbs").toString();
                int num = jjgZdhGzsdMapper.selectcdnumid(proname,htd,zx,idlist);
                int cds = 0;
                if (num == 1){
                    cds = 2;
                }else {
    ...
    private void handlezxData(String proname, String htd, String zx, int cdsl, String sjz, List<Long> idlist) throws IOException, ParseException {
        ...
        if (zx.equals("主线")){
             ...
             */
            List<Map<String,Object>> datazf = jjgZdhGzsdMapper.selectzfListyh(proname,htd,zx,idlist);
            List<Map<String,Object>> datayf = jjgZdhGzsdMapper.selectyfListyh(proname,htd,zx,idlist);
            ...
            if (jjgLqsSdzf.size()>0){
                for (JjgLqsSd jjgLqsSd : jjgLqsSdzf) {
                    ...
                    hpsdzfdata.addAll(jjgZdhGzsdMapper.selectSdZfDatayh(proname,htd,zx,zhq,zhz,idlist));
                }
            }
            ...
            if (jjgLqsSdyf.size()>0){
                for (JjgLqsSd jjgLqsSd : jjgLqsSdyf) {
                    String zhq = String.valueOf(jjgLqsSd.getZhq());
                    String zhz = String.valueOf(jjgLqsSd.getZhz());
                    hpsdyfdata.addAll(jjgZdhGzsdMapper.selectSdyfDatayh(proname,htd,zx,zhq,zhz,idlist));
                }
            }
            ...
            if (jjgLqsQlzf.size()>0){
                for (JjgLqsQl jjgLqsQl : jjgLqsQlzf) {
                    ...
                    hpqlzfdata.addAll(jjgZdhGzsdMapper.selectQlZfDatayh(proname,htd,zx,zhq,zhz,idlist));
                }
            }
            ...
            if (jjgLqsQlyf.size()>0){
                for (JjgLqsQl jjgLqsQl : jjgLqsQlyf) {
                    String zhq = String.valueOf(jjgLqsQl.getZhq());
                    String zhz = String.valueOf(jjgLqsQl.getZhz());
                    hpqlyfdata.addAll(jjgZdhGzsdMapper.selectQlYfDatayh(proname,htd,zx,zhq,zhz,idlist));
                }
            }
            ...
            //查询的是摩擦系数表中的连接线
            List<Map<String,Object>> dataljxzf = jjgZdhGzsdMapper.selectzfListyh(proname,htd,zx,idlist);
            List<Map<String,Object>> dataljxyf = jjgZdhGzsdMapper.selectyfListyh(proname,htd,zx,idlist);
            //连接线
                ...
                for (JjgLjx jjgLjx : jjgLjxList) {
                    ...
                    if (jjgLqssd != null){
                        for (JjgLqsSd jjgLqsSd : jjgLqssd) {
                            ...
                            //sdmcxs.addAll(jjgZdhGzsdMapper.selectsdgzsdyh(proname,bz,lf,sdzhq,sdz,zx,zhq1,zhz1,idlist));
                            sdmcxs.addAll(jjgZdhGzsdMapper.selectsdgzsdyhx(proname,lf,wz,sdzhq,sdz,sdname,zhq1,zhz1,idlist));
                        }
                    }
                    ...
                    if (jjgLqsql != null){
                        for (JjgLqsQl jjgLqsQl : jjgLqsql) {
                            ...
                            //qlmcxs.addAll(jjgZdhGzsdMapper.selectqlgzsdyh(proname,bz,lf, qlzhq, qlzhz, zx, zhq1, zhz1,idlist));
                            qlmcxs.addAll(jjgZdhGzsdMapper.selectqlgzsdyhx(proname,lf,wz, qlzhq, qlzhz, qlname, zhq1, zhz1,idlist));
            ...
            List<Map<String, Object>> qlzfsj = groupByZh12(zfqlmcxs);
            List<Map<String, Object>> qlyfsj = groupByZh12(yfqlmcxs);
            List<Map<String, Object>> sdzfsj = groupByZh12(zfsdmcxs);
            List<Map<String, Object>> sdyfsj = groupByZh12(yfsdmcxs);
            ...
            List<Map<String,Object>> datazdzf = jjgZdhGzsdMapper.selectzfListyh(proname,htd,zx,idlist);
            List<Map<String,Object>> datazdyf = jjgZdhGzsdMapper.selectyfListyh(proname,htd,zx,idlist);
            /**
     ...
     * @return
     */
    private static List<Map<String, Object>> groupByZh12(List<Map<String, Object>> list) {
        if (list == null || list.isEmpty()){
            return new ArrayList<>();
        }else {
            Map<String, List<String>> resultMap = new TreeMap<>();
    ...
    private static List<Map<String, Object>> groupByZh(List<Map<String, Object>> list) {
        if (list == null || list.isEmpty()){
            ...
            Collections.sort(resultList, new Comparator<Map<String, Object>>() {
                ...
                public int compare(Map<String, Object> o1, Map<String, Object> o2) {
                    // 名字相同时按照 qdzh 排序
                    String zh = o1.get("name").toString();
                    String zh1 = o2.get("name").toString();
                    int cmp = zh.compareTo(zh1);
                    if (cmp != 0) {
                        return cmp;
                    }
                ...
                if (!wb.isSheetHidden(wb.getSheetIndex(wb.getSheetAt(j)))) {
                    ...
                    if (proname.equals(xmname.toString()) && htd.equals(htdname.toString())) {
                        if (flag == 1 || wb.getSheetName(j).contains("路面")) {
                            ...
                            map.put("分部工程名称", "路面"); // 由于路面表的鉴定表生成的bug导致的空值，直接设定为“路面”
                            map.put("总点数", zdsz1);
                            ...
                            XSSFRow startRow = slSheet.getRow(6); // 总结数据的开始行
                            if(startRow != null && startRow.getCell(4*cds+5) != null){
                                startRow.getCell(4*cds+5).setCellType(CellType.STRING);//分部工程名称位置
                                while(!startRow.getCell(4*cds+5).getStringCellValue().isEmpty()){
                                    startRow.getCell(4*cds+10).setCellType(CellType.STRING);//总点数
                                    startRow.getCell(4*cds+11).setCellType(CellType.STRING);//合格点数

                                    startRow.getCell(4*cds+6).setCellType(CellType.STRING);//max
                                    startRow.getCell(4*cds+7).setCellType(CellType.STRING);//min
                                    slSheet.getRow(25).getCell(cds*4+3).setCellType(CellType.STRING); // 设计值，固定在这个地方取

                                    double zds = Double.valueOf(startRow.getCell(4*cds+10).getStringCellValue());
                                    double hgds = Double.valueOf(startRow.getCell(4*cds+11).getStringCellValue());
                                    String zdsz1 = decf.format(zds);
                                    String hgdsz1 = decf.format(hgds);
                                    Map map = new HashMap();
                                    map.put("检测项目", zx);
                                    ...
                                    // 拿到分部工程
                                    // 同时匹配中文和英文括号的正则表达式
                                    String s = startRow.getCell(4*cds+5).getStringCellValue();
                                    ...
                                    map.put("合格点数", hgdsz1);
                                    map.put("合格率", zds!=0 ? df.format(hgds/zds*100) : 0);
                                    map.put("最大值", startRow.getCell(4*cds+6).getStringCellValue());
                                    map.put("最小值", startRow.getCell(4*cds+7).getStringCellValue());
                                    jgmap.add(map);

                                    // 对下一行进行初始化
                                    startRow = slSheet.getRow(startRow.getRowNum()+1) != null? slSheet.getRow(startRow.getRowNum()+1) : null;
                                    if(startRow != null){
                                        if(startRow.getCell(4*cds+5) != null){
                                            startRow.getCell(4*cds+5).setCellType(CellType.STRING);//分部工程名称位置
                                        }else break;
                                    }else break;
    ...
    public void exportgzsd(HttpServletResponse response, String cdsl) throws IOException {
        ...
        try {
            ...
            File t = new File(courseFile+File.separator+"构造深度实测数据.xlsx");
            if (t.exists()){
                t.delete();
            }
        }
                                ...
                                new ExcelHandler<JjgZdhGzsdVo>(JjgZdhGzsdVo.class) {
                                    ...
                                    public void handle(List<JjgZdhGzsdVo> dataList) {
                                        for(JjgZdhGzsdVo gzsdVo: dataList)
                                        {
                                            ...
                                            gzsd.setUserid(commonInfoVo.getUserid());
