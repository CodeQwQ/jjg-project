...
public class JjgZdhMcxsServiceImpl extends ServiceImpl<JjgZdhMcxsMapper, JjgZdhMcxs> implements JjgZdhMcxsService {
    ...
    public boolean generateJdb(CommonInfoVo commonInfoVo) throws IOException, ParseException {
        ...
        List<Map<String,Object>> lxlist = jjgZdhMcxsMapper.selectlxid(proname,htd,idlist);
        if (lxlist.size()>0){
            for (Map<String, Object> map : lxlist) {
                String zx = map.get("lxbs").toString();
                int num = jjgZdhMcxsMapper.selectcdnumid(proname,htd,zx,idlist);
                int cds = 0;
                if (num == 1){
                    cds = 2;
                }else {
                ...
                handlezxData(proname,htd,zx,cds,commonInfoVo.getSjz(),idlist);
            }
            return true;
        }else {
            return false;
     ...
     * @param idlist
     */
    private void handlezxData(String proname, String htd, String zx, int cdsl, String sjz, List<Long> idlist) throws IOException, ParseException {
        ...
        if (zx.equals("主线")){
             ...
             */
            List<Map<String,Object>> datazf = jjgZdhMcxsMapper.selectzfListyh(proname,htd,zx,idlist);
            List<Map<String,Object>> datayf = jjgZdhMcxsMapper.selectyfListyh(proname,htd,zx,idlist);
            ...
            if (jjgLqsSdzf.size()>0){
                for (JjgLqsSd jjgLqsSd : jjgLqsSdzf) {
                    ...
                    hpsdzfdata.addAll(jjgZdhMcxsMapper.selectSdZfDatayh(proname,htd,zhq,zhz,String.valueOf(jjgLqsSd.getZhq()),String.valueOf(jjgLqsSd.getZhz()),idlist));
                }
            }
            ...
            if (jjgLqsSdyf.size()>0){
                for (JjgLqsSd jjgLqsSd : jjgLqsSdyf) {
                    String zhq = String.valueOf((jjgLqsSd.getZhq()));
                    String zhz = String.valueOf((jjgLqsSd.getZhz()));
                    hpsdyfdata.addAll(jjgZdhMcxsMapper.selectSdyfDatayh(proname,htd,zhq,zhz,String.valueOf(jjgLqsSd.getZhq()),String.valueOf(jjgLqsSd.getZhz()),idlist));
                }
            }
            ...
            if (jjgLqsQlzf.size()>0){
                for (JjgLqsQl jjgLqsQl : jjgLqsQlzf) {
                    ...
                    hpqlzfdata.addAll(jjgZdhMcxsMapper.selectQlZfDatayh(proname,htd,zhq,zhz,String.valueOf(jjgLqsQl.getZhq()),String.valueOf(jjgLqsQl.getZhz()),idlist));
                }
            }
            ...
            if (jjgLqsQlyf.size()>0){
                for (JjgLqsQl jjgLqsQl : jjgLqsQlyf) {
                    String zhq = String.valueOf((jjgLqsQl.getZhq()));
                    String zhz = String.valueOf((jjgLqsQl.getZhz()));
                    hpqlyfdata.addAll(jjgZdhMcxsMapper.selectQlYfDatayh(proname,htd,zhq,zhz,String.valueOf(jjgLqsQl.getZhq()),String.valueOf(jjgLqsQl.getZhz()),idlist));
                }
            }
            ...
            //查询的是摩擦系数表中的连接线，分左右幅
            List<Map<String,Object>> dataljxzf = jjgZdhMcxsMapper.selectzfListyh(proname,htd,zx,idlist);
            List<Map<String,Object>> dataljxyf = jjgZdhMcxsMapper.selectyfListyh(proname,htd,zx,idlist);
            ...
            for (JjgLjx jjgLjx : jjgLjxList) {
                ...
                for (JjgLqsSd jjgLqsSd : jjgLqssd) {
                    ...
                    //sdmcxs.addAll(jjgZdhMcxsMapper.selectsdmcxsyh(proname,bz,lf,sdzhq,sdz,zx,zhq1,zhz1,idlist));
                    sdmcxs.addAll(jjgZdhMcxsMapper.selectsdmcxsyhx(proname,lf,wz,sdzhq,sdz,sdname,zhq1,zhz1,idlist));
                }
                ...
                for (JjgLqsQl jjgLqsQl : jjgLqsql) {
                    ...
                    //qlmcxs.addAll(jjgZdhMcxsMapper.selectqlmcxsyh(proname,bz,lf, qlzhq, qlzhz, qlname, zhq1, zhz1,idlist));
                    qlmcxs.addAll(jjgZdhMcxsMapper.selectqlmcxsyhx(proname,lf, wz,qlzhq, qlzhz, qlname, zhq1, zhz1,idlist));
            ...
            //匝道
            List<Map<String,Object>> datazdzf = jjgZdhMcxsMapper.selectzfListyh(proname,htd,zx,idlist);
            List<Map<String,Object>> datazdyf = jjgZdhMcxsMapper.selectyfListyh(proname,htd,zx,idlist);
            ...
            for (JjgLqsHntlmzd jjgLqsHntlmzd : zdList) {
                ...
                for (JjgLqsQl jjgLqsQl : jjgLqsql) {
                    ...
                    // 还是摩擦系数表
                    qlmcxs.addAll(jjgZdhMcxsMapper.selectqlmcxsyh(proname,bz,lf, qlzhq, qlzhz, zx, zhq1, zhz1,idlist));
            ...
            //{zdzh=130.0, sfc=57, cd=左幅一车道, zdbs=E, qdzh=130.0, createTime=2023-06-12, name=淮宁湾立交EK0+157.338匝道桥, htd=LM-1, proname=陕西高速, id=0a4480e6b314848249fe87bfba96861e, lxbs=淮宁湾立交},
            List<Map<String, Object>> qlzfsj = groupByZh1zd(zfqlmcxs);// 匝道桥梁左幅
            //[{createTime=2024-06-18, name=航天城A匝道桥, qdzh=40.0, sfc=53, zdbs=A}, {createTime=2024-06-18, name=航天城B匝道桥, qdzh=50.0, sfc=54,54, zdbs=B}, {createTime=2024-06-18, name=航天城B匝道桥, qdzh=60.0, sfc=53,57, zdbs=B}, {createTime=2024-06-18, name=航天城B匝道桥, qdzh=70.0, sfc=54,51, zdbs=B}, {createTime=2024-06-18, name=航天城B匝道桥, qdzh=80.0, sfc=55,56, zdbs=B}, {createTime=2024-06-18, name=航天城B匝道桥, qdzh=90.0, sfc=53,52, zdbs=B}, {createTime=2024-06-18, name=航天城B匝道桥, qdzh=100.0, sfc=53,53, zdbs=B}, {createTime=2024-06-18, name=航天城B匝道桥, qdzh=110.0, sfc=52, zdbs=B}, {createTime=2024-06-18, name=航天城B匝道桥, qdzh=120.0, sfc=51, zdbs=B}, {createTime=2024-06-18, name=航天城B匝道桥, qdzh=130.0, sfc=55, zdbs=B}, {createTime=2024-06-18, name=航天城B匝道桥, qdzh=140.0, sfc=51, zdbs=B}, {createTime=2024-06-18, name=航天城B匝道桥, qdzh=150.0, sfc=57, zdbs=B}]
            //[{createTime=2024-06-18, name=航天城A匝道桥, qdzh=40.0, sfc=53, zdbs=A}, {createTime=2024-06-18, name=航天城A匝道桥, qdzh=50.0, sfc=54, zdbs=A}, {createTime=2024-06-18, name=航天城B匝道桥, qdzh=50.0, sfc=54, zdbs=B}, {createTime=2024-06-18, name=航天城A匝道桥, qdzh=60.0, sfc=57, zdbs=A}, {createTime=2024-06-18, name=航天城B匝道桥, qdzh=60.0, sfc=53, zdbs=B}, {createTime=2024-06-18, name=航天城A匝道桥, qdzh=70.0, sfc=51, zdbs=A}, {createTime=2024-06-18, name=航天城B匝道桥, qdzh=70.0, sfc=54, zdbs=B}, {createTime=2024-06-18, name=航天城A匝道桥, qdzh=80.0, sfc=56, zdbs=A}, {createTime=2024-06-18, name=航天城B匝道桥, qdzh=80.0, sfc=55, zdbs=B}, {createTime=2024-06-18, name=航天城A匝道桥, qdzh=90.0, sfc=52, zdbs=A}, {createTime=2024-06-18, name=航天城B匝道桥, qdzh=90.0, sfc=53, zdbs=B}, {createTime=2024-06-18, name=航天城A匝道桥, qdzh=100.0, sfc=53, zdbs=A}, {createTime=2024-06-18, name=航天城B匝道桥, qdzh=100.0, sfc=53, zdbs=B}, {createTime=2024-06-18, name=航天城B匝道桥, qdzh=110.0, sfc=52, zdbs=B}, {createTime=2024-06-18, name=航天城B匝道桥, qdzh=120.0, sfc=51, zdbs=B}, {createTime=2024-06-18, name=航天城B匝道桥, qdzh=130.0, sfc=55, zdbs=B}, {createTime=2024-06-18, name=航天城B匝道桥, qdzh=140.0, sfc=51, zdbs=B}, {createTime=2024-06-18, name=航天城B匝道桥, qdzh=150.0, sfc=57, zdbs=B}]
            List<Map<String, Object>> qlyfsj = groupByZh1zd(yfqlmcxs);
            List<Map<String, Object>> sdzfsj = groupByZh1zd(zfsdmcxs); // 隧道左幅
            List<Map<String, Object>> sdyfsj = groupByZh1zd(yfsdmcxs);
    ...
    private void DBtoExcelZd(String proname, String htd, List<Map<String, Object>> data, List<Map<String, Object>> zfsdqlData, XSSFWorkbook wb, String sheetname,int cdsl,String sjz,String zx) throws ParseException {
        ...
        if (data!=null && !data.isEmpty()){
             ...
             * 还得查一下收费站的数据
             */
            // lmdata里面的数据就包含了收费站的数据
            List<Map<String, Object>> lmdata = handlezdData(proname,data,zfsdqlData,zx);
            createTableZd(getNum1(lmdata,cdsl),wb,sheetname,cdsl);
            ...
            //String name = data.get(0).get("name")+data.get(0).get("zdbs").toString()+"匝道";
            String name = "匝道路面()"+data.get(0).get("zdbs").toString()+"匝道)";
            int index = 0;
            int tableNum = 0;
            ...
            if (lmdata.size()>0) {
                ...
                String zdbs = lmdata.get(0).get("zdbs").toString(); // 互通才有的，ABC这种， A匝道
                for (Map<String, Object> lm : lmdata) {
                    if (lm.get("zdbs").toString().equals(zdbs)) {
                        ...
                        if (!lm.get("sfc").toString().equals("") && !lm.get("sfc").toString().isEmpty()) {
                            ...
                            for (int i = 0; i < sfc.length; i++) {
                                if (index < s2) {
                                    ...
                                    sheet.getRow(tableNum * 44 + 6 + index % s2).getCell(s3 * (index / s2)).setCellValue((Double.parseDouble(lm.get("qdzh").toString())));
                                    if (sfc[i].equals("-")){
                                        sheet.getRow(tableNum * 44 + 6 + index % s2).getCell(s3 * (index / s2) + 1 + i).setCellValue("-");
                                    }else {
                        ...
                        } else { // 这里只有摩擦系数值为空或者0才会进来，所以应该就是收费站了
    ...
    private void writeExcelData(String proname, String htd, List<Map<String, Object>> lmzflist, List<Map<String, Object>> lmyflist, List<Map<String, Object>> sdzxList, List<Map<String, Object>> sdyxList, List<Map<String, Object>> qlzxList, List<Map<String, Object>> qlyxList, int cdsl,String sjz,String zx) throws IOException, ParseException {
        ...
        try {
           /* File directory = new File("service-system/src/main/resources/static");
            ...
            wb.setSheetHidden(wb.getSheetIndex("保证率系数"), HSSFWorkbook.SHEET_STATE_VERY_HIDDEN);
    ...
    private List<Map<String, Object>> handlezdData(String proname,List<Map<String, Object>> data, List<Map<String, Object>> zfsdqlData,String zx) {
        ...
        for (int i = 0; i < jjgSfzs.size(); i++) {
            ...
            double datazhq = Double.parseDouble(data.get(0).get("qdzh").toString());
            double datazhz = Double.parseDouble(data.get(data.size()-1).get("qdzh").toString());

            String zdsfzname = jjgSfzs.get(i).getZdsfzname();
            String sszd = jjgSfzs.get(i).getSszd();
            //if (datazhq <= zhq && datazhz >= zhz){
            if (datazhq <= zhq ){
                sfzlist.addAll(incrementByTennoend(zhq, zhz, zdsfzname, sszd));
            }
        ...
        if (sfzlist.size() > 0) {
            // 把data里面，收费站和匝道在桩号上重合的数据删除
            // 获取 sfzlist 中 qdzh 和 zdbs 的值
            List<Criteria> criteriaList = sfzlist.stream()
                    .map(map -> new Criteria(
                            Double.valueOf((Double) map.get("qdzh")),
                            (String) map.get("zdbs")))
                    .collect(Collectors.toList());

            // 打印出 criteriaList 以便排查问题
            System.out.println("Criteria List: " + criteriaList);

            // 过滤 data，删除符合条件的元素
            data.removeIf(map -> {
                String qdzhStrValue = (String) map.get("qdzh");
                String zdbsValue = (String) map.get("zdbs");

                // 转换为 Double
                Double qdzhValue = Double.valueOf(qdzhStrValue);

                return criteriaList.stream().anyMatch(criteria ->
                        criteria.qdzh.equals(qdzhValue) && criteria.zdbs.equals(zdbsValue)
                );
            });
    ...
    // 辅助类，用于存储 qdzh 和 zdbs 的值
    static class Criteria {
        Double qdzh;
        String zdbs;

        Criteria(Double qdzh, String zdbs) {
            this.qdzh = qdzh;
            this.zdbs = zdbs;
        }
    ...
    private List<Map<String,Object>> incrementByTennoend(double start, double end,String zdsfzname,String sszd) {
        List<Map<String,Object>> result = new ArrayList<>();
        if (start < end) {
            for (double i = start; i < end; i += 10) {
                Map map = new HashMap();
    ...
    private void calculateTotalData(XSSFSheet sheet, XSSFRow rowstart, XSSFRow rowend, FormulaEvaluator e,int cds) {
        if (cds == 5){
            ...
            sheet.getRow(rowend.getRowNum()-3).getCell(9).setCellFormula("IF(ISERROR(ROUND(IF("
                    +sheet.getRow(rowend.getRowNum()-1).getCell(7).getReference()+">2,("
    ...
    private void fillFiveCommonCellData(XSSFSheet sheet, int tableNum, int index, Map<String, Object> row,int cdsl,String zx) {
        ...
        for (int i = 0 ; i < sfc.length ; i++) {
            if (index < 38){
                ...
                sheet.getRow(tableNum * 44 + 6 + index % 38).getCell((cdsl+1) * (index / 38)).setCellValue(Double.valueOf(Double.valueOf(row.get("qdzh").toString())));
                if (sfc[i].equals("-")){
                    sheet.getRow(tableNum * 44 + 6 + index % 38).getCell((cdsl+1) * (index / 38)+1+i).setCellValue(sfc[i]);
                }else {
                    sheet.getRow(tableNum * 44 + 6 + index % 38).getCell((cdsl+1) * (index / 38)+1+i).setCellValue(Double.parseDouble(sfc[i]));
                ...
                sheet.getRow(tableNum * 44 + 6 + index % 38).getCell(6 * (index / 38)).setCellValue((Double.parseDouble(row.get("qdzh").toString())));
                if (sfc[i].equals("-")){
    ...
    private void fillCommonCellData(XSSFSheet sheet, int tableNum, int index, Map<String, Object> row,int cdsl,String zx) {
        ...
        for (int i = 0 ; i < sfc.length ; i++) {
            if (index < 69){
                sheet.getRow(tableNum * 44 + 6 + index % 38).getCell((cdsl+1) * (index / 38)).setCellValue((Double.valueOf(row.get("qdzh").toString())));
                if (sfc[i].equals("-")){
                    sheet.getRow(tableNum * 44 + 6 + index % 38).getCell((cdsl+1) * (index / 38)+1+i).setCellValue(sfc[i]);
                }else {
                    sheet.getRow(tableNum * 44 + 6 + index % 38).getCell((cdsl+1) * (index / 38)+1+i).setCellValue(Double.parseDouble(sfc[i]));
                ...
                sheet.getRow(tableNum * 44 + 6 + index % 69).getCell((cdsl*2+2) * (index / 69)).setCellValue((Double.parseDouble(row.get("qdzh").toString())));
                if (sfc[i].equals("-")){
     ...
     * @return
     */
    private int getNum1(List<Map<String, Object>> data,int cdsl) {
        Map<String, Integer> resultMap = new HashMap<>();
        for (Map<String, Object> map : data) {
     ...
     * @return
     */
    private static List<Map<String, Object>> groupByZh1zd(List<Map<String, Object>> list) {
        if (list == null || list.isEmpty()){
            return new ArrayList<>();
        }else {
            Map<String, List<String>> resultMap = new TreeMap<>();
            for (Map<String, Object> map : list) {
                ...
                if (resultMap.containsKey(zh+":"+zdbs)) {
                    resultMap.get(zh+":"+zdbs).add(sfc);
                } else {
                    List<String> sfcList = new ArrayList<>();
                    sfcList.add(sfc);
                    resultMap.put(zh+":"+zdbs, sfcList);
                }
            }
            List<Map<String, Object>> resultList = new LinkedList<>();
            for (Map.Entry<String, List<String>> entry : resultMap.entrySet()) {
                Map<String, Object> map = new TreeMap<>();
                map.put("qdzh", StringUtils.substringBefore(entry.getKey(),":"));
                map.put("sfc", String.join(",", entry.getValue()));
                for (Map<String, Object> item : list) {
                    if (item.get("qdzh").toString().equals(StringUtils.substringBefore(entry.getKey(),":")) && item.get("zdbs").toString().equals(StringUtils.substringAfter(entry.getKey(),":"))) {
                        map.put("name", item.get("name"));
                        map.put("createTime", item.get("createTime"));
            ...
            Collections.sort(resultList, new Comparator<Map<String, Object>>() {
                ...
                public int compare(Map<String, Object> o1, Map<String, Object> o2) {
                    // 名字相同时按照 qdzh 排序
                    String zh = o1.get("zdbs").toString();
                    String zh1 = o2.get("zdbs").toString();
    ...
    private List<Map<String, Object>> lookjdb(String proname, String htd, String zx, int cds, int flag) throws IOException {
        ...
        int zdscd = 3*cds+3;
        int sjzcd = cds*2+3;
        int fbgc = 0; // 分部工程位置,车道数应该就是只有2，3，4，5
        if (cds == 2){
            pronamecl = 1;
            htdcl = 7;
            fbgc = 4;
                ...
                if (!wb.isSheetHidden(wb.getSheetIndex(wb.getSheetAt(j)))) {
                    ...
                    if (proname.equals(xmname.toString()) && htd.equals(htdname.toString())) {
                        if (flag == 1 || wb.getSheetName(j).contains("路面")) {
                            slSheet.getRow(1).getCell(zdscd).setCellType(CellType.STRING);//总点数zdscd
                            slSheet.getRow(1).getCell(zdscd + 1).setCellType(CellType.STRING);//合格点数zdscd+1

                            slSheet.getRow(1).getCell(zdscd + 3).setCellType(CellType.STRING);//zdscd+3
                            slSheet.getRow(1).getCell(zdscd + 4).setCellType(CellType.STRING);//zdscd+4
                            slSheet.getRow(37).getCell(sjzcd).setCellType(CellType.STRING);//sjzcd
                            double zds = Double.valueOf(slSheet.getRow(1).getCell(zdscd).getStringCellValue());//zdscd
                            double hgds = Double.valueOf(slSheet.getRow(1).getCell(zdscd + 1).getStringCellValue());//zdscd+1
                            ...
                            // 拿到分部工程,这个指标的表格的分部工程位置不一样
                            slSheet.getRow(1).getCell(fbgc).setCellType(CellType.STRING);
                            // 同时匹配中文和英文括号的正则表达式
                            String s = slSheet.getRow(1).getCell(fbgc).toString();
                            ...
                            map.put("总点数", zdsz1);
                            map.put("设计值", slSheet.getRow(37).getCell(sjzcd).getStringCellValue());//sjzcd
                            map.put("合格点数", hgdsz1);
                            map.put("合格率", zds != 0 ? df.format(hgds / zds * 100) : 0);
                            map.put("Min", slSheet.getRow(1).getCell(zdscd + 4).getStringCellValue());//zdscd+4
                            map.put("Max", slSheet.getRow(1).getCell(zdscd + 3).getStringCellValue());//zdscd+3
                            jgmap.add(map);
                        } else if (flag == 2) {
                            // 遍历工作表行处理每个表格
                            for (int i = slSheet.getFirstRowNum(); i <= slSheet.getLastRowNum(); i++) {
                                ...
                                if (currentRow.getCell(0).toString().contains("鉴定表")) {
                                    ...
                                    // 拿到分部工程,这个指标的表格
                                    slSheet.getRow(i + 1).getCell(fbgc).setCellType(CellType.STRING);
                                    // 同时匹配中文和英文括号的正则表达式
                                    String s = slSheet.getRow(i + 1).getCell(fbgc).toString();
                                    Pattern pattern = Pattern.compile("[(（]([^)）]*)[)）]");
                                    Matcher matcher = pattern.matcher(s);
                                    if (matcher.find()) {
                                        // 提取第一个捕获组（括号内的内容）
                                    ...
                                    // 总点数、合格点数
                                    slSheet.getRow(i + 2).getCell(zdscd).setCellType(CellType.STRING);//总点数zdscd
                                    slSheet.getRow(i + 2).getCell(zdscd + 1).setCellType(CellType.STRING);//合格点数zdscd+1
                                    slSheet.getRow(i + 38).getCell(zdscd + 3).setCellType(CellType.STRING);//最大值
                                    slSheet.getRow(i + 38).getCell(zdscd + 4).setCellType(CellType.STRING);// 最小值
                                    slSheet.getRow(i + 37).getCell(sjzcd).setCellType(CellType.STRING);//设计值

                                    double zds = Double.valueOf(slSheet.getRow(i + 2).getCell(zdscd).getStringCellValue());//zdscd
                                    double hgds = Double.valueOf(slSheet.getRow(i + 2).getCell(zdscd + 1).getStringCellValue());//zdscd+1
                                    String zdsz1 = decf.format(zds);
                                    String hgdsz1 = decf.format(hgds);
                                    map.put("总点数", zdsz1);
                                    map.put("设计值", slSheet.getRow(i + 37).getCell(sjzcd).getStringCellValue());//sjzcd
                                    map.put("合格点数", hgdsz1);
                                    map.put("合格率", zds != 0 ? df.format(hgds / zds * 100) : 0);
                                    map.put("Min", slSheet.getRow(i + 38).getCell(zdscd + 4).getStringCellValue());//zdscd+4
                                    map.put("Max", slSheet.getRow(i + 38).getCell(zdscd + 3).getStringCellValue());//zdscd+3
                                    jgmap.add(map);
                                }
                            }
                        }
    ...
    public void exportmcxs(HttpServletResponse response, String cdsl) throws IOException {
        ...
        try {
            ...
            File t = new File(courseFile+File.separator+"摩擦系数实测数据.xlsx");
            if (t.exists()){
                t.delete();
            }
        }
                                ...
                                new ExcelHandler<JjgZdhMcxsVo>(JjgZdhMcxsVo.class) {
                                    ...
                                    public void handle(List<JjgZdhMcxsVo> dataList) {
                                        for(JjgZdhMcxsVo mcxsVo: dataList)
                                        {
                                            ...
                                            mcxs.setUserid(commonInfoVo.getUserid());
