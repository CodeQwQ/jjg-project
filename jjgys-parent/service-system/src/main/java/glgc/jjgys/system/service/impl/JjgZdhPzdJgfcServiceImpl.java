...
public class JjgZdhPzdJgfcServiceImpl extends ServiceImpl<JjgZdhPzdJgfcMapper, JjgZdhPzdJgfc> implements JjgZdhPzdJgfcService {
    ...
    @Override
    public boolean generateJdb(JgCommonEntity commonInfoVo) throws IOException, ParseException {
        ...
        if (htds!=null){
            for (String htd : htds) {
                ...
                for (Map<String, Object> map : lxlist) {
                    ...
                    handlezxData(proname,htd,zx,cds,sjz,num);
                }
     ...
     * @param num
     */
    private void handlezxData(String proname, String htd, String zx, int cdsl, String sjz, int num) throws IOException, ParseException {
        String[] arr = null;
        if (cdsl == 2) {
        ...
        if (zx.equals("主线")){
            List<Map<String,Object>> datazf = jjgZdhPzdJgfcMapper.selectzfListyh(proname,htd,zx,result);
            List<Map<String,Object>> datayf = jjgZdhPzdJgfcMapper.selectyfListyh(proname,htd,zx,result);
            ...
            if (jjgLqsSdzf.size()>0){
                for (JjgLqsJgSd jjgLqsSd : jjgLqsSdzf) {
                    ...
                    sdzfdata.addAll(jjgZdhPzdJgfcMapper.selectSdZfDatayh(proname,htd,zx,zhq,zhz,result));
                }
            }
            if (jjgLqsSdyf.size()>0){
                for (JjgLqsJgSd jjgLqsSd : jjgLqsSdyf) {
                    String zhq = String.valueOf(jjgLqsSd.getZhq());
                    String zhz = String.valueOf(jjgLqsSd.getZhz());
                    sdyfdata.addAll(jjgZdhPzdJgfcMapper.selectSdyfDatayh(proname,htd,zx,zhq,zhz,result));
                }
            }
            if (jjgLqsQlzf.size()>0){
                for (JjgLqsJgQl jjgLqsQl : jjgLqsQlzf) {
                    ...
                    qlzfdata.addAll(jjgZdhPzdJgfcMapper.selectQlZfDatayh(proname,htd,zx,zhq,zhz,result));
                }
            }
            if (jjgLqsQlyf.size()>0){
                for (JjgLqsJgQl jjgLqsQl : jjgLqsQlyf) {
                    String zhq = String.valueOf(jjgLqsQl.getZhq());
                    String zhz = String.valueOf(jjgLqsQl.getZhz());
                    qlyfdata.addAll(jjgZdhPzdJgfcMapper.selectQlYfDatayh(proname,htd,zx,zhq,zhz,result));
                }
            }
            ...
            //查询的是摩擦系数表中的连接线
            List<Map<String,Object>> dataljxzf = jjgZdhPzdJgfcMapper.selectzfListyh(proname,htd,zx,result);
            List<Map<String,Object>> dataljxyf = jjgZdhPzdJgfcMapper.selectyfListyh(proname,htd,zx,result);
            ...
            for (JjgLqsJgLjx jjgLjx : jjgLjxList) {
                ...
                List<JjgLqsJgSd> jjgLqssd = jjgLqsJgSdMapper.selectsdListx(proname,zhq,zhz,wz,ljxlf);
                for (JjgLqsJgSd jjgLqsSd : jjgLqssd) {
                    ...
                    sdpzd.addAll(jjgZdhPzdJgfcMapper.selectsdpzd1yhx(proname,lf,wz,zhq1,zhz1,sdname,sdzhq,sdzhz));
                }
                List<JjgLqsJgQl> jjgLqsql = jjgLqsJgQlMapper.selectqlListx(proname,zhq,zhz,wz,ljxlf);
                for (JjgLqsJgQl jjgLqsQl : jjgLqsql) {
                    ...
                    qlpzd.addAll(jjgZdhPzdJgfcMapper.selectqlpzd1yhx(proname,lf,wz, qlzhq, qlzhz, qlname, zhq1, zhz1));
                }
            }
            ...
            //拼接IRI数据
            List<Map<String, Object>> sdzList = montageIRI(zfsdpzd);
            List<Map<String, Object>> sdyList = montageIRI(yfsdpzd);
            List<Map<String, Object>> qlzList = montageIRI(zfqlpzd);

            List<Map<String, Object>> qlyList = montageIRI(yfqlpzd);

            List<Map<String, Object>> zdzfList = montageIRI(dataljxzf);
            List<Map<String, Object>> zdyfList = montageIRI(dataljxyf);
            ...
            //匝道的所有数据
            List<Map<String,Object>> datazdzf = jjgZdhPzdJgfcMapper.selectzfListyh(proname,htd,zx,result);
            List<Map<String,Object>> datazdyf = jjgZdhPzdJgfcMapper.selectyfListyh(proname,htd,zx,result);
            ...
            for (JjgLqsJgHntlmzd jjgLqsHntlmzd : zdList) {
                ...
                if (jjgLqssd.size()>0){
                    for (JjgLqsJgSd jjgLqsSd : jjgLqssd) {
                        ...
                        String zhq1 = String.valueOf((jjgLqsSd.getZhq()));
                        String zhz1 = String.valueOf((jjgLqsSd.getZhz()));
                        sdpzd.addAll(jjgZdhPzdJgfcMapper.selectsdpzdyh(proname,bz,lf,zx,zhq1,zhz1,sdzhz));
                    }
                ...
                if (jjgLqsql.size()>0){
                    for (JjgLqsJgQl jjgLqsQl : jjgLqsql) {
                        ...
                        qlpzd.addAll(jjgZdhPzdJgfcMapper.selectqlpzdyh(proname,bz,lf, qlzhq, qlzhz, zx,qlzhzj));
    ...
    private void calculateAsphaltPavementSheet(XSSFWorkbook wb, XSSFSheet sheet,int cdsl) {
        ...
        for (int i = 0; i <= sheet.getPhysicalNumberOfRows()-7; i++) {
            ...
            if (!"".equals(row.getCell(0).toString()) && !name.equals(row.getCell(0).toString()) && flag) {
                ...
                calculateTotalData(sheet, rowstart.getRowNum() + 2, rowstart, rowend, 2,e,cdsl);
            }
    ...
    private void setExtraWholeTotalData(XSSFSheet sheet, XSSFRow rowtotal, FormulaEvaluator e,int cdsl) {
        ...
        double value1 = e.evaluate(rowtotal.getCell(cdsl*4+13)).getNumberValue();
        //rowtotal.getCell(cdsl*4+13).setCellFormula(null);
        rowtotal.getCell(cdsl*4+13).setCellValue(value1);
            ...
            sheet.getRow(rownum)
                    ...
                    .setCellFormula(
                            "(IF("
                                    ...
                                    + sheet.getRow(rownum + 3).getCell(cdsl*4+8)
                                    .getReference() + "))/2");
            double value1 = e.evaluate(sheet.getRow(rownum).getCell(cdsl*4+13)).getNumberValue();
            //sheet.getRow(rownum).getCell(cdsl*4+13).setCellFormula(null);
            sheet.getRow(rownum).getCell(cdsl*4+13).setCellValue(value1);
                    ...
                    for (int j = 0; j < wb.getNumberOfSheets(); j++) {
                        if (!wb.isSheetHidden(wb.getSheetIndex(wb.getSheetAt(j)))) {
                            ...
                            if (proname.equals(xmname.toString()) && htd.equals(htdname.toString())) {
                                slSheet.getRow(5).getCell(cds*4+13).setCellType(CellType.STRING);//22 26 30 34
                                // cds*4+14
                                Map tempmap = new HashMap();
                                tempmap.put("检测项目", lxbs);
                                tempmap.put("路面类型", wb.getSheetName(j));
                                tempmap.put("平均值", slSheet.getRow(5).getCell(cds*4+13).getStringCellValue());
    ...
    private List<Map<String, Object>> lookjdb(String proname, String htd, String zx, int cds) throws IOException {
        ...
        if (zx.equals("主线")){
            ...
            f = new File(jgfilepath + File.separator + proname + File.separator + htd + File.separator + "61互通平整度-"+zx+".xlsx");
        }
        if (!f.exists()) {
            return new ArrayList<>();
        } else {
            ...
            for (int j = 0; j < wb.getNumberOfSheets(); j++) {
                if (!wb.isSheetHidden(wb.getSheetIndex(wb.getSheetAt(j)))) {
                    ...
                    if (proname.equals(xmname.toString()) && htd.equals(htdname.toString())) {
                        ...
                        slSheet.getRow(5).getCell(cds*4+13).setCellType(CellType.STRING);
                        ...
                        map.put("pjz",slSheet.getRow(5).getCell(cds*4+13).getStringCellValue());
                        jgmap.add(map);
                    }
