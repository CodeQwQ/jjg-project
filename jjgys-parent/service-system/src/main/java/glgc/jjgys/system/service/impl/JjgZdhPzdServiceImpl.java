...
public class JjgZdhPzdServiceImpl extends ServiceImpl<JjgZdhPzdMapper, JjgZdhPzd> implements JjgZdhPzdService {
    ...
    public boolean generateJdb(CommonInfoVo commonInfoVo) throws IOException, ParseException {
        ...
        List<Map<String,Object>> lxlist = jjgZdhPzdMapper.selectlxid(proname,htd,idlist);
        if (lxlist.size()>0){
            for (Map<String, Object> map : lxlist) {
                String zx = map.get("lxbs").toString();
                int num = jjgZdhPzdMapper.selectcdnumid(proname,htd,zx,idlist);
                int cds = 0;
                if (num == 1){
                    cds = 2;
                }else {
                ...
                handlezxData(proname,htd,zx,cds,commonInfoVo,idlist,num);
            }
            return true;
        }else {
            return false;
    ...
    private void handlezxData(String proname, String htd, String zx, int cdsl, CommonInfoVo commonInfoVo, List<Long> idlist, int num) throws IOException, ParseException {
        ...
        if (zx.equals("主线")){
            List<Map<String,Object>> datazf = jjgZdhPzdMapper.selectzfListyh(proname,htd,zx,result,idlist);
            List<Map<String,Object>> datayf = jjgZdhPzdMapper.selectyfListyh(proname,htd,zx,result,idlist);
            ...
            if (jjgLqsSdzf.size()>0){
                for (JjgLqsSd jjgLqsSd : jjgLqsSdzf) {
                    ...
                    sdzfdata.addAll(jjgZdhPzdMapper.selectSdZfDatayh(proname,htd,zx,zhq,zhz,result,idlist));
                }
            }
            if (jjgLqsSdyf.size()>0){
                for (JjgLqsSd jjgLqsSd : jjgLqsSdyf) {
                    String zhq = String.valueOf(jjgLqsSd.getZhq());
                    String zhz = String.valueOf(jjgLqsSd.getZhz());
                    sdyfdata.addAll(jjgZdhPzdMapper.selectSdyfDatayh(proname,htd,zx,zhq,zhz,result,idlist));
                }
            }
            if (jjgLqsQlzf.size()>0){
                for (JjgLqsQl jjgLqsQl : jjgLqsQlzf) {
                    ...
                    qlzfdata.addAll(jjgZdhPzdMapper.selectQlZfDatayh(proname,htd,zx,zhq,zhz,result,idlist));
                }
            }
            if (jjgLqsQlyf.size()>0){
                for (JjgLqsQl jjgLqsQl : jjgLqsQlyf) {
                    String zhq = String.valueOf(jjgLqsQl.getZhq());
                    String zhz = String.valueOf(jjgLqsQl.getZhz());
                    qlyfdata.addAll(jjgZdhPzdMapper.selectQlYfDatayh(proname,htd,zx,zhq,zhz,result,idlist));
                }
            }


            List<Map<String, Object>> sdzxList = montageIRI(sdzfdata,num);
            List<Map<String, Object>> sdyxList = montageIRI(sdyfdata,num);
            List<Map<String, Object>> qlzxList = montageIRI(qlzfdata,num);
            List<Map<String, Object>> qlyxList = montageIRI(qlyfdata,num);

            List<Map<String, Object>> lmzfList = montageIRI(datazf,num);
            ...
            List<Map<String, Object>> lmyfList = montageIRI(datayf,num);
            ...
            writeExcelData(proname,htd,lmzf,lmyf,sdzxList,sdyxList,qlzxList,qlyxList,cdsl, commonInfoVo,zx);
        }else if (zx.contains("连接线")){
            //查询的是摩擦系数表中的连接线
            List<Map<String,Object>> dataljxzf = jjgZdhPzdMapper.selectzfListyh(proname,htd,zx,result,idlist);
            List<Map<String,Object>> dataljxyf = jjgZdhPzdMapper.selectyfListyh(proname,htd,zx,result,idlist);
            ...
            for (JjgLjx jjgLjx : jjgLjxList) {
                ...
                for (JjgLqsSd jjgLqsSd : jjgLqssd) {
                    ...
                    //sdpzd.addAll(jjgZdhPzdMapper.selectsdpzd1yh(proname,bz,lf,zx,zhq1,zhz1,sdzhq,sdzhz,idlist));
                    sdpzd.addAll(jjgZdhPzdMapper.selectsdpzd1yhx(proname,lf,wz,zhq1,zhz1,sdname,sdzhq,sdzhz,idlist));
                }
                ...
                for (JjgLqsQl jjgLqsQl : jjgLqsql) {
                    ...
                    //qlpzd.addAll(jjgZdhPzdMapper.selectqlpzd1yh(proname,bz,lf, qlzhq, qlzhz, zx, zhq1, zhz1,idlist));
                    qlpzd.addAll(jjgZdhPzdMapper.selectqlpzd1yhx(proname,lf,wz, qlzhq, qlzhz, qlname, zhq1, zhz1,idlist));
                }
            }
            ...
            //拼接IRI数据
            List<Map<String, Object>> sdzList = montageIRI(zfsdpzd,num);
            List<Map<String, Object>> sdyList = montageIRI(yfsdpzd,num);
            List<Map<String, Object>> qlzList = montageIRI(zfqlpzd,num);

            List<Map<String, Object>> qlyList = montageIRI(yfqlpzd,num);

            List<Map<String, Object>> zdzfList = montageIRI(dataljxzf,num);
            List<Map<String, Object>> zdyfList = montageIRI(dataljxyf,num);
            ...
            writeExcelData(proname,htd,zdzf,zdyf,sdzList,sdyList,qlzList,qlyList,cdsl,commonInfoVo,zx);

        }else {
            //匝道的所有数据
            List<Map<String,Object>> datazdzf = jjgZdhPzdMapper.selectzfListyh(proname,htd,zx,result,idlist);
            List<Map<String,Object>> datazdyf = jjgZdhPzdMapper.selectyfListyh(proname,htd,zx,result,idlist);
            //匝道表中的数据
            ...
            for (JjgLqsHntlmzd jjgLqsHntlmzd : zdList) {
                ...
                if (jjgLqssd.size()>0){
                    for (JjgLqsSd jjgLqsSd : jjgLqssd) {
                        String lf = jjgLqsSd.getLf();
                        Double sdz = jjgLqsSd.getZhz();
                        String sdzhz = String.valueOf(sdz);

                        String zhq1 = String.valueOf((jjgLqsSd.getZhq()));
                        String zhz1 = String.valueOf((jjgLqsSd.getZhz()));
                        sdpzd.addAll(jjgZdhPzdMapper.selectsdpzdyh(proname,bz,lf,zx,zhq1,zhz1,sdzhz,idlist));
                    }
                ...
                if (jjgLqsql.size()>0){
                    for (JjgLqsQl jjgLqsQl : jjgLqsql) {
                        ...
                        qlpzd.addAll(jjgZdhPzdMapper.selectqlpzdyh(proname,bz,lf, qlzhq, qlzhz, zx,qlzhzj,idlist));
            ...
            //拼接IRI数据
            List<Map<String, Object>> sdzList = montageZDIRI(zfsdpzd,cdsl,num);
            List<Map<String, Object>> sdyList = montageZDIRI(yfsdpzd,cdsl,num);
            List<Map<String, Object>> qlzList = montageZDIRI(zfqlpzd,cdsl,num);

            List<Map<String, Object>> qlyList = montageZDIRI(yfqlpzd,cdsl,num);

            List<Map<String, Object>> zdzfList = montageZDIRI(datazdzf,cdsl,num);
            List<Map<String, Object>> zdyfList = montageZDIRI(datazdyf,cdsl,num);
     ...
     * @return
     */
    private static List<Map<String, Object>> montageLJXIRI(List<Map<String, Object>> list,int cdsl) {
        if (list == null || list.isEmpty()){
            ...
            for (Map<String, Object> map : list) {
                ...
                String key = qdzh; // 添加zdbs的唯一标识，防止重复覆盖
            ...
            for (Map.Entry<String, List<String>> entry : resultMapz.entrySet()) {
                ...
                for (Map<String, Object> item : list) {
                    String itemkey = item.get("qdzh").toString();
     ...
     * @return
     */
    private static List<Map<String, Object>> montageZDIRI(List<Map<String, Object>> list, int cdsl, int num) {
        if (list == null || list.isEmpty()){
            return new ArrayList<>();
        }else {
            if (num == 1 ){
                for (Map<String, Object> map : list) {
                    if (map.containsKey("ziri")){
                        String ziri = map.get("ziri").toString()+",-";
                        map.put("ziri",ziri);
                    }
                    if (map.containsKey("yiri")){
                        String yiri = map.get("yiri").toString()+",-";
                        map.put("yiri",yiri);
                    }
                }
            ...
            for (Map.Entry<String, List<String>> entry : resultMapz.entrySet()) {
                ...
                for (Map<String, Object> item : list) {
                    ...
                    if (itemkey.equals(entry.getKey())) {
                        ...
                        map.put("name", item.get("name"));
                        map.put("zdbs", item.get("zdbs"));
                        map.put("cd", item.get("cd").toString().toString().substring(0,2));
                        map.put("createTime", item.get("createTime"));
                        break;
                    }
                }
    ...
    private void writeExcelData(String proname, String htd, List<Map<String, Object>> lmzfList, List<Map<String, Object>> lmyfList, List<Map<String, Object>> sdzxList, List<Map<String, Object>> sdyxList, List<Map<String, Object>> qlzxList, List<Map<String, Object>> qlyxList, int cdsl, CommonInfoVo commonInfoVo, String zx) throws IOException, ParseException {
        ...
        try {
            ...
            if (sdzxList.size() >0 && !sdzxList.isEmpty()){
                ...
                if (zx.equals("主线")){
                    //主线隧道
                    s = commonInfoVo.getZssjz();
                }else if(zx.contains("连接线")){
                    //连接线隧道
                    s = commonInfoVo.getLssjz();
                }else {
                    //匝道隧道
                    s = commonInfoVo.getZssjz();
                }
                DBtoExcel(proname,htd,mapslist,wb,sheetmame,cdsl,s,zx);
            }
            ...
            if (qlzxList.size()>0 && !qlzxList.isEmpty()){
                ...
                if (zx.equals("主线")){
                    //主线隧道
                    s = commonInfoVo.getZqsjz();
                }else if(zx.contains("连接线")){
                    //连接线隧道
                    s = commonInfoVo.getLqsjz();
                }else {
                    //匝道隧道
                    s = commonInfoVo.getZqsjz();
                }
            ...
            if (lmzfList.size()>0 && !lmzfList.isEmpty()){
                ...
                if (zx.equals("主线") || zx.contains("连接线")){
                    String s = "";
                    if (zx.equals("主线")){
                        s = commonInfoVo.getZlsjz();
                    }else {
                        s = commonInfoVo.getLlsjz();
                    }
                    sheetmame = "沥青路面";
                    DBtoExcelLM(proname,htd,addList,sdqlData,wb,sheetmame,cdsl,s,zx);
                }else {
                    sheetmame = "沥青匝道";
                    DBtoExcelZD(proname,htd,addList,sdqlData,wb,sheetmame,cdsl,commonInfoVo.getZlsjz(),zx);
                }
            ...
            String[] arr = {"混凝土匝道隧道","沥青匝道隧道","混凝土匝道桥","沥青匝道桥","沥青匝道","混凝土匝道","混凝土隧道","沥青隧道","混凝土桥","沥青桥","混凝土路面","沥青路面","混凝土收费站"};
    ...
    private void DBtoExcelZD(String proname, String htd, List<Map<String, Object>> data,List<Map<String, Object>> sdqlData, XSSFWorkbook wb, String sheetname, int cdsl, String sjz, String zx) throws ParseException {
        ...
        if (data != null && !data.isEmpty()) {
            int n = getNumzd(data,cdsl);
            /*double bb = Double.valueOf(n)/2;
            int roundedNum = (int) Math.ceil(bb);*/
            createTable(n, wb, sheetname, cdsl);
            ...
            if (sfzmaplist.size()>0){
                ...
                createTable(getZDNum(sfzmaplist, cdsl), wb, sheesfzname, cdsl);
        ...
        if (mapslist.size()>0) {
            ...
            for (Map<String, Object> zd : mapslist) {
                if (zd.get("zdbs").toString().equals(zdbs)) {
                    ...
                    if (!"".equals(zd.get("iri").toString()) && !zd.get("iri").toString().isEmpty()) {
                        double n = Double.valueOf(zd.get("qdzh").toString()) / 1000;
                        int m = (int) n;
                        //fillCommonCellData(sheet, tableNum, index, lm,cdsl,"zx");
                        sheet.getRow(tableNum * a + 7 + index % b).getCell(0).setCellValue(m);
                        sheet.getRow(tableNum * a + 7 + index % b).getCell(4).setCellValue(Double.valueOf(zd.get("qdzh").toString()));
                        String[] sfc = zd.get("iri").toString().split(",");
                        for (int i = 0 ; i < sfc.length ; i++) {
                            if (zd.get("cd").toString().contains("左幅")){
                                if (!sfc[i].equals("-")) {
                        ...
                        for (int i = 0; i < cdsl; i++) {
                            ...
                            sheet.getRow(tableNum * a + 7 + index % b).getCell(4).setCellValue(Double.valueOf(zd.get("qdzh").toString()));
                            if (zd.get("cd").toString().contains("左幅")){
                                sheet.getRow(tableNum * a + 7 + index % b).getCell(5 + (i*2)).setCellValue(zd.get("name").toString());
                                ...
                                sheet.getRow(tableNum * a + 7 + index % b).getCell((2*cdsl+5) + (i*2)).setCellValue(zd.get("name").toString());
                                startRow = tableNum * a + 7 + index % b ;
                                endRow = tableNum * a + 7 + index % b ;
    ...
    private Map<String, List<Map<String, Object>>> handlezdData(String proname, List<Map<String, Object>> data, List<Map<String, Object>> sdqlData, String zx) {
        ...
        for (int i = 0; i < jjgSfzs.size(); i++) {
            ...
            for (Map<String, Object> datum : data) {
                double datazhq = Double.parseDouble(datum.get("qdzh").toString());
        ...
        if (sdqlData.size() > 0) {
            for (Map<String, Object> datum : data) {
                for (Map<String, Object> zfsdqlDatum : sdqlData) {
                    if (datum.get("zdbs").toString().equals(zfsdqlDatum.get("zdbs")) && datum.get("qdzh").toString().equals(zfsdqlDatum.get("qdzh")) && datum.get("cd").toString().equals(zfsdqlDatum.get("cd").toString())) {
                        datum.put("iri", "");
    ...
    private void fillTotalData(XSSFSheet sheet, int rownum, XSSFRow rowstart, XSSFRow rowend, int left_right, int fillcolum, boolean hasdata,FormulaEvaluator e,int cdsl) {
        ...
        if (left_right == 0){
            ...
            if (hasdata) {
                ...
                sheet.getRow(rownum + 6).getCell(fillcolum)
                        .setCellFormula("IFERROR("+
    ...
    private void DBtoExcelLM(String proname, String htd, List<Map<String, Object>> data,List<Map<String, Object>> sdqlData, XSSFWorkbook wb, String sheetname, int cdsl, String sjz, String zx) throws ParseException {
        ...
        SimpleDateFormat outputDateFormat = new SimpleDateFormat("yyyy.MM.dd");
        if (data != null && !data.isEmpty()) {
            int n = getNum(data,cdsl);
            double b = Double.valueOf(n)/2;
            int roundedNum = (int) Math.ceil(b);
            createTable(roundedNum, wb, sheetname, cdsl);
            XSSFSheet sheet = wb.getSheet(sheetname);
    ...
    private void writezyf(XSSFSheet sheet, List<Map<String, Object>> lmdata, String proname, String htd, String name, String sj, String sheetname, int cdsl, String sjz) {
        ...
        for (Map<String, Object> lm : lmdata) {
            ...
            if (!lm.get("iri").toString().equals("") && !lm.get("iri").toString().isEmpty()) {
                double n = Double.valueOf(lm.get("qdzh").toString()) / 1000;
                int m = (int) n;
                //fillCommonCellData(sheet, tableNum, index, lm,cdsl,"zx");
                sheet.getRow(tableNum * a + 7 + index % b).getCell(0).setCellValue(m);
                sheet.getRow(tableNum * a + 7 + index % b).getCell(4).setCellValue(Double.valueOf(lm.get("qdzh").toString()));
    ...
    private int getNumzd(List<Map<String, Object>> data,int cdsl) {
        List<Map<String, Object>> zfdata = new ArrayList<>();
        List<Map<String, Object>> yfdata = new ArrayList<>();
        for (Map<String, Object> stringObjectMap : data) {
            if (stringObjectMap.get("cd").toString().equals("左幅")){
                zfdata.add(stringObjectMap);
            }
            if (stringObjectMap.get("cd").toString().equals("右幅")){
                yfdata.add(stringObjectMap);
            }
        }
        int num1 = 0;
        if (zfdata.size()>0){
            ...
            for (Map<String, Object> map : data) {
                String name = map.get("name").toString()+map.get("zdbs").toString();
    ...
    private void fillCommonCellData(XSSFSheet sheet, int tableNum, int index, Map<String, Object> row, int cdsl, String zx) {
        ...
        double n = Double.valueOf(row.get("qdzh").toString()) / 1000;
        int m = (int) n;
        sheet.getRow(tableNum * a + 7 + index % b).getCell(0).setCellValue(m);
        sheet.getRow(tableNum * a + 7 + index % b).getCell(4).setCellValue(Double.valueOf(row.get("qdzh").toString()));
     ...
     * @return
     */
    private static List<Map<String, Object>> montageIRI(List<Map<String, Object>> list, int num) {
        if (list == null || list.isEmpty()){
            return new ArrayList<>();
        }else {
            if (num == 1 ){
    ...
    public void exportpzd(HttpServletResponse response, String cdsl) throws IOException {
        ...
        try {
            ...
            File t = new File(courseFile+File.separator+"平整度实测数据.xlsx");
            if (t.exists()){
                t.delete();
            }
        }
                        ...
                        .registerReadListener(
                                new ExcelHandler<JjgZdhPzdVo>(JjgZdhPzdVo.class) {
                                    ...
                                    public void handle(List<JjgZdhPzdVo> dataList) {
                                        ...
                                        {
                                            Double aDouble = Double.valueOf(pzdVo.getQdzh());
                                            if (aDouble % 100 !=0){
                                                throw new JjgysException(2001,"请检查第"+rowNumber+"行的起始桩号，确保是从100的整数倍开始");
                                            }
                                            ...
                                            pzd.setUserid(commonInfoVo.getUserid());
