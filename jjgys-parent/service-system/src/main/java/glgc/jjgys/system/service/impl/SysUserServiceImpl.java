...
public class SysUserServiceImpl extends ServiceImpl<SysUserMapper, SysUser> implements SysUserService {
    ...
    public Map<String, Object> getUserInfo(String username, HttpServletRequest request) {
        ...
        List<RouterVo> routerVolist = sysMenuService.getUserMenuList(sysUser.getId().toString(),request);
        //根据userid查询按钮权限值
        List<String> permsList = sysMenuService.getUserButtonList(sysUser.getId().toString());

        Map<String,Object> result = new HashMap<>();
        result.put("name",sysUser);
        ...
        List<RouterVo> routerVolist = sysMenuService.getUserMenuList(sysUser.getId().toString(),request);
        //根据userid查询按钮权限值
        List<String> permsList = sysMenuService.getUserButtonList(sysUser.getId().toString(),request);
