...
public class JjgFbgcUtils {
     ...
     * @return
     */
    public static File jkfile(MultipartFile multipartFile, String proname, String filespath) {
        File file = null;
        InputStream inputStream = null;
        OutputStream outputStream = null;
        try {
            // 确保目标目录存在
            File dir = new File(filespath+File.separator+proname);
            if (!dir.exists()) {
                dir.mkdirs();
            }
    ...
    public static void copyFile(String OriginalPathName, java.nio.file.Path path, String pathName) {
        java.nio.file.Path originalPath = Paths.get(pathName);
        //System.out.println("wq"+OriginalPathName);
        try {
            /*File dbfFile = new File(path.toUri());
            if (!dbfFile.exists() ) {
                System.out.println(dbfFile.getName()+":"+dbfFile.getPath());
    ...
    public static void copyFile666(String sourceFile, String destinationDirectory, String destinationFileName) {
        Path sourcePath = Paths.get(sourceFile);
        Path destinationPath = Paths.get(destinationDirectory, destinationFileName);


        try {
            File fdir = new File(destinationDirectory);
            if (!fdir.exists()){
                fdir.mkdirs();
            }
            Files.copy(sourcePath, destinationPath, StandardCopyOption.REPLACE_EXISTING); // 使用REPLACE_EXISTING选项来覆盖已存在的文件
        } catch (IOException e) {
            e.printStackTrace();
            // 在这里处理异常，比如记录日志、提示用户等
        }
    }
    ...
    public static void addFile(List<File> files, String path, String pathName) {
        ...
        for (File file : files) {
            if (file.exists()){
                ...
                //String targetPath = Paths.get(path + File.separator + name).toString();
                //copyFile(file.getName(), targetPath, pathName);
                copyFile666(file.getPath(),path,name);
            }
        }
    }
