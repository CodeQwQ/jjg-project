<template>
    <div class="app-container">
        ...
        <div class="tools-div" v-show="!(username=='admin'&&type==1  )">
          <div class="anniu">
            <el-button
              class="btn-add"
              type="success"
              @click="exportjathldmcc()"
              style="margin-left: 0px"
              icon="el-icon-bottom"
              size="mini"
              >导出交安砼护栏断面尺寸文件</el-button
            >
            <!-- :disabled="$hasBP('bnt.ql.export')  === true" -->
            <el-button
              class="btn-add"
              type="success"
              @click="importjathldmcc()"
              style="margin-left: 0px"
              icon="el-icon-top"
              size="mini"
              >导入交安砼护栏断面尺寸文件</el-button
            >
            <el-button
              class="btn-add"
              type="success"
        ...
        >
          ...
          <el-table-column prop="jcrq" label="检测日期" width="100px" />
          <el-table-column prop="zh" label="桩号" width="220px" />
          <el-table-column prop="bw" label="部位" />
          
          <el-table-column prop="lb" label="类别" width="100px" />
        ...
        <img v-viewer style="height: 100px;width: 450px;" src="@/assets/填写说明/交安/交安砼护栏断面尺寸.png" alt="" >
        ...
        <el-dialog title="修改" :visible.sync="dialogVisible2" width="40%" >
          <el-form ref="dataForm" :model="modifyData"  label-width="150px" size="small" style="padding-right: 40px;" :rules="rules">
            ...
            <el-form-item label="检测日期" prop="jcrq">
              <el-input v-model="modifyData.jcrq"></el-input>
            </el-form-item>
    ...
    <script>
    // 引入定义接口js文件
    import api from "@/api/project/fbgc/jagc/jathldmcc.js";
    ...
    export default {
      ...
      methods: {
        ...
        onUploadSuccess() {
          ...
          let load=Loading.service({fullscreen:true,text:'加载中,请稍后!'})
          api.importjathldmcc(fd).then((res)=>{
            console.log('res',res)
            if(res.message=='成功'){
              this.fetchData();
              this.$message({
        ...
        exportjathldmcc() {
          ...
          api.exportjathldmcc().then((res) => {
            ...
            // 设置导出的文件名称
            link.download = `05交安砼护栏断面尺寸实测数据` + ".xlsx";
