<template>
    <div class="app-container">
        ...
        <div class="tools-div" v-show="!(username=='admin'&&type==1  )">
          <div class="anniu">
            <el-button
              class="btn-add"
              type="success"
              @click="exporthntlmhdzx()"
              style="margin-left: 0px"
              icon="el-icon-bottom"
              size="mini"
              >导出混凝土路面厚度钻芯法文件</el-button
            >
            <!-- :disabled="$hasBP('bnt.ql.export')  === true" -->
            <el-button
              class="btn-add"
              type="success"
              @click="importhntlmhdzx()"
              style="margin-left: 0px"
              icon="el-icon-top"
              size="mini"
              >导入混凝土路面厚度钻芯法数据文件</el-button
            >
            <el-button
              class="btn-add"
              type="success"
        ...
        >
          ...
          <el-table-column label="序号" width="50px">
            ...
            <el-table-column prop="cz1" label="测值1(mm)" />
            <el-table-column prop="cz2" label="测值2(mm)" />
            <el-table-column prop="cz3" label="测值3(mm)" />
            <el-table-column prop="cz4" label="测值4(mm)" />
            <el-table-column prop="sjz" label="设计值(mm)" />
            <el-table-column prop="yxps" label="允许偏差(mm)" width="80px" />
        ...
        <img v-viewer style="height: 100px;width: 450px;" src="@/assets/填写说明/路面/混凝土路面厚度钻芯法.png" alt="" >
        ...
        <el-table-column prop="检测点数" label="检测点数" />
          <el-table-column prop="合格点数" label="合格点数" />
          
          <el-table-column prop="合格率" label="合格率(%)" />
          <el-table-column prop="允许偏差" label="允许偏差" />
        ...
        <el-dialog title="修改" :visible.sync="dialogVisible2" width="40%" >
          <el-form ref="dataForm" :model="modifyData"  label-width="150px" size="small" style="padding-right: 40px;" :rules="rules">
            ...
            <el-form-item label="测值1" prop="cz1">
            <el-input v-model="modifyData.cz1"></el-input>
            </el-form-item>
            <el-form-item label="测值2" prop="cz2">
            <el-input v-model="modifyData.cz2"></el-input>
            </el-form-item>
            <el-form-item label="测值3" prop="cz3">
            <el-input v-model="modifyData.cz3"></el-input>
            </el-form-item>
            <el-form-item label="测值4" prop="cz4">
            <el-input v-model="modifyData.cz4"></el-input>
            </el-form-item>
    ...
    <script>
    // 引入定义接口js文件
    import api from "@/api/project/fbgc/lmgc/hntlmhdzx";
    ...
    export default {
      ...
      methods: {
        ...
        onUploadSuccess() {
          ...
          let load=Loading.service({fullscreen:true,text:'加载中,请稍后!'})
          api.importhntlmhdzx(fd).then((res)=>{
            console.log('res',res)
            if(res.message=='成功'){
              this.fetchData();
              this.$message({
        ...
        exporthntlmhdzx() {
          ...
          api.exporthntlmhdzx().then((res) => {
            ...
            // 设置导出的文件名称
            link.download = `09混凝土路面厚度钻芯法实测数据` + ".xlsx";
