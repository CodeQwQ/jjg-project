<template>
    <div class="app-container">
        ...
        <div class="tools-div" v-show="!(username=='admin'&&type==1  )">
          <div class="anniu">
            <el-button
              class="btn-add"
              type="success"
              @click="exportlmwcbkmlf()"
              style="margin-left: 0px"
              icon="el-icon-bottom"
              size="mini"
              >导出路面弯沉贝克曼梁法文件</el-button
            >
            <!-- :disabled="$hasBP('bnt.ql.export')  === true" -->
            <el-button
              class="btn-add"
              type="success"
              @click="importlmwcbkmlf()"
              style="margin-left: 0px"
              icon="el-icon-top"
              size="mini"
              >导入路面弯沉贝克曼梁法数据文件</el-button
            >
            <el-button
              class="btn-add"
              type="success"
        ...
        >
          ...
          <el-table-column prop="phsdljdmhtml" label="平衡湿度路基顶面回弹模量EO" width="120px"/>
        ...
        <img v-viewer style="height: 100px;width: 450px;" src="@/assets/填写说明/路面/路面弯沉贝克曼梁法.png" alt="" >
        ...
        <el-form ref="dataForm" :model="modifyData"  label-width="210px" size="small" style="padding-right: 40px;" :rules="rules">
          ...
          <el-form-item label="平衡湿度路基顶面回弹模量EO" prop="phsdljdmhtml">
            <el-input v-model="modifyData.phsdljdmhtml"></el-input>
          </el-form-item>
          
          <el-form-item label="后轴重(T)" prop="hzz">
    ...
    <script>
    // 引入定义接口js文件
    import api from "@/api/project/fbgc/lmgc/lmwcbkmlf";
    ...
    export default {
      ...
      methods: {
        ...
        onUploadSuccess() {
          ...
          let load=Loading.service({fullscreen:true,text:'加载中，请稍后'})
          api.importlmwcbkmlf(fd).then((res)=>{
            console.log('res',res)
            if(res.message=='成功'){
              this.fetchData();
              this.$message({
        ...
        exportlmwcbkmlf() {
          ...
          api.exportlmwcbkmlf().then((res) => {
            ...
            // 设置导出的文件名称
            link.download = `02路面弯沉(贝克曼梁法)实测数据` + ".xlsx";
