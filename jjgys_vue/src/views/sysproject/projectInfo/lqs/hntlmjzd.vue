<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column prop="hntlmname" label="混凝土路面名称" />
      ...
      <el-table-column prop="zdlx" label="A/B/M(匝道桥填匝道标志)" />
      <el-table-column prop="wz" label="位置" />
      <el-table-column prop="htlmsshtd" label="互通路面所属合同段" />
...
<script>
// 引入定义接口js文件
import api from "@/api/project/lqs/hntlmzd.js";
  ...
  methods: {
    ...
    fileChange(file,fileList){
      ...
      onUploadSuccess() {
        ...
        fd.append('proname',this.$route.query.name)
        fd.append('userid',this.userid)
        let load=Loading.service({fullscreen:true,text:'加载中,请稍后!'})
        api.importhntlmzd(fd).then((res)=>{
          console.log('res',res)
          if(res.status=='200'){
            this.fetchData();
            this.$message({
